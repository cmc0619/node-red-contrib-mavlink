...
| `eslint-plugin-n` compatibility checks | error | `no-unsupported-features/es-builtins`, `no-unsupported-features/es-syntax`, and `no-unsupported-features/node-builtins` enforce the package's Node `>=20` floor, set in `eslint.config.mjs`'s `settings.node.version` (the plugin does not read `engines` here — both must move together). The builtins check allows experimental APIs; `node:test` and `fetch` are stable on this floor, so the Node-18-era `ignores` for `test`/`test.before`/`test.after` are gone, while newer modules such as `node:sqlite` still fail. |
