...
*Superseded (ruled 2026-08-12, #278):* the input-side frame vocabulary above is deleted with
the parsing layer — `lib/move` accepts only the numeric `MAV_FRAME` values the Action surface
derives (0 `GLOBAL`, 1 `LOCAL_NED`, 3 `GLOBAL_RELATIVE_ALT`, 8 `BODY_NED`, 9 `BODY_OFFSET_NED`);
string names, the `*_INT` aliases (names and numbers 5/6/11), `LOCAL_OFFSET_NED`, and the
terrain frames all throw as not a `SET_POSITION_TARGET` frame. Nothing changed on the wire:
global setpoints still transmit the `*_INT` twins per the ruling above, `COMMAND_INT` still
carries spec numbers 0/3, and every §14-measured pin is unchanged. What shrank is the
operator-facing vocabulary — the Action surface derives frames instead of accepting them, so
...
**PX4 accepts a one-shot `DO_REPOSITION` without an OFFBOARD stream (2026-08-11).**
*Wrong belief:* PX4 needs an OFFBOARD setpoint stream — or at minimum a mode pre-switch the
caller performs — before it will act on a guided goto, so a one-shot `DO_REPOSITION` is
ignored or denied and the honest shipping answer is a "PX4 requires OFFBOARD" advisory. The
belief is well-supported on paper: MAVSDK's `goto_location` pre-switches mode before sending,
