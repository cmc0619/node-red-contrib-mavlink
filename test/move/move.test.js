...
test('an initial send that throws leaves the stream stopped and propagates', () => {
  // start() sends before committing state, so a throwing first send never
  // marks the stream active or installs a timer — no zombie for a later stop()
  // to act on, and no try/catch needed to get there.
  const stream = createMoveStream({
    connection: {
