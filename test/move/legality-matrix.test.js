...
test('the ack-timeout re-send is gated on idempotency — a relative Turn never re-sends', async () => {
  // lib/command/ack.js: pass DEFAULT_MAX_RESENDS "only for a command
  // affirmatively known to tolerate re-issue", because re-sending is premised
  // on a lost command and a lost ack being indistinguishable. That premise is
  // per *command*, and it was lost when confirmReposition was generalised to
  // serve every acked action (Gitar, #303).
  //
  // A relative CONDITION_YAW is a delta: if the vehicle turned and only the ack
  // was lost, a re-send turns it again. Driven on the real node against a
  // connection that never acks, so the whole timeout path runs.
  const { AckWaiter } = require('../../lib/command');
  const seen = [];
  const realStart = AckWaiter.prototype.start;
  // Record the policy, then run the real waiter — it subscribes and sends, and
  // drive() feeds the matching ACCEPTED ack, so the whole confirm path executes
  // rather than being replaced by a stub that could pass while broken.
  AckWaiter.prototype.start = function patched() {
    seen.push(this._maxResends);
    return realStart.call(this);
  };

  try {
    for (const [label, config, expected] of [
      ['relative turn', { ...configFor('turn', 'confirm', 'relative'), connection: 'conn' }, 0],
      ['absolute turn', { ...configFor('turn', 'confirm', 'absolute'), connection: 'conn' }, 3],
      ['speed', { ...configFor('speed', 'confirm', 'groundspeed'), connection: 'conn' }, 3],
      ['reposition', { ...configFor('goto', 'confirm', 'home'), connection: 'conn' }, 3],
    ]) {
      seen.length = 0;
      const { node } = await drive(config, { firmware: 'ardupilot' });
      node.emit('close', () => {});
      assert.equal(seen.length, 1, `${label} armed exactly one waiter`);
      assert.equal(seen[0], expected, `${label} maxResends`);
    }
  } finally {
    AckWaiter.prototype.start = realStart;
  }
});
