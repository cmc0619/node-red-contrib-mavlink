...
test('backup retries the full list with the same collector and reports received count on failure', async () => {
  ...
  const done = new ParamBackup(machineOptions(stub, clock, { maxRetries: 1 })).start();
  clock.flush();
...
test('backup decode errors settle and tear down the subscription and timer', async () => {
  ...
  const clock = new FakeTimers();
  stub.onSend(() => {});
  const machine = new ParamBackup(machineOptions(stub, clock));
...
test('backup preserves signed zero and nonfinite REAL32 values through JSON and the wire', async () => {
  ...
  for (const value of saved) {
    ...
    const backup = await new ParamBackup(machineOptions(stub, clock, { encoding: 'c-cast' })).start();
    ...
    const restored = await new ParamRestore(machineOptions(restoreStub, restoreClock, {
      encoding: 'c-cast',
      params: roundTripped,
    })).start();
...
test('restore round-trips bytewise and c-cast saved values sequentially', async () => {
  for (const [encoding, params] of [
    ...
    const outcome = await new ParamRestore(machineOptions(stub, clock, {
      encoding,
...
test('restore waits for a matching echo and preserves confirmed prefix on failure', async () => {
  ...
  const done = new ParamRestore(machineOptions(stub, clock, { params, maxRetries: 1 })).start();
  clock.flush();
  const outcome = await done;
...
test('restore ignores wrong source and wrong echo before confirming a parameter', async () => {
  ...
  const outcome = await new ParamRestore(machineOptions(stub, clock, { params })).start();
...
test('restore retries only the current parameter with a bounded ceiling', async () => {
  ...
  const done = new ParamRestore(machineOptions(stub, clock, { params, maxRetries: 2 })).start();
  clock.flush();
  const outcome = await done;
...
test('backup and restore cancel cleanly with partial result information', async () => {
  ...
  const backupMachine = new ParamBackup(machineOptions(backupStub, backupClock));
  ...
  const restoreMachine = new ParamRestore(machineOptions(restoreStub, restoreClock, { params }));
