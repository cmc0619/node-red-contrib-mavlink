...
test('fieldMetaFromBundle renders no control for the driver-owned Sequence slot', () => {
  ...
  // The sequence number is bookkeeping the driver stamps per camera (471#115);
...
test('every recipe derives exactly the fields the dialog shows', () => {
  ...
  const expected = {
    ...
    'camera|stop-stream|': ['cameraId', 'streamId'],
    'camera|track-point|': ['cameraId', 'pointX', 'pointY', 'radius'],
    'camera|track-rectangle|': ['bottomRightX', 'bottomRightY', 'cameraId', 'topLeftX', 'topLeftY'],
    'camera|stop-tracking|': ['cameraId'],
    'camera|storage-format|': ['format', 'resetImageLog', 'storageId'],
    ...
    'gimbal|configure|': ['gimbalDeviceId', 'primaryCompid', 'primarySysid', 'secondaryCompid', 'secondarySysid'],
    'gimbal|take|': ['gimbalDeviceId'],
    'gimbal|release|': ['gimbalDeviceId'],
