...
test('orbit params with JSON "NaN" center coerce to numeric NaN in the param array', () => {
  const user = mergeParams(
    JSON.parse('{"1":100,"2":5,"3":0,"5":"NaN","6":"NaN","7":"NaN"}'),
    {}
  );
...
test('absent orbit centre, velocity and altitude encode the spec NaN sentinels; explicit values win', () => {
  ...
  const user = mergeParams(JSON.parse('{"1":100,"2":5,"3":0}'), {});
  const arr = buildParamArray(getPreset('orbit'), user);
  ...
  const placed = buildParamArray(getPreset('orbit'), mergeParams(JSON.parse('{"5":47.4,"6":8.5}'), {}));
  assert.equal(placed[4], 47.4);
  assert.equal(placed[5], 8.5);

  const blankVelocity = buildParamArray(getPreset('orbit'), mergeParams(JSON.parse('{"1":100}'), {}));
...
test('blank is absent, and everything else still merges as before (#141)', () => {
  ...
  assert.ok(Number.isNaN(mergeParams(JSON.parse('{"5":"NaN"}'), {})[5]));
  assert.ok(Number.isNaN(mergeParams(JSON.parse('{}'), { 5: NaN })[5]));

  // Ordinary values, including the ones a blank check could plausibly eat.
  assert.equal(mergeParams(JSON.parse('{}'), { 1: 0 })[1], 0, 'explicit zero is a value');
  assert.equal(mergeParams(JSON.parse('{}'), { 7: -35.2 })[7], -35.2);
  assert.equal(mergeParams(JSON.parse('{}'), { 2: '47.4' })[2], 47.4, 'numeric strings still coerce');
  ...
  for (const blank of ['', '   ', '\t', null]) {
    assert.equal(mergeParams(JSON.parse('{}'), { 5: blank })[5], undefined, JSON.stringify(blank));
  }

  // And a blank override no longer wipes a configured value to 0 — it falls
  // through to what the node was configured with.
  assert.equal(mergeParams(JSON.parse('{"5":47.4}'), { 5: '' })[5], 47.4);
});
