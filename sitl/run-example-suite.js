...
const PROFILE = {
  ...
  '44-system-ops': {
    ...
    notes:
      ...
      'capture is a valid PASS. Empty log list on cold SITL is still succeeded. Status debugs drop ' +
      'progress so the suite scrape stays on terminal records.',
  },
  '04-param-defs-live': {
    restart: 'none',
    waitMs: 20000,
...
function verdictFrom(profile, summary, log) {
  ...
  if (/system logs list, FTP round-trip, and backup/i.test(expect)) {
    ...
    // have to have transferred for that to count. Progress records share the
    // same debug tag, so take the last terminal result — find() would stop on
    // the first 'progress' and green nothing after a finished backup.
    const lastTerminal = (re) => {
      let hit = null;
      for (const d of summary.debug) {
        if (!re.test(d.tag) || !d.result || d.result === 'progress') continue;
        hit = d;
      }
      return hit;
    };
    const logList = lastTerminal(/log list status/i)?.result === 'succeeded';
    const ftpUp = lastTerminal(/ftp upload status/i)?.result === 'succeeded';
    const ftpDown = lastTerminal(/ftp download status/i)?.result === 'succeeded';
    const roundTrip = lastTerminal(/ftp roundtrip assert/i)?.result === 'succeeded';
    const backup = lastTerminal(/backup status/i);
    const backupOk =
      Boolean(backup) &&
