...
session record. Shared editor glue lives in `resources/mavlink-editor.js` (not Local Identity).
...
- Shared resource: `test/nodes/mavlink-editor-resource.test.js`
...
- Runtime: Build + concrete dialect does not inherit profile targets; Param/Mission firmware
  from field vs profile; Mission confirm without Connection fails loud
...
- `DESIGN.md` §6 / §14 updated in the same change as this spec
