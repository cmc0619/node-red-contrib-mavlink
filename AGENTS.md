...
**Boundary (owner rulings, 2026-08-14 — the selection-typo cluster, §14):** the rules above
forbid *re-validating* what the editor bounded — ranges, formats, required-ness. They do not
forbid the crater on a **closed-vocabulary token that selects a wire behavior** (action,
delivery, sendAs, preset, band, firmware, altRef, reference): membership there is resolved at
runtime and an unknown token throws naming the vocabulary, because a wrong pick is a wrong
wire message delivered with a clean success report, and most of these are payload-overridable
