...
> This is the repository README, shown on GitHub. The shorter [root `README.md`](../README.md)
> is what ships in the npm tarball — it drops the sections below that only apply to a git
> checkout (Docker bind-mounts, the SITL lab, development).
...
[`sitl/`](../sitl/README.md). That short guide is the operator entry point for ports,
`compose up`, and log checks.

`sitl/` is a repository-only directory — it is not part of the npm package.

## Examples

Importable flows live in [`examples/`](../examples), and they **do** ship in the npm package.
