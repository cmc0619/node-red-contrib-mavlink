 ...
 * {@link DialectBundle}. It touches no
 ...
 * @property {string|undefined} special  generated MAVLink field marker
 * @property {number|undefined} constValue  generated wire constant
 ...
 * to `uint8_t` and carries a special marker; a scalar carries
 * `arrayLength: null`.
 *
 * @param {string} raw
 * @returns {{type: string, arrayLength: number|null, special: string|undefined}}
 */
function splitType(raw) {
  const match = raw.match(/^(.*?)\[(\d+)\]$/);
  let type = match ? match[1] : raw;
  const arrayLength = match ? Number(match[2]) : null;
  const special = type === 'uint8_t_mavlink_version' ? 'mavlink_version' : undefined;
  if (type === 'uint8_t_mavlink_version') {
    type = 'uint8_t';
  }
  return { type, arrayLength, special };
}
 ...
 * @property {number} version  pymavlink's default is MAVLink 2
 * @property {ParsedEnum[]} enums
 * @property {ParsedMessage[]} messages
 */

/**
 * Parse one dialect XML file into its includes, enums, and messages.
...
function parseFile(fileName, xml) {
  ...
  const out = { includes: [], version: 2, enums: [], messages: [] };
...
function parseField(fieldNode, extension) {
  ...
  const { type, arrayLength, special } = splitType(attrs['@_type']);
  const field = {
    name: attrs['@_name'],
    type,
    arrayLength,
 ...
 * @param {Map<string, ParsedFile>} [parsedByFile]  files already parsed by the
 *   caller (the catalog's include walk); the rest are parsed here
 * @returns {DialectBundle}
 */
function compileXml(files, entryFileName, parsedByFile = new Map()) {
  const order = resolveIncludeOrder(files, entryFileName, parsedByFile);

  /** @type {Map<string, {name:string,bitmask:boolean,description:string|null,entries:Map<string,{value:number|string,description:string|null,hasLocation:boolean|null,params:ParsedParam[]}>}>} */
  const enums = new Map();
  /** @type {Map<string, ParsedMessage>} */
  ...
  for (const fileName of order) {
    ...
    mergeEnums(enums, parsed.enums);
    mergeMessages(messages, idToName, parsed.messages, fileName, parsed.version);
  }
 ...
 * name; a same-name redefinition nearer the entry overrides (later wins), while
 * a same-name-same-value duplicate (a diamond include) is absorbed silently.
 *
 * @param {Map} enums
 * @param {ParsedEnum[]} fileEnums
 * @returns {void}
 */
function mergeEnums(enums, fileEnums) {
  for (const e of fileEnums) {
    ...
    for (const entry of e.entries) {
      ...
      // Prefer the newer non-null detail.
 ...
 * nearer the entry overrides (resolved by include order) and frees its previous
 * id; a different message claiming an id already live is a collision and fails
 * loud.
 *
 * @param {Map} messages
 * @param {Map} idToName
 * @param {ParsedMessage[]} fileMessages
 * @param {string} fileName  for the collision message
 * @param {number} fileVersion  version used for generated constants in this file
 * @returns {void}
 */
function mergeMessages(messages, idToName, fileMessages, fileName, fileVersion) {
  for (const msg of fileMessages) {
    const existing = messages.get(msg.name);
    if (existing && idToName.get(existing.id) === msg.name) {
    ...
    messages.set(msg.name, {
      ...msg,
      fields: msg.fields.map((field) => field.special
        ? { ...field, constValue: fileVersion }
        : field),
    });
...
function assembleBundle(merged) {
  ...
  for (const e of merged.enums.values()) {
    ...
    // `entries`. XML lets two names share a value (deprecated aliases); the
    // first declared name is the one `byValue` shows.
    const byName = {};
    const byValue = {};
...
module.exports = { compileXml, parseFile, resolveIncludeOrder };
