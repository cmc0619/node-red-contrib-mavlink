...
const { dialectFromVehicleId } = require('../addressing/dialect');

/**
 * Resolve a dialect catalog source from `?vehicle=` / `?dialect=` (DESIGN.md §6).
 *
 * Prefer a deployed Vehicle Profile's compiled bundle (custom-safe). Never invent
 * a bundled dialect under a bare `vehicle=` key. Strict mode returns HTTP error
 * bodies; soft mode returns empty notices (Payload field-tips).
 *
 * @param {object} RED
...
function resolveCatalogSource(RED, api, query, opts = {}) {
  ...
  if (vehicleId) {
    try {
      const bundle = dialectFromVehicleId(RED, vehicleId, { rethrow: true });
      if (bundle) {
        const vehicleNode = RED.nodes.getNode(vehicleId);
        return {
          kind: 'bundle',
          bundle,
          dialect: (vehicleNode && vehicleNode.dialect) || bundle.dialect || 'custom',
        };
      }
    } catch (err) {
      // getDialect failures can include filesystem paths — keep the client
      // body generic; callers that need the detail should log err.message.
      return {
        kind: 'error',
        status: 400,
        body: {
          error: 'dialect unavailable',
          dialects: dialects(),
        },
    ...
    (req, res) => {
      ...
      try {
        ...
        if (source.kind === 'error') {
          if (source.detail && RED.log && typeof RED.log.error === 'function') {
            RED.log.error(`[${logLabel}] catalog dialect unavailable: ${source.detail}`);
          }
