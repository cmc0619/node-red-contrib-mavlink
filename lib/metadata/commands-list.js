 ...
 * @property {object[]} params  editor-ready param specs (including `hidden`)
 */
...
/**
 * Enum option label: prefer the entry description, always show the numeric
 * value in parentheses (§6).
 *
...
function enumOptionLabel(entry) {
  let text = entry.description && String(entry.description).trim()
    ...
    : entry.name;
  // Keep the first sentence / line so long descriptions stay compact in a select.
...
/**
 * Full Advanced-mode catalog for one bundled dialect: commands (with param
 * specs) plus the enum tables those params reference.
 *
 * @param {string} dialect
 * @returns {{dialect: string, commands: CommandListEntry[], enums: Object<string, Array<{name:string,value:number,label:string}>>}}
 */
function listCommandsCatalog(dialect) {
  ...
  return { dialect: name, commands, enums };
}
