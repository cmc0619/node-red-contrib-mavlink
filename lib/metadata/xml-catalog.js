...
class XmlCatalog {
  ...
  async update(opts = {}) {
    ...
    while (queue.length) {
      ...
      const incs = parseFile(file, text).includes;
...
function compileXmlFromFile(entryPath, opts = {}) {
  ...
  while (queue.length) {
    ...
    files[name] = text;
    for (const inc of parseFile(name, text).includes) {
      if (!seen.has(inc)) {
 ...
 * with a path separator or a `..` segment is rejected here as
...
function makeSnapshotId(repo, ref, commit, downloadedAt) {
  ...
  return `${sanitize(repo)}-${sanitize(ref)}-${commit.slice(0, 7)}-${stamp}`;
}
 ...
 *
 * @param {import('./compile').Message} msg
 * @returns {string}
 */
 ...
 *
 * @param {import('./compile').DialectBundle} bundled  installed bundled dialect
 * @param {import('./compile').DialectBundle} downloaded  compiled downloaded XML
 * @returns {object}
 */
...
function dialectLibrary(catalog) {
  ...
  for (const m of catalog.list()) {
    const banned = new Set(m.unusable.map((u) => u.file));
    ...
    for (const { name: fileName } of m.files) {
      ...
      if (!catalog.filePath(fileName, m.snapshotId)) continue;
      ensure(dialect).push({
        id: m.snapshotId,
        kind: 'snapshot',
