...
/**
 * Downloadable MAVLink XML dialect catalog (DESIGN.md §4; adapted from the
 * mavlink-ai catalog).
 *
 * This does NOT add a third runtime dialect mode: downloaded XML files are just
 * managed **Custom** XML paths that {@link compileXml} already knows how to
 * load. What the catalog adds is a local cache of official MAVLink XML
...
function compileXmlFromFile(entryPath, opts = {}) {
  ...
  if (!raw) {
    throw codedError('XML_DIALECT_NO_PATH', 'A custom dialect XML path is required.');
  }
 ...
 * @param {string} name
 * @returns {string}
 * @throws {Error} XML_CATALOG_BAD_FILE
 */
function normalizeFileName(name) {
  const base = path.basename(String(name).trim());
  if (!base || base !== String(name).trim() || !/^[\w.-]+\.xml$/i.test(base)) {
    throw codedError('XML_CATALOG_BAD_FILE', `Unsafe or invalid XML file name '${name}'.`, { name });
  }
  return base;
}
