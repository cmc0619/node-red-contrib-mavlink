...
const { extractIncludes } = require('./xml-catalog');
...
async function fetchDialect(opts) {
  ...
  if (typeof listFiles === 'function') {
    ...
    // An empty listing is a real answer ("nothing here"), not "skip validation".
    if (Array.isArray(available) && !available.includes(entry)) {
      throw new Error(
...
module.exports = { REMOTE_SOURCES, fetchDialect };
