...
function loadBundled(name) {
  ...
  const manifest = readManifest();
  const row = (manifest.dialects || []).find((d) => d.name === key);
  if (!row) {
    throw new Error(
  ...
  if (!bundle) {
    // compileXml names the bundle after the entry file (`uAvionix`) and cannot
    // know where the XML came from; the seed's key and provenance are ours.
    bundle = Object.assign({}, compileXml(loadPacked().sources, row.entry), {
      dialect: key,
      fetched: {
        repo: manifest.repo,
