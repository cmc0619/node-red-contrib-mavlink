...
function loadPacked() {
  ...
  if (!parsed || !parsed.manifest || !parsed.sources) {
...
function seedEntryFor(name) {
  ...
  const row = (readManifest().dialects || []).find((d) => d.name === key);
  if (!row) {
    throw new Error(
...
function writeCompiledCache(key, bundle) {
  ...
  fs.writeFileSync(
    file,
    JSON.stringify({ stamp: seedStamp(), commit: manifest.commit, commitDate: manifest.commitDate || null, bundle })
  );
}

/**
 * Drop every compiled dialect, in memory and on disk, so the next load
 * recompiles from the current seed. The only way a cache entry is replaced —
 ...
 * then cached on disk. Unknown names fail loud naming the available set — no
...
function loadBundledSet(names) {
  const keys = names.map((n) => String(n || '').toLowerCase()).filter(Boolean);
  const key = keys.join('+');
  if (cache.has(key)) {
  ...
  const rows = keys.map((k) => {
    const row = (manifest.dialects || []).find((d) => d.name === k);
    if (!row) {
      throw new Error(
        `Unknown bundled dialect '${k}'. Available: ${knownDialects().join(', ')}.`
      );
    }
    return row;
  });
  ...
  if (!bundle) {
    ...
    if (rows.length > 1) {
      entry = PROFILE_ENTRY;
      const includes = rows.map((r) => `<include>${r.entry}</include>`).join('');
      files = Object.assign({}, sources, {
        [PROFILE_ENTRY]: `<?xml version="1.0"?><mavlink>${includes}</mavlink>`,
      });
    }
    // compileXml names the bundle after the entry file (`uAvionix`, or the
    // synthetic one) and cannot know where the XML came from; the seed's key
    // and provenance are ours.
    bundle = Object.assign({}, compileXml(files, entry), {
      dialect: keys.join('+'),
      fetched: {
        repo: manifest.repo,
