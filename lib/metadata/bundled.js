...
/**
 * The installed XML parser's package.json, found by walking up from the
 * file `require` resolves — the package exports no `./package.json` path.
 *
 * @returns {Buffer}
 */
function parserPackageJson() {
  let dir = path.dirname(require.resolve('fast-xml-parser'));
  while (!fs.existsSync(path.join(dir, 'package.json'))) dir = path.dirname(dir);
  return fs.readFileSync(path.join(dir, 'package.json'));
}

/**
 * Compiler stamp: a short hash of everything that shapes a compiled bundle —
 * the compiler, this module's bundle assembly, and the installed XML parser's
 * package manifest — in every cache entry's file name. An entry written under
 * another stamp is another name, never read, so the dialect recompiles. Seed
 * changes invalidate nothing; the operator's rebuild sweeps every entry.
 */
const COMPILER_STAMP = createHash('sha256')
  .update(fs.readFileSync(path.join(__dirname, 'compile.js')))
  .update(fs.readFileSync(__filename))
  .update(parserPackageJson())
  .digest('hex')
  .slice(0, 8);
