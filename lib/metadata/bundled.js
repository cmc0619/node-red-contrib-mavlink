...
/** Bump when compiled bundle metadata changes shape. */
...
function readCompiledCache(key) {
  ...
  const file = path.join(compiledCacheDir, `${key}@seed-v${COMPILED_CACHE_VERSION}.json`);
...
function writeCompiledCache(key, bundle) {
  ...
  fs.writeFileSync(
    path.join(compiledCacheDir, `${key}@seed-v${COMPILED_CACHE_VERSION}.json`),
