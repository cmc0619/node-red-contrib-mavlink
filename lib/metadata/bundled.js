 ...
 * graph so `files` reads as the real dependency order. Dialects that include
 * `common.xml` expand to `minimal` -> `standard` -> `common` -> entry;
 * dialects with no includes (e.g. `icarous`) are self-contained — never force
 * MSC onto them (§4: never assume a dialect includes common).
 *
