...
function readCompiledCache(key) {
  ...
  // A DialectBundle carries `enums` at the top level. Older writers stored
  // `{ stamp, commit, commitDate, bundle }` — treat that shape as a miss so
  // the next load recompiles rather than handing callers a wrapper with no
  // enums (Connection then crashes on bundle.enums.MAV_TYPE).
  if (!parsed || typeof parsed !== 'object' || !parsed.enums) return null;
  return parsed;
}
...
/**
 * Compile several roots as one profile: the synthetic entry includes each in
 * turn, so the include chain resolves exactly as MAVLink defines it — shared
 * files appear once, a later root overrides an earlier one. One owner for the
 * assembly, here and for lib/vehicle's snapshot-mixing profiles.
 *
 * @param {Object<string, string>} files  every XML the roots reference, by file name
 * @param {string[]} entries  root entry file names, in selection order
 * @param {string} key  the bundle's dialect key
 * @returns {import('./compile').DialectBundle}
 */
function compileProfile(files, entries, key) {
  return {
    ...compileXml({ ...files, [PROFILE_ENTRY]: profileEntry(entries) }, PROFILE_ENTRY),
    dialect: key,
  };
}
...
function loadBundledSet(names) {
  ...
  if (!bundle) {
    ...
    bundle = rows.length > 1
      ? compileProfile(sources, rows.map((r) => r.entry), key)
      : { ...compileXml(sources, rows[0].entry), dialect: key };
