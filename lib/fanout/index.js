...
function matchesFilter(member, filter) {
  // armed: the editor select writes 'true' / 'false'; a payload filter may
  // carry the boolean itself. Compared as spelled: any other value matches nobody.
  return (filter.type === undefined || Number(filter.type) === Number(member.type))
    ...
    && (filter.armed === undefined || String(filter.armed) === String(member.armed));
}
