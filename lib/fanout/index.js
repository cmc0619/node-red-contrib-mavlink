...
async function executeSequential(options, members, warnings, startedAt) {
  ...
  for (let i = 0; i < members.length; i += 1) {
    ...
    // The pause takes the cancel signal so it can clear its own timer. An
    // injected `wait` (tests) ignores the second argument and resolves on its
    // own terms, which is fine — those never park for real time.
    if (i > 0 && intervalMs > 0) await wait(intervalMs, cancel && cancel.stopped);
...
async function confirmMember(options, member, built) {
  ...
  // Publish the in-flight waiter so a close can cut the current member short
  // instead of waiting out its timeout and retries.
  if (options.cancel) options.cancel.hold(waiter);
  let outcome;
  try {
    outcome = await waiter.start();
  } finally {
    if (options.cancel) options.cancel.release(waiter);
  }
...
function confirmParamMember(options, member, built) {
  return new Promise((resolve) => {
    ...
    // closed (Greptile #140).
    if (options.cancel) {
      options.cancel.stopped.then(() => settle({
        result: 'cancelled',
        success: false,
        message: built.message,
        confirmedBy: 'none',
...
function confirmBroadcast(options, members, built, target) {
  ...
  return new Promise((resolve) => {
    ...
    // run as cancelled.
    if (options.cancel) options.cancel.stopped.then(settle);
    options.connection.send(built.message, sendOptions(options, built, target));
...
function aggregateFromMembers(options, mode, members, warnings, startedAt, message = null) {
  ...
  const cancelled = !!(options.cancel && options.cancel.cancelled);
  return aggregateRecord({
...
/**
 * Inter-member pause, cut short when `stopped` resolves.
 *
 * The timer is cleared rather than merely raced: an orphaned `setTimeout` keeps
 * the event loop alive for the rest of the interval, which on a redeploy means
 * Node-RED cannot settle until a pause nobody is waiting for finally fires.
 *
 * @param {number} ms
 * @param {Promise<void>} [stopped]  cancel signal from {@link createFanoutCancel}
 * @returns {Promise<void>}
 */
function sleep(ms, stopped) {
  return new Promise((resolve) => {
...
/**
 * A handle that stops an in-flight sequential fan-out.
 *
 * Node-RED's `close` does not abort a running promise chain, so without this a
 * redeployed fan-out keeps walking its member list — sending arm, mode and
 * takeoff commands to real vehicles from a node that no longer exists, for up
 * to members × (timeout + interval).
 *
 * Two things have to stop: the loop, which checks {@link cancelled} between
 * members, and the member currently waiting on a COMMAND_ACK, which is held
 * here so `cancel()` can reach it rather than letting it time out.
 *
 * @returns {{cancelled: boolean, cancel: Function, hold: Function, release: Function}}
 */
function createFanoutCancel() {
  let cancelled = false;
  let waiter = null;
  let signal;
  // Resolves on cancel, so the inter-member pause can be raced rather than
  // waited out. Without this a redeploy during a long interval blocks `close`
  // for the rest of that interval, and Node-RED's close has its own timeout.
  const stopped = new Promise((resolve) => { signal = resolve; });
  return {
    get cancelled() {
      return cancelled;
    },
    /** @returns {Promise<void>} resolves when {@link cancel} is called */
    get stopped() {
      return stopped;
    },
    cancel() {
    ...
    /** @param {object} next  the AckWaiter now in flight */
    hold(next) {
      waiter = next;
      // Cancelled between members and this one still started: stop it now
      // rather than letting it run its full timeout.
      if (cancelled) waiter.cancel();
    },
    /** @param {object} done  clear only if it is still the one we hold */
    release(done) {
