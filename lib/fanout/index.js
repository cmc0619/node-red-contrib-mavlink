...
const { isBlank } = require('../addressing');
...
/** Execution-mode vocabulary (`options.mode`) — which wire machine runs. */
const EXECUTION_MODES = ['sequential', 'broadcast'];

/**
 * Execution-mode membership (owner ruling, 2026-08-14, selection-typo
 * cluster). This one picks which wire machine runs — `executeSequential` or
 * `executeBroadcast` — so a typo used to stay truthy past
 * `options.mode || 'sequential'`, miss every `mode === 'broadcast'` gate
 * below, and run a sequential fan-out with none of the broadcast confirm
 * gates for an operator who asked for Broadcast (confirmed:
 * `mode: 'broadcasts'`). Absent/blank resolves to 'sequential' — the
 * documented API default (`nodes/mavlink-fanout.js`'s own comment, and the
 * editor's `executionMode` select, which has no blank option and defaults to
 * 'sequential') — a present non-member throws naming the token.
 *
 * @param {*} value  options.mode
 * @returns {'sequential'|'broadcast'}
 */
function resolveExecutionMode(value) {
  if (isBlank(value)) return 'sequential';
  if (EXECUTION_MODES.includes(value)) return value;
  throw new Error(
    `unknown Fan-out execution mode ${JSON.stringify(value)} — expected one of ${EXECUTION_MODES.join(', ')}`
  );
}

/** Selection-mode vocabulary (`selection.mode`) — how the member group is chosen. */
const SELECTION_MODES = ['all', 'list', 'filter'];

/**
 * Selection-mode membership (owner ruling, 2026-08-14, reversing this arm of
 * the 2026-08-09 trusted-author ruling — §14 "Fan-out selection"). The mode
 * used to act only through two negative `===` gates, so an unrecognised
 * token — the audit's live repro was `{mode: 'lits', sysids: [1]}` — selected
 * every active vehicle by *failing* both gates: the sysid list was never
 * read, and a subset command widened to the fleet under a clean success
 * report. Dispatch is affirmative now: "it has to equal x, y or z — a
 * present non-member must not default to an action via `!==`". Blank stays
 * 'all', the documented default, so no deployed flow changes. Selection
 * *content* stays trusted as given — sysids (their uint8 strictness is ruled
 * separately in selectFanoutMembers), filter values, per-member patches —
 * and the #226 loud/quiet empty split is untouched.
 *
 * @param {*} value  selection.mode
 * @returns {'all'|'list'|'filter'}
 */
function resolveSelectionMode(value) {
  if (isBlank(value)) return 'all';
  if (SELECTION_MODES.includes(value)) return value;
  throw new Error(
    `unknown Fan-out selection mode ${JSON.stringify(value)} — expected one of ${SELECTION_MODES.join(', ')}`
  );
}
...
function selectFanoutMembers(peerTable, selection = {}) {
  const mode = resolveSelectionMode(selection.mode);
...
async function executeFanout(options) {
  ...
  // Validated once, up front — before any member's send is in flight. Every
  // later `deliveryTier(options)` call (executeMember, executeBroadcast)
  // reads the same immutable `options.delivery`, so this is the only call
  // that can throw; a throw here rejects synchronously, before concurrency
  // and Promise.race enter the picture, rather than surfacing as a stray
  // rejection deep in a per-member launch loop.
  deliveryTier(options);
...
/** Delivery tiers the editor's `delivery` select can save (§10). */
const FANOUT_DELIVERY_TIERS = ['build', 'send', 'confirm'];

/**
 * Delivery-tier membership (owner ruling, 2026-08-14, selection-typo cluster
 * — the `lib/fanout delivery` site named alongside Command and Mission).
 * `options.delivery || 'build'` stayed truthy for a typo and missed both
 * `tier === 'build'` and `tier === 'confirm'` gates in `executeMember` /
 * `executeBroadcast`, falling into the plain send path: a hand-edited
 * `delivery: 'cofnirm'` silently ran Send — a real message left the wire
 * with none of Confirm's ack-wait — and reported `sent` for a tier nobody
 * asked for. Blank resolves to 'build', the editor's own default (the
 * `delivery` select has no blank option, so a blank here is unclaimed
 * config, not an operator choice); present non-member throws.
 *
 * @param {object} options
 * @returns {'build'|'send'|'confirm'}
 */
function deliveryTier(options) {
  const value = options.delivery;
  if (isBlank(value)) return 'build';
  if (FANOUT_DELIVERY_TIERS.includes(value)) return value;
  throw new Error(
    `unknown Fan-out delivery ${JSON.stringify(value)} — expected one of ${FANOUT_DELIVERY_TIERS.join(', ')}`
  );
}
