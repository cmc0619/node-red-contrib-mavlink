...
const { AckWaiter, MAV_RESULT, DEFAULT_TIMEOUT_MS } = require('../command');
...
function classifyMessage(message, mode) {
  ...
  if (name.startsWith('MISSION_')) {
    return { refusal: 'Mission messages are part of a multi-message transaction the vehicle drives and cannot be replicated by Fan-out' };
  }
  ...
  if (name.startsWith('SET_POSITION_TARGET_')) {
...
async function executeSequential(options, members, warnings, startedAt) {
  ...
  if (stopOnError && !stopped && !(signal && signal.aborted) && anyFailed()) {
...
function confirmBroadcast(options, members, message, target) {
  ...
  return new Promise((resolve) => {
    ...
    const unsubscribe = options.connection.subscribe({ message: 'COMMAND_ACK' }, (decoded) => {
      ...
      // GCS on a shared link is not ours — same gate as AckWaiter.
      if (
        ourIds &&
        fields.target_system !== undefined &&
        Number(fields.target_system) !== 0 &&
        Number(fields.target_system) !== Number(ourIds.sysid)
      ) {
        return;
      }
...
function memberMessage(options, member) {
  ...
  return {
    ...base,
    fields: { ...base.fields, ...patch, target_system: member.sysid },
  };
}
...
/**
 * Our own source sysid/compid for ack attribution, when the connection can
 * resolve them (§9/§10). Stubs without the accessor leave the gate off.
 *
 * @param {object} options
 * @returns {{sysid: number, compid: number}|null}
 */
function sourceIds(options) {
  return typeof options.connection.resolveSourceIds === 'function'
    ? options.connection.resolveSourceIds(options.identityId)
    : null;
}
