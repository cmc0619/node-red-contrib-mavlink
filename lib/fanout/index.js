...
async function executeSequential(options, members, startedAt) {
  ...
  for (; i < members.length; i += 1) {
    ...
    const component = options.connection.peerTable.getComponent(member.sysid, member.compid);
    if (!component || !isActive(component)) {
      records[index] = memberRecord(member, {
        result: 'failed',
        success: false,
        detail: `member ${component ? component.state : 'expired'}`,
      });
      continue;
    }
...
function confirmBroadcastAck(options, members, message, target) {
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(settle, Number(options.timeoutMs));
...
/**
 * Address a built message to one member. The system field is
 * `target_system`, except on `MANUAL_CONTROL`, which names it `target` — the
 * one message that does; stamping `target_system` there is dropped by the
 * serializer and every member's packet keeps the original vehicle. Messages
 * addressing a system but not a component (SET_MODE, MANUAL_CONTROL) keep
 * their shape: a field the message does not declare is never invented.
 *
 * @param {{name: string, fields: object}} message
 * @param {{sysid:number, compid:number}} member
 * @returns {{name: string, fields: object}}
...
function retarget(message, member) {
  ...
  if (message.fields.target !== undefined) fields.target = member.sysid;
  else fields.target_system = member.sysid;
