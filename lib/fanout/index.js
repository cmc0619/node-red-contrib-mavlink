...
/** Affirmative dispatch: a member returns, anything else — blank included — throws. */
function resolveSelectionMode(mode) {
  switch (mode) {
    case 'all':
    case 'list':
    case 'filter':
    ...
    default:
      throw new Error(
        `unknown Fan-out selection mode ${JSON.stringify(mode)} — expected one of all, list, filter`
      );
  }
}
...
function deliveryTier(options) {
  switch (options.delivery) {
    ...
    default:
      throw new Error(
        `unknown Fan-out delivery ${JSON.stringify(options.delivery)} — expected one of ${FANOUT_DELIVERY_TIERS.join(', ')}`
      );
  }
}
