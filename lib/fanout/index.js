...
const { AckWaiter, ackAddressedTo, sendFnFor, MAV_RESULT, isGlobalFrame } = require('../command');
...
async function confirmMember(options, member, message) {
  ...
  const waiter = new AckWaiter({
    ...
    // The LONG carrier's confirmation byte is stamped per (re-)send; INT
    // rides as built — the one owner of that rule is lib/command's sendFnFor.
    sendFn: sendFnFor(options.connection, message, sendOptions(options, member)),
...
/**
 * Our own source sysid/compid for ack attribution (§9/§10). An identity that
 * does not resolve fails on the send path, before any ack can arrive.
 *
 * @param {object} options
