...
function matchesFilter(member, filter) {
  ...
  // The editor select writes 'true' / 'false'; a payload filter may carry
  // the boolean itself. Compared as spelled: any other value matches nobody.
  if (filter.armed !== undefined && String(filter.armed) !== String(member.armed)) return false;
  return true;
}
