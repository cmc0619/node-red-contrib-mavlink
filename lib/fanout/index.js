...
const { MAV_RESULT } = require('../command/status-record');
const { ackWaiterFor, ackRecordFields, ackAddressedTo } = require('../command/ack');
const { isGlobalFrame } = require('../command/carrier');
 ...
 * `target_system` (`target` on MANUAL_CONTROL) and `target_component` per
 * member. What the message *means* is inferred from its name; how it is
 * confirmed rides along:
 *
 ...
 *   `{sysid, north?, east?, up?, patch?}`. Consulted only when the
...
function executeFanout(options) {
  ...
  // Config members become the targets array only when the payload
  // supplied none.
  ...
  if (members.length === 0) {
    ...
    // loud/quiet decision branches on it: a filter matching zero
...
async function executeSequential(options, members, startedAt) {
  ...
  // or timing-out straggler does not delay the rest by timeout × retries.
  // The inter-member pause still paces every *launch* but the first, so at
  // concurrency 1 the launch/pause/launch cadence is a sequential loop.
  // Aborts (redeploy) halt further launches; members
...
async function executeBroadcast(options, members, startedAt) {
  ...
  const each = (fields) => members.map((member) => memberRecord(member, fields));
  // A tier the select cannot save matches no arm and every member reports so.
  let records = members.map((member) => unmatchedTierRecord(options, member, message));
  try {
    switch (options.delivery) {
      case 'build':
        records = members.map((member) => memberRecord(member, {
          result: 'built',
          success: true,
        ...
        break;
      case 'confirm': {
        const refused = broadcastStreamRefusal(options, members, startedAt);
        if (refused) return refused;
        records = await confirmBroadcast(options, members, message, target);
        break;
      }
      case 'send': {
        const refused = broadcastStreamRefusal(options, members, startedAt);
        ...
        records = each({ result: 'sent', success: true, message });
        break;
      }
      default: break; // This space intentionally left blank (§5)
    }
  } catch (err) {
    records = each({ result: 'failed', success: false, message, detail: err.message });
  }
  return aggregateFromMembers(options, 'broadcast', records, startedAt, message);
}
 ...
 * a single held lock refuses the whole broadcast (mirrors the
 ...
 */
function broadcastStreamRefusal(options, members, startedAt) {
...
/**
 * Single-owner setpoint streams: a vehicle another node is actively
...
async function confirmAckMember(options, member, message) {
  ...
  // memberRecord names the fields it keeps; the waiter's retries/elapsed fall away.
  return memberRecord(member, {
...
function confirmParamMember(options, member, message) {
  ...
  return new Promise((resolve) => {
    ...
    // trust ruling); plain unsigned links carry no mark and pass.
...
function confirmBroadcastAck(options, members, message, target) {
  ...
  return new Promise((resolve) => {
    ...
    // ruling); plain unsigned links carry no mark and pass.
    ...
    // until every member acks or the timeout fires. Members
...
function aggregateFromMembers(
  options, mode, members, startedAt, message = null, total = members.length
) {
...
/**
 * One member's row in a fan-out result: the member's address plus the
 * outcome fields, with the ack fields present on every row (null when no
 * ack answered) so a consumer reads one shape whatever the tier.
 *
 * @param {{sysid: number, compid: number}} member
 * @param {object} fields  result/success/message/detail and, from an ack,
 *   confirmedBy/resultCode/resultParam2
 * @returns {object}
 */
function memberRecord(member, fields) {
  return {
    ...
    detail: fields.detail || null,
    confirmedBy: fields.confirmedBy,
...
/**
 * The position surfaces Fan-out can offset in metres — the same
...
const POSITION_SURFACES = {
  ...
  // be told apart from coordinates here — operator's contract.
 ...
 * belongs to exactly one of two surfaces"), with the addressing forced back
...
function memberMessage(options, member) {
  ...
  // be too).
  delete patch[systemField(options.message)];
...
/**
 * The field a message names its target system by: `target_system`, except
 * on `MANUAL_CONTROL`, the one common message that calls it `target`. Keyed
 * by name, not by field presence: a custom message may carry an unrelated
 * `target` beside its real `target_system`.
 *
 * @param {{name: string}} message
 * @returns {'target'|'target_system'}
 */
function systemField(message) {
  return message.name === 'MANUAL_CONTROL' ? 'target' : 'target_system';
}

/**
 * Address a built message to one member. Stamping `target_system` on
 * MANUAL_CONTROL is dropped by the serializer, and every member's packet
 * keeps the original vehicle, so the system field comes from
 * {@link systemField}. Messages addressing a system but not a component
 * (SET_MODE, MANUAL_CONTROL) keep their shape: a component field the message
 * does not declare is never invented.
 *
 * @param {{name: string, fields: object}} message
 * @param {{sysid:number, compid:number}} member
 * @returns {{name: string, fields: object}}
...
function retarget(message, member) {
  const fields = { ...message.fields, [systemField(message)]: member.sysid };
...
/** MAV_AUTOPILOT wire code → firmware label, for the firmware filter. */
...
function memberFrom(sysid, component) {
  return {
    ...
    type: component.type,
    firmware: FIRMWARE_BY_AUTOPILOT[component.autopilot],
...
function matchesFilter(member, filter) {
  return (filter.type === undefined || Number(filter.type) === Number(member.type))
    && (!filter.firmware || String(filter.firmware) === String(member.firmware))
    && (filter.armed === undefined || booleanFilter(filter.armed) === member.armed);
}
