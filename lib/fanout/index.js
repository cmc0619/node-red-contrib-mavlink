...
function classifyMessage(message, mode) {
  ...
  if (name === 'PARAM_SET') {
    ...
    // No mode matched: the classification falls to the generic arm below,
    // which is harmless — the run itself selects no behavior at the
    // execution-mode dispatch in executeFanout.
  }
...
function executeFanout(options) {
  ...
  // Warnings are mode-selected (§5): a broadcast pre-computes its
  // fleet-uniformity warnings against the resolved group; a sequential run
  // carries none — each member is addressed individually.
  let warnings;
  switch (mode) {
    case 'broadcast':
      warnings = broadcastWarnings(members);
      break;
    case 'sequential':
      warnings = [];
      break;
...
function validateTargets(targets, mode) {
  ...
  // The verdict on a well-formed array is mode-selected (§5): broadcast
  // refuses the list outright, sequential vets each entry.
  switch (mode) {
    case 'broadcast':
  ...
  // No mode matched: no verdict — the run itself selects no behavior at the
  // execution-mode dispatch, so there is nothing here to refuse.
  return null;
}
 ...
 * read it, so a vehicle that is live for one is live for the other.
 *
 * `expired` is listed for completeness — `sweep()` deletes those outright, so
 * a snapshot only ever carries `active` and `stale`.
 *
