...
async function executeFanout(options) {
  ...
  // Default on absence only — `|| 'all'` would launder a supplied-but-falsy
  // mode ('', null) into 'all' before the allowlist sees it (Codex, #231).
  const selectionMode = selection.mode === undefined ? 'all' : selection.mode;
  // The selection can arrive on msg.payload, so its mode is runtime input: an
  // unreadable mode must refuse, because the only alternative is falling
  // through to every active peer — an arm or land widened to the whole fleet
  // by a typo (#231). Absent means 'all' by design; unreadable does not.
  if (!['all', 'list', 'filter'].includes(selectionMode)) {
    return refuse(`unknown fan-out selection "${selectionMode}" — mode must be all, list, or filter`);
  }
