...
const { buildMoveMessage, usesGlobalPosition } = require('../move');
const { buildParamMessage, matchesParamEcho } = require('../param');
const { buildPayloadMessage, payloadVerbNeedsCarrier } = require('../payload');
...
/**
 * Execute one wrapped single-message action across the selected members and
...
async function executeFanout(options) {
  ...
  // Ask the dialect XML how this command's param5/6 ride the INT carrier
  // (§9 "ask the XML"). The compiled bundle arrives as an explicit option —
  // the fanout node resolves it from the Vehicle Profile via getDialect(),
  // because the connection's public vehicle snapshot deliberately carries no
  // bundle (Codex review on #61). No bundle → historical treat-as-latlon
  // scaling.
  if (action.type === 'command' && action.carrier === CARRIER.INT && options.vehicleBundle) {
    const preset = action.preset ? getPreset(action.preset) : null;
    const cid = preset ? Number(preset.commandId) : Number(action.commandId);
    const kinds = intCoordKinds(options.vehicleBundle, cid);
    if (kinds) options = { ...options, action: { ...action, coordKinds: kinds } };
  }

  const members = selectFanoutMembers(options.connection.peerTable, options.selection);
  ...
  // Per-member replacement for the whole-action blank-location check that
  // validateWrap skips when memberParams is present: every member's merged
  // params must carry lat/lon before the first send — a partial run must
  // never scatter half a formation to their slots and refuse the rest.
  if (action.type === 'command' && action.memberParams) {
    const preset = action.preset ? getPreset(action.preset) : null;
    const blank = preset
      ? members.filter((member) => blankLocationRefusal(preset, memberParamsFor(action, member.sysid)))
      : [];
    if (blank.length > 0) {
      return aggregateRecord({
        result: 'refused',
        ...
        members: [],
        warnings,
        detail: `${preset.name} requires latitude and longitude for sysid ${blank.map((m) => m.sysid).join(', ')} ` +
...
async function executeSequential(options, members, warnings, startedAt) {
  ...
  for (let i = 0; i < members.length; i += 1) {
    // Inter-member spacing runs at the top of the loop rather than the bottom,
    // so the single check below covers both "cancelled while sending to the
    // previous member" and "cancelled during the pause" — a redeploy must not
    // have to wait out a pending interval before the loop notices.
    // The pause takes the abort signal, so an aborted wait clears its own
    // timer instead of holding the event loop for the rest of the interval. An
    // injected `wait` (tests) ignores the second argument, which is fine —
    // those never park for real time.
...
/**
 * Canonical params (1–7) for one member: `action.memberParams[sysid]` merged
 * over `action.params`. Members without an entry use `action.params` unchanged.
 * The merge happens here, at the canonical layer, so the carrier's frame-aware
 * scaling and refusals apply to the merged values. Property lookup coerces
 * numeric keys to strings, so JSON payloads with string sysid/param keys and
 * test objects with numeric keys behave identically — the same rule
 * buildParamArray/keepParam already rely on for `action.params`.
 *
 * @param {object} action  wrapped action config
 * @param {number} sysid
 * @returns {Object<number, *>}
 */
function memberParamsFor(action, sysid) {
  const overrides = action.memberParams && action.memberParams[sysid];
  return overrides ? { ...(action.params || {}), ...overrides } : (action.params || {});
}

function validateWrap(action, mode) {
  if (action.type === 'mission') return 'Mission actions are multi-message transactions and cannot be wrapped by Fan-out';
  if (action.type === 'param' && action.action === 'request-list') {
    return 'Param request-list is a bulk transfer and cannot be wrapped by Fan-out';
  }
  if (action.type === 'param' && action.action !== 'set') {
    return 'Fan-out only wraps Param set-one actions';
  }
  ...
  if (!['command', 'move', 'payload', 'param'].includes(action.type)) {
    return `unknown Fan-out action ${JSON.stringify(action.type)}`;
  }
  // memberParams is runtime-boundary input (msg.payload.memberParams). A
  // malformed shape — an array, a scalar, a non-object member entry — must
  // refuse, not quietly fall back to the shared params: `{}` lookups on the
  // wrong shape return undefined, and every vehicle would silently receive
  // the same command the caller meant to differentiate.
  if (action.memberParams !== undefined && action.memberParams !== null) {
    const isPlainObject = (value) => value !== null && typeof value === 'object' && !Array.isArray(value);
    if (!isPlainObject(action.memberParams) || !Object.values(action.memberParams).every(isPlainObject)) {
      return 'memberParams must be an object keyed by sysid with {paramIndex: value} objects';
    }
  }
  if (action.memberParams && action.type !== 'command') {
    return 'memberParams is only defined for Command actions';
  }
  // A broadcast packet carries one param set — uniform commands only (§10).
  if (action.memberParams && mode === 'broadcast') {
    return 'broadcast carries one param set (uniform commands only, §10); memberParams requires sequential fan-out';
  }
  if (action.type === 'command') {
    ...
    // Fan-out builds preset params itself, so the command node's guard never
    // runs here — without this, a fleet-wide Go To with a blank latitude sends
    // every vehicle to 0,0 (§10 "blank coordinates must not become 0,0").
    // With memberParams the lat/lon legitimately live per member, so the
    // per-member gate in executeFanout replaces this whole-action check.
    if (preset && !action.memberParams) {
      const refusal = blankLocationRefusal(preset, action.params || {});
      if (refusal) return refusal;
    }
  }
  // Carrier is a required operator choice for anything that rides a MAV_CMD
  // (§9): no default wire form, same rule as the command node. Message-kind
  // payload verbs (gimbal manager aiming) never consult the carrier, so they
  // are exempt (Codex review on #61).
  const needsCarrier =
    action.type === 'command' ||
    (action.type === 'payload' && payloadVerbNeedsCarrier(action.topic, action.verb, action.path));
  if (needsCarrier) {
    if (action.carrier !== CARRIER.INT && action.carrier !== CARRIER.LONG) {
      return `${action.type} action requires carrier 'int' or 'long' (§9 — no default wire form)`;
    }
  }
  if (action.type === 'move') {
    let globalPosition;
    try {
      globalPosition = usesGlobalPosition(action);
    } catch (err) {
      // Unknown mode/frame refuses the whole action here rather than failing
      // per member mid-fan-out.
      return err.message;
    }
    if (globalPosition) {
...
function buildWrappedMessage(action, target, member, confirmation = 0) {
  ...
  const paramRequest = {
    ...action,
    target,
    firmware: action.firmware || member.firmware,
  };
  return {
...
function confirmationRefused(action, options) {
  ...
  return 'safety command requires msg.confirmed === true (or node confirmation) before it can be fanned out';
}
...
function buildCommand(action, target, confirmation) {
  ...
  if (!Number.isFinite(commandId) || commandId <= 0) {
    throw new Error(`Fan-out command requires a positive numeric commandId, got ${JSON.stringify(action.commandId)}`);
  }
  const userParams = memberParamsFor(action, target.sysid);
  const params = preset
    ? buildParamArray(preset, userParams)
    : [1, 2, 3, 4, 5, 6, 7].map((index) => keepParam(userParams[index]));

  // Wire envelope comes from the shared carrier builders (§9): positional
  // params are canonical degrees, scaled to degE7 by the INT carrier. The
  // confirmation counter only exists on the LONG carrier; buildCommandInt
  // has no confirmation byte to increment.
  const message = action.carrier === CARRIER.INT
    ? buildCommandInt(commandId, target.sysid, target.compid, params, {
        frame: action.frame,
        coordKinds: action.coordKinds,
      })
    : buildCommandLong(commandId, target.sysid, target.compid, params, confirmation);
