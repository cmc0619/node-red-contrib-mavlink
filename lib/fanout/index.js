...
async function executeFanout(options) {
  ...
  // deliveryTier validates delivery once, up front — before any member's send
  // is in flight. Every
...
function deliveryTier(options) {
  switch (options.delivery) {
    ...
    default:
      throw new Error(
        `unknown Fan-out delivery ${JSON.stringify(options.delivery)} — expected one of build, send, confirm`
      );
  }
}
