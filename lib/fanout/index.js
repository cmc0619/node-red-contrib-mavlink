...
async function executeSequential(options, members, warnings, startedAt) {
  ...
  for (; i < members.length; i += 1) {
    // `inFlight.size > 0` is liveness, not validation: with nothing in
    // flight there is nothing to race, and Promise.race([]) never settles.
    // A trusted concurrency of 0 or less therefore degenerates to strict
    // one-at-a-time instead of hanging the run before its first launch.
    while (inFlight.size > 0 && inFlight.size >= concurrency && !(signal && signal.aborted)) {
