...
function selectFanoutMembers(peerTable, selection = {}) {
  ...
  switch (selection.mode) {
    case 'all':
      ...
      break;
    case 'list': {
      // Strict, because this list can arrive on msg.payload: a sysid that is not
      // a uint8 is a bad list, and the only alternative to refusing is dropping
      // it — which sends to the members that did parse and reports success, the
      // partial fan-out parseSysidList exists to prevent. Build tier refuses the
      // same way (buildListStub, nodes/mavlink-fanout.js). Config lists are
      // editor-validated and pass untouched.
      const wanted = new Set(parseSysidList(selection.sysids));
      keep = (member) => wanted.has(member.sysid);
      break;
    }
    case 'filter':
      keep = (member) => matchesFilter(member, filter);
      break;
  }
...
async function executeBroadcast(options, members, warnings, startedAt) {
  ...
  try {
    ...
    // No tier arm matched, so nothing was sent. Reported as a record rather
    // than thrown or dropped: a member the run never reached must never
    // aggregate as a success (§0 — never a validation throw, never silence).
    return aggregateFromMembers(options, 'broadcast', members.map((member) => memberRecord(member, {
      result: 'failed',
      success: false,
      message,
      detail: `no delivery tier matched ${JSON.stringify(options.delivery)}`,
    })), warnings, startedAt, message);
  } catch (err) {
    const records = members.map((member) => memberRecord(member, {
      result: 'failed',
      success: false,
...
async function executeMember(options, member) {
  ...
  try {
    ...
    // No tier arm matched — see executeBroadcast.
    return memberRecord(member, {
      result: 'failed',
      success: false,
      message,
