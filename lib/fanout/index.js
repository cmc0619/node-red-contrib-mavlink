 ...
 * @param {boolean} [options.dryRun]  resolve every per-member message and
 *   report exactly what would be dispatched (result 'dryrun') without
 *   sending; the configured delivery tier still classifies and gates the run
 ...
 *   (Promise from broadcast/sequential; plain object on refuse/empty; undefined
...
function executeFanout(options) {
  ...
  const refuse = (detail) => aggregateRecord(options.nodeType, {
    result: 'refused',
    success: false,
    mode,
    message,
  ...
  if (options.dryRun) {
    return dryRunPreview(run, members, warnings, startedAt);
  }
  switch (mode) {
    case 'broadcast':
...
async function executeSequential(options, members, warnings, startedAt) {
  ...
  // The editor validator requires an integer >= 1; absence defaults to 1
  // (strictly sequential). No clamp — the runtime trusts the saved config.
  const concurrency = Number(options.concurrency === undefined ? 1 : options.concurrency);
  ...
  // in its own .finally, and the race's await yields to those handlers.
  // `inFlight.size > 0` is liveness, not validation: with nothing in flight
  // there is nothing to race, and Promise.race([]) never settles, so a
  // trusted concurrency of 0 or less degenerates to strict one-at-a-time
  // instead of hanging the run before its first launch.
  const atCapacity = () =>
    inFlight.size > 0 && inFlight.size >= concurrency && !signal?.aborted;
  let stopped = false;
  let i = 0;
...
function confirmParamMember(options, member, message) {
  ...
  const vehicle = options.connection.vehicle;
  const encoding = resolveParamEncoding({ firmware: vehicle && vehicle.firmware });
  return new Promise((resolve) => {
    ...
    // closed (Greptile #140).
    if (options.signal) options.signal.addEventListener('abort', abortSettle, { once: true });

    function cleanup() {
      clearTimeout(timeout);
      unsubscribe();
      if (options.signal) options.signal.removeEventListener('abort', abortSettle);
    }
...
function confirmBroadcast(options, members, message, target) {
  ...
  return new Promise((resolve) => {
    ...
    function cleanup() {
      ...
      if (options.signal) options.signal.removeEventListener('abort', settle);
    }
...
/**
 * Dry-run preview: resolve exactly what the run would dispatch — every
 * per-member message after retargeting, per-member field patches, and metre
 * offsets — and report it without sending anything. Placement after every
 * classification/selection refusal above is deliberate: a dry run of a run
 * that would be refused reports the refusal, because the preview's contract
 * is "exactly what would happen".
 *
 * The record's result is 'dryrun' and success/continue stay false: nothing
 * was dispatched, so no consumer can read the preview as a delivered run
 * (§2). Member records wear the build tier's 'built' — resolving the message
 * *is* the operation the preview performs — and a member whose message
 * cannot be resolved (a throwing payload getter in a patch) fails exactly
 * where a live run's executeMember would fail it.
 *
 * @param {object} options the run (`...options, message, kind, overrides`)
 * @param {Array<object>} members resolved members
 * @param {string[]} warnings
 * @param {number} startedAt
 * @returns {object} aggregate status record, result 'dryrun'
 */
function dryRunPreview(options, members, warnings, startedAt) {
  // Broadcast dispatches one packet to everyone; the per-member record shows
  // what each member hears (mirroring the build tier), and the packet itself
  // rides the aggregate's message field.
  const broadcastMessage = options.mode === 'broadcast'
    ? retarget(options.message, { sysid: 0, compid: 1 })
    : null;
  const records = members.map((member) => {
    try {
      return memberRecord(member, {
        result: 'built',
        success: true,
        message: broadcastMessage ? retarget(options.message, member) : memberMessage(options, member),
      });
    } catch (err) {
      return memberRecord(member, {
        result: 'failed',
  ...
  return aggregateRecord(options.nodeType, {
    ...
    warnings,
    broadcastMessage,
    detail: `dry-run: resolved ${records.length} member message(s), nothing sent`,
    elapsed: now(options) - startedAt,
    outcomes: outcomeSummary(records),
  });
}

/**
 * The per-member outcome tally on a terminal record: sysid lists, not just
 * counts, so a consumer sees *which* vehicles accepted, failed, timed out on
 * their confirmation, or were never dispatched (stop-on-error) — partial
 * failure reported, never hidden (§2). 'built' members (build tier, dry-run)
 * dispatched nothing and land in no bucket; every other result the executor
 * produces — 'failed', 'refused', 'send failed', a MAV_RESULT denial name or
 * `result_N`, 'cancelled' — is a dispatch that did not succeed, so it counts
 * as failed. The exact outcome stays on each member record's `result`.
 *
 * @param {Array<object>} members member records
 * @returns {{accepted: number[], failed: number[], timedOut: number[], skipped: number[]}}
 */
...
function outcomeSummary(members) {
  const summary = { accepted: [], failed: [], timedOut: [], skipped: [] };
  for (const member of members) {
    const bucket = Object.prototype.hasOwnProperty.call(OUTCOME_BUCKETS, member.result)
      ? OUTCOME_BUCKETS[member.result]
      : 'failed';
    if (bucket) summary[bucket].push(member.sysid);
  }
  return summary;
}
...
function aggregateFromMembers(options, mode, members, warnings, startedAt, message = null) {
  ...
  return aggregateRecord(options.nodeType, {
    ...
    elapsed: now(options) - startedAt,
    outcomes: outcomeSummary(members),
  });
}
...
function aggregateRecord(nodeType, fields) {
  return delivery.makeStatusRecord(nodeType, {
    ...
    // Null where no run executed (refused/empty selection): the tally only
    // exists once members were actually dispatched (or dry-run-resolved).
    outcomes: fields.outcomes === undefined ? null : fields.outcomes,
  });
}
...
function validateTargets(targets, mode) {
  ...
  for (const entry of targets) {
    ...
    // The override map merges last-wins, so a duplicated sysid would silently
    // shrink the fleet and drop the earlier patch — an operator error reported
    // as success. Refuse it loudly instead (§10 duplicate-targets, #265).
    if (seen.has(n)) {
      return `targets lists sysid ${n} more than once — a duplicate would silently drop the earlier entry's patch; list each vehicle once`;
    }
    seen.add(n);
...
function currentMemberState(peerTable, member) {
  ...
  if (!isActive(component)) return { active: false, state: component.state };
  return { active: true, state: component.state };
}
...
function parseSysidList(value) {
  ...
  return parts.filter((part) => part !== '').map(Number);
}
...
/** Affirmative dispatch (§5): a member returns itself, anything else nothing. */
