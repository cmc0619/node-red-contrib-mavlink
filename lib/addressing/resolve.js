...
/**
 * Build-tier dialect with one explicit legacy compatibility path (AGENTS.md /
 * DESIGN.md §14): flows saved before the Dialect field existed stored only
 * `vehicle`. Map that shape to `__vehicle`. Empty dialect with no vehicle stays
 * empty (editor-invalid for fresh nodes).
 *
 * @param {{dialect?: *, vehicle?: *}|null|undefined} config
 * @returns {string}
 */
function resolveBuildDialect(config) {
  if (!config) return '';
  const dialect = config.dialect;
  if (dialect !== undefined && dialect !== null && dialect !== '') {
    return String(dialect);
  }
  if (config.vehicle) return '__vehicle';
  return '';
}
