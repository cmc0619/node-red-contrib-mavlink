 ...
 *
 * Flow `msg` values are programmer-authored (same trust class as editor
 * config) — resolve and normalize coerce/default; they do not validate.
 * Editor form fields still use {@link parseTargetUint8}.
 */
...
/**
 * Validate a MAVLink uint8 target field for the Vehicle Profile *editor*.
 * Target ids allow 0 (broadcast) — unlike source ids which disallow it.
 *
 * @param {*}      value
 ...
 * Missing → 1; explicit 0 kept. Does not validate — callers are trusted.
 *
...
function normalizeTarget(target) {
  const t = target || {};
  return {
    sysid: Number(firstDefined(t.sysid, 1)),
    compid: Number(firstDefined(t.compid, 1)),
  };
}
