 ...
 * and silently become broadcast (#174). A padded number (`' 4 '`) is still a
...
/**
 * Blank resolves to `fallback`; a present value that does not coerce to a
 * finite number throws naming `label` (owner ruling, 2026-08-14, the
 * timer/rate/port half of the selection-typo audit).
 *
 * This is deliberately the arm `requireNumber` (`lib/move/frames.js`) does
 * NOT carry — a 2026-08-14 ruling (`DESIGN.md` §14, "a finite-number check
 * on operator input is a guardrail") declined a non-finite refusal there
 * because *something already catches it*: on a float, NaN is legal MAVLink
 * and rides; on an integer, `lib/connection/wire.js` refuses non-finite at
 * the one choke point every outbound message crosses. Neither safety net
 * exists here — these values never reach the wire at all. They arm a local
 * `setInterval`/`setTimeout` or open a socket, and Node's timers substitute
 * a ~1 ms delay for a non-finite one instead of refusing, so a garbage
 * heartbeat interval, stream rate, ack timeout, or stale/expire threshold
 * degrades into a flood, a peer that can never go stale, or a comparison
 * that is silently always false — with no error, ever. That is the
 * "resource exhaustion" runtime boundary AGENTS.md already carves out, not
 * a re-validation of a range the editor bounds: only finiteness is checked,
 * never a minimum, maximum, or sign.
 *
 * @param {*} value
 * @param {number} fallback  returned verbatim (not itself finiteness-checked
 *   — callers pass an already-trusted default or editor-validated sibling)
 * @param {string} label     named in the thrown message
 * @returns {number}
 */
function finiteNumberOr(value, fallback, label) {
  ...
  if (!Number.isFinite(n)) {
    // eslint-disable-next-line no-restricted-syntax -- UNJUSTIFIED, pending removal. Fits none of §0's three rules. Introduced by an agent (cf65cc3) citing a §14 ruling that says the reverse — "a finite-number check on operator input is a guardrail" — via a carve-out §14 does not contain. Every field it guards already carries RED.validators.number() in its dialog, making it the duplicate validator §6 forbids.
 ...
 * @returns {{targetSystem: number, targetComponent: number, firmware: string}|null}
 */
