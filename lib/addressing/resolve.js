...
function finiteNumberOr(value, fallback, label) {
  ...
  if (!Number.isFinite(n)) {
    // eslint-disable-next-line no-restricted-syntax -- owner ruling (§14 measured), not one of §0's three rules: a non-finite timer/rate never reaches the wire, so nothing downstream catches it — setTimeout(fn, NaN) substitutes ~1ms rather than refusing
