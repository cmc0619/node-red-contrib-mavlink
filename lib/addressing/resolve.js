 ...
 *
 * Flow `msg` is programmer-authored (same trust class as editor config) —
 * resolve and normalize coerce/default; they do not validate.
 */
...
/**
 * MAVLink uint8 id for config-node constructors (Vehicle defaults, Local
 * Identity source ids). Blank → fallback; else integer in [0, 255].
 * Source-id min 1 stays an editor rule — runtime trusts that path.
 *
 * @param {*}      value
 * @param {string} field
 ...
 */
function parseUint8(value, field, fallback) {
