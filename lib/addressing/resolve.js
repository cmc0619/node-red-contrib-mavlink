...
/**
 * Blank resolves to `fallback`; anything else is `Number(value)`.
 *
 * Only the blank arm survives, and §14 is why: refusing — or here, replacing
 * — a **blank** keeps information the flow already lost, because the editor
 * saves '' for "unset" and `Number('')` is 0, a different instruction. What
 * the caller actually supplied rides untouched. The non-finite arm this
 * carried was the guardrail §14 assigns to the dialog, and every field routed
 * through here already carries `RED.validators.number()` there.
 *
 * @param {*} value
 * @param {number|undefined} fallback  six call sites pass `undefined` so the
 *   consuming library's own default applies
 ...
 * @param {boolean} [opts.derivesSysidFromVehicle] selected identity is a companion
 * @param {number} [opts.sourceSysid] Connection-scoped source sysid
...
function resolveActionTarget(opts) {
  ...
  const companion = opts.derivesSysidFromVehicle === true;

  const sysid = Number(firstDefined(
    payloadTarget.sysid,
    companion ? opts.sourceSysid : undefined,
    companion ? undefined : opts.configSysid,
    companion ? undefined : profile?.targetSystem
  ));
