 ...
 * path. Callers must emit any output-1 status record before invoking this
 * helper and must not also call `node.error` or bare `done()`.
 *
 * @param {Error} err
 * @param {Function} done  the input handler's done callback
 */
function reportDoneError(err, done) {
  done(err);
}
