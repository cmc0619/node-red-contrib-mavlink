...
/**
 * Report an action-node input-handler error through exactly one Node-RED Catch
 * path. Modern Node-RED passes `done`; in that case Catch observes only
 * `done(err)`. Legacy runtimes omit `done`, so this falls back to
 * `node.error(err, msg)`. Callers must emit any output-1 status record before
 * invoking this helper and must not also call `node.error` or bare `done()`.
 *
 * @param {object} node  Node-RED node instance
 * @param {Error} err
 * @param {object} msg
