 ...
 *   {@link failInput}        — terminal failure: badge, status record, done(err)
 ...
 * @param {object} [fields]  extra record fields beside result and detail
 */
function failInput(node, send, err, done, fields) {
  applyActionStatus(node, 'error', err.message);
  send([null, makeStatusRecord(node.type, { result: 'failed', detail: err.message, ...fields })]);
  done(err);
}
