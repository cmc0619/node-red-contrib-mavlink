...
function failInput(node, send, err, done) {
  ...
  send([null, makeStatusRecord({ node: node.type, result: 'failed', detail: err.message })]);
  done(err);
}

/**
 * Terminal failure of an in-flight action, when the node has already put the
 * rich outcome on output 1 and only needs to halt the chain. The thrown Error
 * exists solely to trip a Catch, so its text is a constant: which node failed
 * is on Node-RED's error hover and the node's type, and why is on the output-1
 * record. One message, one place — kept sparse on purpose (owner ruling).
 *
 * @param {Function} done  the input handler's done
 */
function failAction(done) {
  const err = new Error('Action failed');
  // Node-RED's debug renders `err.stack` verbatim (@node-red/util encodeObject),
  // so the driver's own call frames land in the sidebar as a multi-line trace —
  // noise, since the rich outcome is already on output 1 and the node type plus
  // Node-RED's error-hover name say which node. Collapse the stack to its header
  // line so the sidebar shows just `Error: Action failed`. `failInput` keeps its
  // real stack: that path is the loud crash, where the trace is the point.
