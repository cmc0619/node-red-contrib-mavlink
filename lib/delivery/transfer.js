...
class Transfer {
  ...
  /**
   * Resolve the promise once. A base that sends a closing frame (log's
   * LOG_REQUEST_END, FTP's TERMINATESESSION) flips `_settled` itself before
   * that send and then calls `_teardown`, so a reply the send delivers
   * synchronously cannot re-settle the transfer with a different outcome.
   *
