...
class FtpMachine extends Transfer {
  ...
  _messages() { return ['FILE_TRANSFER_PROTOCOL']; }
  ...
  /** Every base-shaped failure — start, send, retry send — is a send failure here. */
  _abort(reason) {
    this._fail('send', reason);
  }
  ...
  _beginList() {
    ...
    this._sendRequest(OPCODE.LISTDIRECTORY, 0, 0, path, path.length, 'list');
  }

  _beginDownload() {
    const path = Buffer.from(this._path);
    this._progress({ phase: 'open', operation: 'download' });
    this._sendRequest(OPCODE.OPENFILERO, 0, 0, path, path.length, 'open');
  }

  _beginUpload() {
    const path = Buffer.from(this._path);
    this._progress({ phase: 'open', operation: 'upload' });
    this._sendRequest(OPCODE.CREATEFILE, 0, 0, path, path.length, 'create');
  }

  _beginCreateDirectory() {
    const path = Buffer.from(this._path);
    this._progress({ phase: 'create-directory', path: this._path });
    this._sendRequest(OPCODE.CREATEDIRECTORY, 0, 0, path, path.length, 'create-directory');
  }
  ...
  async _restoreDirectory(path) {
    ...
    if (outcome.protocol?.errorCode !== NAK_ERROR.FILEEXISTS) return outcome;
  ...
  _responseMatches(response) {
    ...
    // ArduPilot's GCS_FTP::error() leaves offset at 0 (setup_reply memset),
    // including ReadFile EOF. ACK still echoes the request offset; NAK does
    // not, so requiring offset equality drops the EOF that ends a download.
    if (response.seq !== current.replySeq) return false;
    if (response.reqOpcode !== current.opcode) return false;
    if (current.opcode !== OPCODE.OPENFILERO
      && current.opcode !== OPCODE.CREATEFILE
      && response.session !== current.session) {
      return false;
    }
    if (response.opcode === OPCODE.NAK) return true;
    return response.offset === current.offset;
  }
  ...
  _handleNak(response) {
    ...
    if (handled) return;
    // MavFtpErr is a numeric enum, so it reverse-maps its own spelling; the
    // NAK reads as the identifier the MAVLink spec uses, not a paraphrase.
    const name = NAK_ERROR[code] || `error ${code}`;
    const protocol = responseProtocol(response, code);
    if (code === NAK_ERROR.FAILERRNO) protocol.errno = response.data[1];
  ...
  _listAck(response) {
    ...
    const path = Buffer.from(this._path);
    this._sendRequest(OPCODE.LISTDIRECTORY, 0, this._listOffset, path, path.length, 'list');
  }
  ...
  _sendRead() {
    const size = this._offset < this._fileSize
      ...
      : MAX_DATA_BYTES;
    this._sendRequest(OPCODE.READFILE, this._session, this._offset, Buffer.alloc(0), size, 'read');
  }
  ...
  _sendWrite() {
    ...
    this._sendRequest(OPCODE.WRITEFILE, this._session, this._offset, data, data.length, 'write');
  }
  ...
  _cleanupFailed(reason, protocol) {
    ...
    if (cleanup.outcome.result === 'succeeded') {
      this._finalize({
        ...
        phase: 'cleanup',
        reason,
        protocol: protocol || { opcode: OPCODE.TERMINATESESSION, session: cleanup.session },
      });
      return;
    }
  ...
  _finalize(outcome, bestEffortSession = null) {
    ...
    if (bestEffortSession !== null) {
      try {
        ...
        terminal = { ...outcome, cleanupError: `FTP terminate-session send failed: ${err.message}` };
      }
    }
    this._teardown(terminal);
  }
