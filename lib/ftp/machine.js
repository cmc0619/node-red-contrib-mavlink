...
class FtpMachine {
  ...
  _beginRestore() {
    ...
    this._runComposite(async () => {
      ...
      for (const relative of directories) {
        const outcome = await this._restoreDirectory(this._underRoot(relative));
        if (this._settled) return;
        if (outcome.result !== 'succeeded') {
          this._settle({
      ...
      for (const file of files) {
        const outcome = await this._startChild('upload', {
          path: this._underRoot(file.path),
  ...
  /**
   * Place one bundle entry under the restore root. The entry is a path
   * *relative* to that root, so it is rooted before it is joined. Joining it
   * directly lets remotePath normalise a leading `..` upward — `/restore` and
   * `../etc/passwd` compose to `/etc/passwd` — and write outside the directory
   * the operator chose, which is not what a relative path means. Nothing is
   * refused here (§0): every entry still rides, `../etc/passwd` as
   * `<root>/etc/passwd`. A path the backup itself wrote is unaffected.
   *
   * @param {string} relative  one bundle entry's path
   * @returns {string}
   */
  _underRoot(relative) {
    return remotePath.join(this._path, remotePath.join('/', relative));
  }
  ...
  _responseMatches(response) {
    ...
    return response.seq === current.replySeq
      ...
      && (current.opcode === OPCODE.OPENFILERO
