...
class FtpMachine {
  ...
  _handleNak(response) {
    ...
    const protocol = this._responseProtocol(response, code);
  ...
  _listAck(response) {
    ...
    if (page.count === 0) {
      this._fail('protocol', 'FTP directory acknowledgement contained no entries', this._responseProtocol(response));
      return;
    }
  ...
  _openAck(response) {
    if (response.size !== 4 || response.data.length < 4) {
      this._fail('protocol', 'FTP open acknowledgement omitted the file size', this._responseProtocol(response));
      return;
    }
  ...
  _readAck(response) {
    ...
    if (response.size === 0 || response.size > current.size || response.data.length < response.size) {
      this._fail('protocol', 'FTP read acknowledgement carried an invalid data size', this._responseProtocol(response));
      return;
    }
  ...
  _createAck(response) {
    if (response.size !== 0 && (response.size !== 4 || response.data.length < 4)) {
      this._fail('protocol', 'FTP create acknowledgement carried data', this._responseProtocol(response));
      return;
    }
    this._session = response.session;
    this._offset = 0;
  ...
  _writeAck(response) {
    const current = this._current;
    if (response.size !== 0 && (response.size !== 4 || response.data.length < 4)) {
      this._fail('protocol', 'FTP write acknowledgement carried data', this._responseProtocol(response));
      return;
    }
    ...
    if (bytesWritten === 0 || bytesWritten > current.data.length) {
      this._fail('protocol', 'FTP write acknowledgement made no valid progress', this._responseProtocol(response));
      return;
    }
  ...
  _settle(outcome) {
    ...
    if (this._session !== null) {
      ...
      try {
        ...
        const cleanupError = `FTP terminate-session send failed: ${err.message}`;
        if (terminal.result === 'succeeded') {
          terminal.result = 'failed';
          terminal.phase = 'cleanup';
          terminal.reason = cleanupError;
          terminal.protocol = { opcode: OPCODE.TERMINATE_SESSION, session };
        } else {
          terminal.cleanupError = cleanupError;
        }
      }
    }
  ...
  _responseProtocol(response, errorCode) {
    return {
      seq: response.seq,
