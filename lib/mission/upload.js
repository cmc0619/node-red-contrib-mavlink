...
class MissionUpload extends MissionTransfer {
   ...
   *
   * A positive (ACCEPTED) ack only completes the upload once every item has
   * been delivered. A premature ACCEPTED — before the items are answered — is
   * stale (e.g. a leftover ack from a prior transfer) and is ignored rather
   * than reported as a success for an upload that never happened (§9). A
   * non-zero result is the vehicle actively rejecting the plan and is a real
