...
class MissionUpload extends MissionTransfer {
   ...
   *
   * An **ACCEPTED** ack is phase-gated on {@link _expectingAck}: a leftover
   * success from a preceding same-target transfer can arrive while this upload
   * is still answering item requests, and honouring it would report a plan
   * uploaded that never was — the one failure mode the gate exists to stop.
   *
   * An **error** ack is not gated. The protocol lets a vehicle refuse at any
   * point, including immediately after MISSION_COUNT (NO_SPACE for a plan too
   * big to hold), and that refusal is the whole answer — retrying items into a
   * vehicle that has said no only ends in a "stalled at count" abort that hides
   * the real reason. A failed upload fails, with the vehicle's own code (§9).
   *
   * @param {number} type  a `MAV_MISSION_RESULT`
   */
  _onAck(type) {
    if (type === MAV_MISSION_RESULT.ACCEPTED) {
      if (!this._expectingAck()) return;
    ...
    this._settle({
      ...
      phase: 'ack',
      resultCode: type,
      reason: `vehicle rejected upload: ${missionResultName(type)}`,
    });
  }
}
