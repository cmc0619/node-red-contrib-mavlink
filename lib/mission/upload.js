...
class MissionUpload extends MissionTransfer {
   ...
   *
   * Both ACCEPTED and non-zero results are phase-gated on {@link _expectingAck}:
   * a leftover ack from a preceding same-target transfer can arrive while this
   * upload is still answering item requests. Treating a premature DENIED as
   * this transfer's failure would abort a plan the vehicle has not rejected
   * (§9). Once every item has been delivered (or count is zero), any ack —
   * success or rejection — is the vehicle's answer for *this* upload.
   *
