...
class MissionTransfer {
  ...
  start() {
    return new Promise((resolve) => {
      ...
      const handler = (decoded) => {
        ...
        // A reply explicitly addressed to a different GCS on a shared link
        // must not settle or step this transfer — the same rule ack.js's
        // AckWaiter already applies to COMMAND_ACK (mavlink-audit-20260905
        // #3). Every MISSION_* message carries target_system/target_component
        // as core (non-extension) fields, so they always decode to a number:
        // 0 (unaddressed) and our own ids pass, another station's are
        // rejected — ackAddressedTo's own contract.
        if (this._sourceIds && !ackAddressedTo(decoded.fields, this._sourceIds)) return;
      ...
      // Source sysid/compid narrow the filter only when the target names a
      // real one. 0 is a destination address — MAV_MISSION_TYPE broadcast
      // (e.g. Clear, which the editor allows at target sysid/compid 0) —
      // never a reply's source, so filtering source-equals-0 could never
      // match any real vehicle's answer and every broadcast op would time
      // out despite a positive ack (mavlink-audit-20260905 #8). Omitting the
      // dimension accepts a reply from any source on that axis instead;
      // `ackAddressedTo` above still rejects one addressed to another GCS.
      // trustedOnly: an explicitly untrusted frame must never step a mission
      // transfer (§7 trust ruling); plain unsigned links carry no mark.
  ...
  _onTimeout() {
    ...
    // Same timer-context hazard as the retry send above: download's override
    // sends the INT→legacy fallback here (MissionDownload#_onStepExhausted),
    // and a saturated Bulk queue or dead link throws synchronously — an
    // escape here is an uncaught exception, not a failed transfer
    // (mavlink-audit-20260905 #2). The base implementation only calls
    // `_abort`, which never throws, so this is a no-op for every other
    // subclass.
    try {
