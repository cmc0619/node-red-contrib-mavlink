 ...
 *   - `_acceptMessage(decoded)` — optional source-attribution gate override;
 *     the addressless `MISSION_CURRENT` status has no target fields to check.
 *
  ...
  start() {
    return new Promise((resolve) => {
      ...
      const handler = (decoded) => {
        ...
        // AckWaiter already applies to COMMAND_ACK. MISSION_CURRENT is the
        // one addressless mission status reply, so its machine may opt out
        // while this strict gate remains for every transfer ack.
        if (!this._acceptMessage(decoded)) return;
