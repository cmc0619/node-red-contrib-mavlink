 ...
 *
 * Four operations (download / upload / clear / set-current) use the MAVLink
 * mission service. The first three run over one item-transfer protocol, each
 * parameterised by a `mission_type`; set-current selects the current mission
 * sequence and receives a `MISSION_CURRENT` broadcast. This module holds the
...
/**
 * `MAV_MISSION_TYPE` — the plan an item transfer operates on. Carried on the
 * item-transfer messages as the `mission_type` extension field (default 0 =
 * MISSION when a peer omits it); `MISSION_SET_CURRENT` has no mission type.
 *
 * @enum {number}
 */
const MISSION_TYPE = {
