 ...
 *
 * Three item-transfer state machines plus the Set Current status exchange —
 * the only place in the package where the vehicle drives the conversation. All
 * logic lives here; the
 ...
 *
 * The vocabulary (`./types`) and the item/control message builders
 * (`./items`) are required from their own files.
 */
...
const { MissionSetCurrent } = require('./set-current');
...
function createMachine(operation, opts) {
  switch (operation) {
    ...
    case types.OPERATION.SET_CURRENT:
      return new MissionSetCurrent(opts);
