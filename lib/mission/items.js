...
// Coordinate frames have one owner (§9 "Coordinate frames"):
// lib/command/carrier.js holds the degE7 constant, the unconditional-scale
// rationale, and the global-frame classification; the mission builders reuse
// all three.
//
// Reusing that classification makes the mission builder knowingly asymmetric
// with the spec: `common.xml` defines `MISSION_ITEM_INT` x/y as "local: metres
// × 1e4, global: degrees × 1e7", so a non-global frame should scale ×1e4 the
// way lib/command/carrier.js does for COMMAND_INT — here it passes through
// unscaled instead. The asymmetry is unreachable rather than harmless.
// ArduPilot answers `MAV_MISSION_UNSUPPORTED_FRAME` for non-global mission
// frames (`AP_Mission.cpp`), and PX4's mission manager accepts only
// GLOBAL(_RELATIVE_ALT)(_INT) and MAV_FRAME_MISSION (`mavlink_mission.cpp`) —
// so a local-frame mission item is refused by the vehicle before the scaling
// could ever be observed. It fails loud, not wrong. Add the ×1e4 rule here the
// day a firmware accepts one, with a SITL run behind it.
const { scaleLatLon, isGlobalFrame } = require('../command/carrier');
 ...
 * frame `x`/`y` are metres and pass through unscaled — the frame note on the
 * carrier import says why that departs from the spec and why it does not
 * matter.
 */
...
/**
 * `MISSION_SET_CURRENT` — select the current mission item (§9 "Set current").
 * The message addresses a target, but its acknowledgement is the target's
 * addressless `MISSION_CURRENT` broadcast, so no `mission_type` is present.
 *
 * @param {{sysid: number, compid: number}} target
 * @param {number} seq
 * @returns {{name: string, fields: object}}
 */
function buildSetCurrent(target, seq) {
  return {
    name: 'MISSION_SET_CURRENT',
    fields: { ...targetFields(target), seq },
  };
}
...
function buildItemInt(item, target, seq, missionType) {
  ...
  const scaleXY = (raw) => {
    ...
    return global ? scaleLatLon(raw) : raw;
  };
  return {
...
/**
 * Coerce an optional numeric field to a number. Blank rides as undefined and
 * null as null — incomplete items must not invent 0 in the builder (§0).
 *
 * @param {*} value
 * @returns {*|number}
 */
