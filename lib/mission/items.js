...
// day a firmware accepts one, with a SITL run behind it (#99).
...
function buildItemInt(item, target, seq, missionType) {
  // Blank/null frame is not GLOBAL (0): Number('')/Number(null) would make
  // isGlobalFrame invent global scaling while frame itself stays unset (§0).
  const global = !isBlank(item.frame) && isGlobalFrame(item.frame);
  const scaleXY = (raw) => {
...
/**
 * Coerce an optional numeric field to a number. Absent/null/blank pass through
 * unset — incomplete items must not invent 0 in the builder (§0).
 *
 * @param {*} value
 * @returns {*|number}
 */
