 ...
 * ignored (the step timeout then retries the request). Items are requested
 * with `MISSION_REQUEST_INT` only and arrive as `MISSION_ITEM_INT`; there is
 * no legacy fallback (§14.148). An error `MISSION_ACK` from the vehicle ends
 * the download at any phase, carrying its result code — the same rule as
 * upload (§14).
 */
...
const { DEG_E7, isGlobalFrame } = require('../command/carrier');
...
class MissionDownload extends MissionTransfer {
  ...
  _messages() {
    return ['MISSION_COUNT', 'MISSION_ITEM_INT', 'MISSION_ACK'];
  }
  ...
  /** @param {object} f  MISSION_ITEM_INT fields */
  _onItem(f) {
    ...
    this._items[seq] = canonicalItem(f);
    this._onProgress({
      phase: 'item',
      seq,
      count: this._count,
 ...
 * double-scale (`buildItemInt` re-multiplies by 1e7). Non-global frames carry
 * metres and pass through unscaled (§9 "Coordinate frames").
 *
 * @param {object} f  received MISSION_ITEM_INT fields
 * @returns {object} canonical item
 */
function canonicalItem(f) {
  const item = { ...f };
  if (isGlobalFrame(f.frame)) {
