...
/**
 * `MAV_CMD_NAV_FENCE_*` command ids. A fence item must be one of these and
 * nothing else. This set is the fence-family reservation, so it must track
 * *every* dialect that defines a fence command — `common.xml` (5000–5004) and
 * `development.xml` (5005, WIP), not common alone. A fence id missing here would
 * leak into a mission upload and be rejected as a fence item (issue found in
 * #113 review). Measured against the XML, not a reserved id range — a new
 * fence id is added here when the dialect actually defines one (§14).
 * @type {Set<number>}
 */
const FENCE_COMMANDS = new Set([
  5000, // NAV_FENCE_RETURN_POINT            (common.xml)
  5001, // NAV_FENCE_POLYGON_VERTEX_INCLUSION (common.xml)
  5002, // NAV_FENCE_POLYGON_VERTEX_EXCLUSION (common.xml)
  5003, // NAV_FENCE_CIRCLE_INCLUSION         (common.xml)
  5004, // NAV_FENCE_CIRCLE_EXCLUSION         (common.xml)
  5005, // NAV_FENCE_HOME_CIRCLE_INCLUSION    (development.xml, WIP)
]);
 ...
 * @returns {ValidationResult|undefined} undefined for a type no family
 *   answers to (§5) — the caller craters on the missing `.ok`
 */
function validateItems(items, missionType) {
  switch (missionType) {
    case MISSION_TYPE.MISSION: return validateMissionItems(items);
    case MISSION_TYPE.FENCE: return validateFenceItems(items);
    case MISSION_TYPE.RALLY: return validateRallyItems(items);
