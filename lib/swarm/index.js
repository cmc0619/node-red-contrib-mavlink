...
async function executeSwarm(options) {
  ...
  // (§9 "ask the XML"): the connection's vehicle bundle knows whether they
  // are scalable lat/lon, natively degE7, or raw non-location values. No
  // bundle (build+list stub) → historical treat-as-latlon scaling.
  if (action.type === 'command' && action.carrier === CARRIER.INT) {
    const bundle =
      options.connection && options.connection.vehicle && options.connection.vehicle.bundle;
    const preset = action.preset ? getPreset(action.preset) : null;
    const cid = preset ? Number(preset.commandId) : Number(action.commandId);
    const kinds = bundle ? intCoordKinds(bundle, cid) : null;
...
function validateWrap(action, mode) {
  ...
  // (§9): no default wire form, same rule as the command node. Payload actions
  // are included because most payload verbs are command-backed.
  if (action.type === 'command' || action.type === 'payload') {
