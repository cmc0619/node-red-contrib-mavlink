...
async function executeSwarm(options) {
  ...
  // the swarm node resolves it from the Vehicle Profile via getDialect(),
...
function validateWrap(action, mode) {
  ...
  if (action.type === 'command') {
    ...
    try {
      if (preset && preset.requireLocation) {
        ...
        !preset &&
        action.coordKinds &&
        action.coordKinds[5] === 'latlon' &&
        action.coordKinds[6] === 'latlon'
      ) {
        requireLocationParamValues(params);
      }
    } catch (err) {
