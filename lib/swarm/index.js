...
async function executeSwarm(options) {
  ...
  // Ask the dialect XML how this command's param5/6 are classified (§9 "ask
  // the XML") *before* validateWrap — advanced hasLocation commands must
  // refuse blank lat/lon/alt on both carriers, not only after INT scaling is
  // attached (issue #88 / CodeRabbit on #116). The bundle arrives as an
  // explicit option from the Vehicle Profile via getDialect(); the
  // connection snapshot deliberately carries none (Codex on #61).
  if (action.type === 'command' && options.vehicleBundle) {
    ...
    if (kinds) {
      action = { ...action, coordKinds: kinds };
