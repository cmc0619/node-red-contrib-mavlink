 ...
 * @param {(cancel: () => void) => void} [options.registerCancel] Sequential
 *   fan-out only: called synchronously with a `cancel()` hook before the
 *   member loop starts, so the caller (the node's close handler) can stop it
 *   mid-run and cancel the active member's AckWaiter.
...
async function executeSwarm(options) {
  ...
  // Sequential fan-out awaits an AckWaiter (up to timeoutMs) then an inter-
  // member pause (intervalMs) per member — a redeploy mid-run would otherwise
  // keep sending live vehicle commands for members × (timeoutMs + intervalMs)
  // with nothing to stop it. `registerCancel` hands the caller (the node's
  // close handler) a hook that stops the loop and cancels the in-flight
  // AckWaiter, mirroring mavlink-command's `_activeWaiter` cancellation.
  const cancelState = { cancelled: false, waiter: null, abort: null };
  if (typeof options.registerCancel === 'function') {
    options.registerCancel(() => {
      cancelState.cancelled = true;
      if (cancelState.waiter) cancelState.waiter.cancel();
      // Param-echo confirms have no AckWaiter — they register a settle-based
      // abort instead, so a close mid-echo-wait tears down their subscription
      // and timer immediately rather than running out the full timeout.
      if (cancelState.abort) cancelState.abort();
    });
  }
  return executeSequential(options, members, warnings, startedAt, cancelState);
}

async function executeSequential(options, members, warnings, startedAt, cancelState) {
  const records = [];
  ...
  for (let i = 0; i < members.length; i += 1) {
    if (cancelState && cancelState.cancelled) {
      return aggregateRecord({
        ...
        members: records,
        warnings,
        detail: 'cancelled before the fan-out finished',
        elapsed: now(options) - startedAt,
      });
    }
    ...
    if (!live.active) {
      ...
      records.push(await executeMember(options, member, cancelState));
    }
    if (cancelState && cancelState.cancelled) {
      return aggregateRecord({
        result: 'cancelled',
...
async function executeMember(options, member, cancelState) {
  ...
  try {
    if (tier === 'confirm' || tier === 'sendConfirm') {
      return await confirmMember(options, member, built, cancelState);
    }
...
async function confirmMember(options, member, built, cancelState) {
  if (built.confirmation === 'param_echo') {
    return confirmParamMember(options, member, built, cancelState);
  }
  ...
  // Register with the cancel token so a close mid-wait stops this member's
  // AckWaiter instead of letting it run out the full timeout.
  if (cancelState) cancelState.waiter = waiter;
  const outcome = await waiter.start();
  if (cancelState && cancelState.waiter === waiter) cancelState.waiter = null;
...
function confirmParamMember(options, member, built, cancelState) {
  return new Promise((resolve) => {
    ...
    function settle(fields) {
      ...
      unsubscribe();
      if (cancelState && cancelState.abort === abort) cancelState.abort = null;
      resolve(memberRecord(member, fields));
    }

    // Mirror of the AckWaiter registration in confirmMember: a close mid-wait
    // settles immediately as 'cancelled' instead of holding the subscription
    // and timer for the full echo timeout.
    function abort() {
      settle({
        result: 'cancelled',
        success: false,
    ...
    if (cancelState) cancelState.abort = abort;
