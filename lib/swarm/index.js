...
/**
 * Create a cancellation handle for one fan-out. Node-RED calls `close` on every
 * redeploy, and a sequential fan-out runs for up to members × (timeoutMs +
 * intervalMs): without a way to stop it the orphaned loop keeps commanding live
 * vehicles and settles on a node that no longer exists (§7 "Teardown"). The node
 * passes the handle as `options.cancel` and cancels it from its close handler.
 *
 * @returns {{cancelled: boolean, waiter: object|null, cancel: Function}}
 */
function createSwarmCancel() {
  return {
    cancelled: false,
    /** AckWaiter of the member currently awaiting its COMMAND_ACK, if any. */
    waiter: null,
    cancel() {
      this.cancelled = true;
      if (this.waiter) {
 ...
 * @returns {boolean} whether this run has been cancelled
 */
...
async function executeSequential(options, members, warnings, startedAt) {
  ...
  for (let i = 0; i < members.length; i += 1) {
    // A redeploy cancelled the run: stop before the next member's send instead
    // of commanding vehicles on behalf of a node that is gone (§7 Teardown).
    if (isCancelled(options)) break;
    // Inter-member spacing is taken at the top of every later member so the
    // check above also skips the wait once cancelled.
  ...
  if (isCancelled(options)) {
    return aggregateRecord({
      ...
      members: records,
      warnings,
      detail: 'node closed during fan-out',
      elapsed: now(options) - startedAt,
    });
  }
...
async function confirmMember(options, member, built) {
  ...
  // Publish the in-flight waiter so a close cancels it rather than holding the
  // deploy for the full ack timeout (mirrors mavlink-command's active waiter).
  if (options.cancel) options.cancel.waiter = waiter;
  const outcome = await waiter.start();
  if (options.cancel && options.cancel.waiter === waiter) options.cancel.waiter = null;
