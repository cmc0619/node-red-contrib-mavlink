 ...
 * record carrying `lib/delivery`'s marker so all action nodes in the palette
 * detect each other's records correctly (§9 "A status record is refused").
 *
 * This module wraps lib/delivery's makeStatusRecord / isStatusRecord and adds
 * the command-specific MAV_RESULT tables.
 */
...
/**
 * Construct a command status record using lib/delivery's marker so it is
 * detectable by every other action node in the palette (§9 "one implementation
 * per concept").
 *
 * Fields default: resultCode=null, confirmedBy='none', target=null,
 * elapsed=0, retries=0, command=null, commandId=null, detail=null.
 *
 * @param {object} fields
 * @param {string}  fields.result  'accepted'|'failed'|'unconfirmed'|'timeout'|
 *   'miswire'|'cancelled'|...; matches RESULT_NAME values for ack results
...
/**
 * True if `msg` is a status record (carries lib/delivery's marker).
 *
 * @param {*} msg
 * @returns {boolean}
 */
const isStatusRecord = delivery.isStatusRecord;

/** The marker key from lib/delivery (exported for test assertions). */
const MARKER = delivery.STATUS_RECORD_MARKER;
