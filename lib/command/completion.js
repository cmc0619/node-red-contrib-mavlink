 ...
 * (for altitude) by subscribing to GLOBAL_POSITION_INT. The peer table's live
 * component record (`getComponent`) holds position, armed state, flight mode,
 * and system status independently timestamped (§8).
 *
 * Conditions per §9:
 ...
 *   LAND  (21):  MAV_LANDED_STATE ON_GROUND where EXTENDED_SYS_STATE is
 *               reported; relativeAlt ≤ LAND_ALT_THRESHOLD otherwise
 *   RTL   (20):  same as LAND
 *   SET_MODE(176): flightMode (custom_mode) matches requested value — on PX4
 *               the requested main/sub pair, packed as HEARTBEAT reports it
 *               (§14.110: param2=3 accepted, HEARTBEAT then 196608)
 *
...
const { MAV_FRAME } = require('./carrier');
const { px4CustomMode } = require('../vehicle/modes');
 ...
 * not .relative_alt — the two differ by the home elevation. Every
 * other frame (and COMMAND_LONG, which carries no frame) is relative-to-home.
 */
const ABSOLUTE_ALT_FRAMES = new Set([MAV_FRAME.GLOBAL, MAV_FRAME.GLOBAL_INT]);
 ...
 * @param {string} [firmware]  the Vehicle Profile firmware the request was
 *   built for. `px4` split the mode into main (param2) and sub (param3), so
 *   SET_MODE packs them back before comparing with the HEARTBEAT word.
 ...
 */
function checkCompletion(completionKey, params, peerTable, sysid, compid, frame, firmware) {
  ...
  const comp = peerTable.getComponent(sysid, compid);
  if (!comp) {
  ...
  switch (completionKey) {
    ...
    case COMPLETION.TAKEOFF: {
      ...
      // A blank altitude rides as it is: the climb target is NaN, no report
      // satisfies it, and the wait times out.
      ...
      // meaningful (90% of an AMSL number is nonsense).
    ...
    case COMPLETION.LAND: {
      // MAV_LANDED_STATE (EXTENDED_SYS_STATE) is the vehicle's own discrete
      // landed detector — authoritative wherever it is reported, unlike a
      // relative-altitude threshold, which reads "landed" for an armed,
      // airborne vehicle a few metres above home and reads "still flying" for
      // a disarmed vehicle sitting on a pad well above home elevation
      // (mavlink-audit-20260905 #4). UNDEFINED (0), or the field never
      // arriving, means the firmware does not report it — fall back to
      // altitude only then, exactly as before this vehicle ever answered.
      const landedState = comp.landed ? comp.landed.landedState : undefined;
      if (landedState !== undefined && landedState !== 0 /* UNDEFINED */) {
        if (landedState === 1 /* ON_GROUND */) {
          return { done: true, detail: 'landed (EXTENDED_SYS_STATE)' };
        }
        return { done: false, detail: `landedState=${landedState}` };
      }
      const pos = comp.position;
      if (!pos || pos.relativeAlt === null || pos.relativeAlt === undefined) {
        // Fallback: disarmed and MAV_STATE_STANDBY (3), the status ArduPilot
        // and PX4 both report once landed and disarmed.
        if (comp.armed === false && comp.systemStatus === 3) {
          return { done: true, detail: 'landed (armed=false, status)' };
        }
        return { done: false, detail: 'no position data' };
      }
      const altM = pos.relativeAlt / 1000;
      if (altM <= LAND_ALT_THRESHOLD_M) {
        // Altitude alone cannot tell landed from airborne-below-home: terrain
        // lower than the takeoff point puts a flying vehicle under the
        // threshold. With no landed-state telemetry to break the tie, `armed`
        // is the only other signal, and it is imperfect in one direction: a
        // vehicle that has touched down but not yet (or not at all) disarmed
        // waits out the deadline instead of completing. That false
        // negative is chosen over the false positive — reporting LAND complete
        // to an airborne vehicle — and matches the no-position fallback above,
        // which already requires disarmed. Unknown armed state (no HEARTBEAT
        // decoded yet) does not block, so that verdict is unchanged.
        if (comp.armed === true) return { done: false, detail: `armed at alt ${altM.toFixed(1)} m` };
        return { done: true, detail: `alt ${altM.toFixed(2)} m` };
      }
    ...
    case COMPLETION.SET_MODE: {
      ...
      let requestedCustomMode;
      switch (firmware) {
        case 'px4': requestedCustomMode = px4CustomMode(params[1], params[2]); break;
        case 'ardupilot': requestedCustomMode = params[1]; break;
 ...
 * to `timeoutMs` and then emit onto the closed node. `cancel`
 ...
 * @param {string}   [opts.firmware]  Vehicle Profile firmware, for the PX4
 *   SET_MODE packing
 * @param {number}   opts.timeoutMs  the caller's completion window; the
 *   command node reads it from its own config
 * @param {number}   [opts.pollMs]
 * @param {() => number} [opts.now]
 * @returns {{promise: Promise<{success: boolean, confirmedBy?: 'state'|'ack', detail: string, elapsed: number, cancelled?: true}>, cancel: () => void}}
...
function waitForCompletion(opts) {
  ...
  /** @param {{detail: string}} check  a completion check that came back done */
  function settleState(check) {
    settle({ success: true, confirmedBy: 'state', detail: check.detail, elapsed: now() - startMs });
  }

  function poll() {
    const check = checkCompletion(completionKey, params, peerTable, sysid, compid, frame, firmware);
    ...
    if (check.done) settleState(check);
  }
