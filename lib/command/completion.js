...
/**
 * MAV_AUTOPILOT_PX4. PX4 takes DO_SET_MODE as main_mode in param2 and
 * sub_mode in param3, but its HEARTBEAT reports the pair packed into one
 * `custom_mode` (measured, §14.110: param2=3 accepted, HEARTBEAT 196608), so
 * the request is packed before it is compared. Which encoding the report
 * uses is the vehicle's own, read from its HEARTBEAT `autopilot`. An
 * unsupplied param3 went out as the wire's zero-fill, so sub_mode 0 is what
 * the vehicle was asked for.
 */
...
function checkCompletion(completionKey, params, peerTable, sysid, compid, frame) {
  ...
  switch (completionKey) {
    ...
    case COMPLETION.SET_MODE: {
      ...
      const requestedCustomMode = comp.autopilot === MAV_AUTOPILOT_PX4
        ? px4CustomMode(params[1], params[2] === undefined ? 0 : params[2])
        : params[1];
