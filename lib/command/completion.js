...
function checkCompletion(completionKey, params, peerTable, sysid, compid, frame) {
  ...
  switch (completionKey) {
    ...
    case COMPLETION.SET_MODE: {
      ...
      const provided = requestedCustomMode != null && Number.isFinite(requestedCustomMode);
      if (provided) {
      ...
      // No custom mode value supplied at all — a base-only set leaves nothing
      // observable, so state is no evidence in EITHER direction. Not done:
      // the ack-timeout path must stay unconfirmed (reporting "accepted"
      // because a peer merely exists is a false success, §0 rule 3), and the
      // post-ack complete tier settles this case from the accepted ack via
      // `unverifiable` (waitForCompletion).
