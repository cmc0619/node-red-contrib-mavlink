...
function checkCompletion(completionKey, params, peerTable, sysid, compid, frame) {
  ...
  switch (completionKey) {
    ...
    case COMPLETION.SET_MODE: {
      ...
      // No custom mode value supplied at all — the peer table cannot verify a
      // base-only mode change, so treat the base mode set as satisfied.
 ...
 * @returns {{promise: Promise<{success: boolean, confirmedBy: 'state'|'none', detail: string, elapsed: number, cancelled?: true}>, cancel: () => void}}
 */
