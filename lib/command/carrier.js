...
function longToIntFields(paramArray, opts = {}) {
  ...
  /**
   * One x/y int32 from a canonical param. A non-finite value stays non-finite
   * — int32 cannot express NaN, so it craters at the wire's own choke point
   * (lib/connection/wire.js) rather than coercing to 0 and silently aiming a
   * global-frame command at null island (§9/§10).
   */
  const intXY = (value, kind) => {
    ...
    // NaN, not undefined: absent stays legal at the wire choke, so undefined
    // would serialize as 0 — null island. NaN on an int32 field throws there
    // naming the field (§5; eslint.config.mjs).
...
function intFieldsToLong(fields, opts = {}) {
  const frame = fields.frame === undefined ? DEFAULT_FRAME : fields.frame;
  const global = isGlobalFrame(frame);
  const kinds = opts.coordKinds || { 5: 'latlon', 6: 'latlon' };
