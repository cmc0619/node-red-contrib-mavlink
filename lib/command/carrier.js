 ...
 * is neither, and PX4 decodes it with the degE7 divisor, so folding it into
 * the local bucket would corrupt it. Measured members: LOCAL_NED (1),
 * BODY_NED (8), LOCAL_FRD (20) — all ÷1e4 on PX4. The rest are the remaining
 * local/body position frames from the same MAV_FRAME family and share the
 * field's "local: metres" semantics.
 * @type {Set<number>}
 */
...
function unscaleLocalMetres(value) {
  return (Number(value) || 0) / LOCAL_E4;
}
