 ...
 * invent this when `opts.frame` is omitted (§0); blank/omitted stays unset and
 * the wire poison-init + finite-integer choke refuses a missing frame.
 *
...
function longToIntFields(paramArray, opts = {}) {
  // Spoken → Number(frame). Blank/omitted → leave unset (wire poison refuses).
  // Bare Number(opts.frame) would turn null/'' into 0 (invented GLOBAL).
  let frame;
  ...
  const intXY = (value, kind) => {
    // Absent stays unset — wire poison-init refuses a missing int32 (§14).
