 ...
 *
 * Named explicitly rather than derived as "not global": the dialect describes
 * `MAV_FRAME_MISSION` (2) as "NOT a coordinate frame", and PX4 decodes it with
 * the degE7 divisor, so folding it into the local bucket would corrupt it.
 * Measured members: LOCAL_NED (1), BODY_NED (8), LOCAL_FRD (20) — all ÷1e4 on
 * PX4. The rest are the remaining local/body position frames of the same
 * MAV_FRAME family and share the field's "local: metres" semantics.
 *
 ...
 * slot. Scaling a reserved value would invent semantics no autopilot is known
 * to implement, so it falls to the unclassified path and passes through
 * unscaled like any frame we do not classify.
 * @type {Set<number>}
 */
const LOCAL_FRAMES = new Set([
  1, // LOCAL_NED       (measured)
  4, // LOCAL_ENU
  7, // LOCAL_OFFSET_NED
  8, // BODY_NED        (measured)
  9, // BODY_OFFSET_NED
  12, // BODY_FRD
  20, // LOCAL_FRD      (measured)
  21, // LOCAL_FLU
]);
...
function intCoordKinds(bundle, commandId) {
  ...
  const cmd = Object.values(bundle.commands).find(
    (c) => c && Number(c.value) === Number(commandId)
  );
  if (!cmd) return null;
  const kind = (index) => {
 ...
 * (GLOBAL) applies. One implementation for the command, payload, and swarm
 * nodes (§9).
 *
