 ...
 *
 * `MAV_FRAME_GLOBAL_RELATIVE_ALT` (3), because that is the only frame
 * ArduPilot Copter accepts for a COMMAND_INT takeoff — the check is a strict
 ...
 *
 * The same file answers the "what did they mean?" question directly: when
 * ArduPilot upconverts a COMMAND_LONG takeoff to its internal int form, it
 * fills in `MAV_FRAME_GLOBAL_RELATIVE_ALT` itself (`mav_frame_for_command_long`).
 * A default is exactly that question, so we give the same answer. PX4 accepts
 * either frame, so nothing is lost there.
 *
 * Note `GLOBAL_RELATIVE_ALT_INT` (6) is *not* interchangeable here despite
 * meaning the same altitude reference — the check above is on the value, and 6
 * fails it. The `_INT` variants belong to a message's lat/lon encoding, not to
 * what the operator meant by "altitude".
 *
 * This is not a safety net, and the comment that used to sit here claiming one
 * was wrong: a rejected frame comes back `MAV_RESULT_DENIED` (4), not
 * `COMMAND_UNSUPPORTED_MAV_FRAME`, and only ack codes 7 and 8 arm the carrier
 * swap — so a wrong frame is a flat refusal with no retry behind it. Getting
 * the default right is the whole mechanism.
 *
 * The node lets an operator override this via `msg.mavFrame` or config.
 * @type {number}
 */
...
/**
 * Resolve a node's configured carrier choice, or null when it is missing or
 * invalid (§9). The editor supplies the default; runtime does not repair
 * malformed or hand-edited flow data — it refuses, because the two carriers
 * are different wire messages and guessing sends the wrong one.
 *
 * Shared rather than per-node: Command, Payload and Formation all read the
 * same `sendAs` key, and Formation reading it as a bare `=== CARRIER.INT`
 * meant a missing choice silently became `COMMAND_LONG` where the other two
 * refused (§2 — one spelling of one decision).
 *
