...
/**
 * Read the editor's **Send as** choice, or null when it is missing or
 * unreadable (§9). The editor supplies the default; runtime does not repair
 * malformed or hand-edited flow data — it reports the absence and lets the
 * caller refuse in its own words. Shared because every node with the row
 * (Command, Formation) must agree on what counts as a choice.
 *
 * @param {object} config  node config from the editor
 * @returns {'long'|'int'|null}
 */
function resolveSendAs(config) {
