 ...
 * invent this when `opts.frame` is omitted (§0); blank/omitted becomes NaN
 * and chokes at the wire integer check.
 *
 ...
 * @param {object} [opts]
 * @param {number} [opts.frame]  MAV_FRAME; blank/omitted is NaN (not invented)
...
function intFieldsToLong(fields, opts = {}) {
  const frame =
    fields.frame !== undefined && fields.frame !== null && fields.frame !== ''
      ? fields.frame
      : NaN;
  const global = isGlobalFrame(frame);
  const kinds = opts.coordKinds || { 5: 'latlon', 6: 'latlon' };
