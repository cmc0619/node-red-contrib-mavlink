 ...
 * heuristic pass-through on magnitude could silently misread a
 ...
 * @param {object} [opts]
 * @param {number} [opts.frame]  MAV_FRAME, as resolveFrame returns it; omitted is left unset
...
function longToIntFields(paramArray, opts = {}) {
  ...
  const intXY = (value, kind) => {
    ...
    // do not classify — passes through unscaled. A natively-degE7 param and
    // a non-location param5/6 (gimbal flags, etc.) carry what the operator
    // entered in every frame.
    switch (kind) {
      case 'latlon':
  ...
  return {
    frame,
    // Always 0 for a direct command: the spec reserves both for mission
    // items, and no caller has ever set either (§3).
    current: 0,
    autocontinue: 0,
...
function buildCommandInt(commandId, targetSystem, targetComponent, paramArray, opts = {}) {
  return {
    ...
    fields: {
      ...
      target_component: targetComponent,
      command: commandId,
      ...longToIntFields(paramArray, opts),
    },
  };
}
