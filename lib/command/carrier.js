 ...
 *     global frame (degrees × 1e7, §9 "Coordinate frames") or metres × 1e4
 *     otherwise;
 ...
 * mission) carries metres, which the INT carrier scales by 1e4 instead. Mirrors
 * the mission item builder's frame table (§9 "Coordinate frames").
 * @type {Set<number>}
 */
...
/**
 * Scale factor from metres to the wire int32 for a local frame: common.xml
 * documents COMMAND_INT x/y as "local: x position in meters * 1e4".
 */
const METRES_E4 = 1e4;

/**
 * COMMAND_INT's x/y declare `invalid="INT32_MAX"` — the spec's "unused / keep
 * the current value" sentinel. The COMMAND_LONG float params carry that meaning
 * as `NaN` (first-class here: see `keepParam` in lib/swarm), which an int32
 * cannot hold, so the sentinel is translated in both directions.
 */
 ...
 *
 * A `NaN` lat/lon is the operator's "use the current position" sentinel (e.g. a
 * DO_REPOSITION that only changes altitude); it becomes {@link INT32_MAX} here,
 * never 0 — that would command null island.
 *
...
function scaleLatLon(value) {
  ...
  if (Number.isNaN(n)) return INT32_MAX;
 ...
 * @returns {number} decimal degrees, or NaN for the keep-current sentinel
 */
function unscaleLatLon(value) {
  const n = Number(value);
  if (n === INT32_MAX) return NaN;
  return n / DEG_E7;
}

/**
 * Scale a local-frame offset in metres into the wire int32 (metres × 1e4, see
 * {@link METRES_E4}). Rounding to whole metres instead would quantize every
 * local reposition to the nearest metre and under-report it by 10⁴.
 *
...
function scaleLocalMetres(value) {
  const n = Number(value);
  if (Number.isNaN(n)) return INT32_MAX;
  return Math.round(n * METRES_E4);
}

/**
 * Reverse {@link scaleLocalMetres}.
 *
 * @param {number} value  int32 metres × 1e4
 * @returns {number} metres, or NaN for the keep-current sentinel
 */
 ...
 * param7 → z. For a global frame x/y are scaled to `degE7`; for any other frame
 * they are metres scaled by 1e4. z is a float altitude and is never scaled.
 *
...
function longToIntFields(paramArray, opts = {}) {
  ...
  return {
    ...
    param3: num(p[2]),
    param4: num(p[3]),
    x: global ? scaleLatLon(num(p[4])) : scaleLocalMetres(num(p[4])),
    y: global ? scaleLatLon(num(p[5])) : scaleLocalMetres(num(p[5])),
    z: num(p[6]),
  };
}
 ...
 * y → param6 (un-scaled from `degE7` for a global frame, from metres × 1e4
 * otherwise), z → param7.
 *
...
function intFieldsToLong(fields) {
  ...
  return [
    ...
    num(f.param3),
    num(f.param4),
    global ? unscaleLatLon(num(f.x)) : unscaleLocalMetres(num(f.x)),
    global ? unscaleLatLon(num(f.y)) : unscaleLocalMetres(num(f.y)),
    num(f.z),
  ];
}
