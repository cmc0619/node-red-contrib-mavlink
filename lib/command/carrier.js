...
function longToIntFields(paramArray, opts = {}) {
  ...
  const intXY = (value, kind, name) => {
    ...
    // metres × 1e4 when local (§9, §14 — both halves measured against SITL).
    // A natively-degE7 param and a non-location param5/6 (gimbal flags, etc.)
    // carry whatever the operator entered, unscaled, in either frame.
    if (kind === 'latlon') return global ? scaleLatLon(n) : scaleLocalMetres(n);
    return Math.round(n);
  };
...
function intFieldsToLong(fields, opts = {}) {
  ...
  const fromXY = (value, kind) => {
    ...
    return global ? unscaleLatLon(n) : unscaleLocalMetres(n);
  };
  return [
    num(f.param1),
    num(f.param2),
