...
function commandByValue(bundle, commandId) {
  ...
  return index.get(Number(commandId)) || null;
}
...
function longToIntFields(paramArray, opts = {}) {
  ...
  // Absent stays unset — Number('') / Number(' ') / Number(null) are finite 0
  // and would bypass the pack bag check (§0).
  const scalar = (value) => {
  ...
  return {
    ...
    param1: scalar(paramArray[0]),
    param2: scalar(paramArray[1]),
    param3: scalar(paramArray[2]),
    param4: scalar(paramArray[3]),
    x: intXY(paramArray[4], kinds[5]),
    y: intXY(paramArray[5], kinds[6]),
    z: scalar(paramArray[6]),
  };
}
...
/**
 * Run an acked transaction under the §9 wrong-carrier rule: at most ONE swap
 * per transaction. When the vehicle acks `COMMAND_INT_ONLY` (8) or
 * `COMMAND_LONG_ONLY` (7) it will only accept the *other* carrier — announce
 * through `onSwap` and re-run once in that form, rebuilt by the caller's `run`.
 * A second wrong-carrier ack (the same code again, or a contradictory one), or
 * a first ack demanding the carrier already sent, cannot be resolved by
 * (another) swap: the outcome comes back with `wrongCarrier` naming why, and
 * the caller fails loud (§9 user requirement). One owner for mavlink-command
 * and mavlink-payload.
 *
 * @param {object} opts
 * @param {'long'|'int'} opts.carrier  first attempt: the configured carrier
 * @param {(carrier: 'long'|'int') => Promise<import('./ack').AckResult>}
 *   opts.run  run one AckWaiter transaction in the given carrier — the caller
 *   rebuilds the wire message from the same canonical params
 * @param {(outcome: object, from: string, to: string) => void} [opts.onSwap]
 *   called before the one re-run (warn + badge belong to the node)
 * @returns {Promise<{outcome: object, carrier: string, swapped: boolean,
 *   wrongCarrier: ?string}>}  the final outcome, the carrier that produced it,
 *   and — when no carrier satisfies the vehicle — the fail-loud detail
 */
async function runWithCarrierSwap({ carrier, run, onSwap }) {
  const outcome = await run(carrier);
  const wanted = carrierWantedBy(outcome.resultCode);
  // §5 affirmative dispatch on the demanded carrier. The dynamic
  // `case carrier:` sits first so a demand for the carrier already sent —
  // a contradiction no swap can resolve — matches before the swap arms.
  switch (wanted) {
    case null:
      return { outcome, carrier, swapped: false, wrongCarrier: null };
    case carrier: {
      const label = `COMMAND_${carrier === CARRIER.INT ? 'INT' : 'LONG'}`;
      return {
        outcome,
        carrier,
        swapped: false,
        wrongCarrier: `vehicle demands ${label} but that carrier was already sent`,
      };
    }
    case CARRIER.LONG:
    case CARRIER.INT: {
      if (onSwap) onSwap(outcome, carrier, wanted);
      const second = await run(wanted);
      ...
      const stillWrong = carrierWantedBy(second.resultCode) !== null;
      return {
        outcome: second,
        carrier: wanted,
        swapped: true,
        wrongCarrier: stillWrong
          ? `carrier swap ${carrier}→${wanted} still rejected as ` +
            `${RESULT_NAME[second.resultCode]} — no carrier satisfies the vehicle`
          : null,
      };
    }
    default: break; // This space intentionally left blank (§5)
