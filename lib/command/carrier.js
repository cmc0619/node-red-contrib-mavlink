...
async function runWithCarrierSwap({ carrier, run, onSwap }) {
  ...
  if (!wanted) return { outcome, carrier, swapped: false, wrongCarrier: null };
  const from = carrier;
  carrier = wanted;
  if (onSwap) onSwap(outcome, from, carrier);
  outcome = await run(carrier);
  ...
  const stillWrong = carrierWantedBy(outcome.resultCode) !== null;
  return {
    outcome,
    carrier,
    swapped: true,
    wrongCarrier: stillWrong
      ? `carrier swap ${from}→${carrier} still rejected as ` +
        `${RESULT_NAME[outcome.resultCode]} — no carrier satisfies the vehicle`
      : null,
  };
}
