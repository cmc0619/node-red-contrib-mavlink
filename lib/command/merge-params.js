...
function mergeParams(config, payload) {
  ...
  for (const [k, v] of Object.entries(raw)) {
    const idx = Number(k);
    // Empty string is blank, not zero — Number('') === 0 would aim coords at
    // null island (issue #88). Skip so the slot stays absent.
    if (Number.isInteger(idx) && idx >= 1 && idx <= 7 && v !== undefined && v !== '') {
      out[idx] = Number(v);
    }
  }
