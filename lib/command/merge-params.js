...
/**
 * Once-parsed `config.params` per node config object — the string is
 * deploy-constant, so the per-input JSON.parse repeated one answer. Only a
 * successful parse is cached: a malformed saved value still throws on every
 * input (crater unchanged, no deploy-time error path gained).
 * @type {WeakMap<object, object>}
 */
const parsedByConfig = new WeakMap();
...
function mergeParams(config, payload) {
  const out = {};
  let raw = parsedByConfig.get(config);
  if (raw === undefined) {
    raw = JSON.parse(config.params);
    parsedByConfig.set(config, raw);
  }
