...
class AckWaiter {
  ...
  /**
   * Whether the ack is addressed to this station. MAVLink 2 COMMAND_ACK
   * carries target_system/target_component extension fields naming the GCS the
   * ack answers; on a shared link two stations issuing the same MAV_CMD to the
   * same vehicle would otherwise settle each other's waits with the wrong
   * result. Only an ack *explicitly addressed elsewhere* is rejected: absent
   * fields (MAVLink 1) or 0 mean unaddressed and pass, and with no
   * `sourceIds` configured the gate is off entirely.
   *
   * @param {object} fields  decoded COMMAND_ACK fields
   * @returns {boolean}
   */
  _addressedToUs(fields) {
    if (!this._sourceIds) return true;
    if (
      fields.target_system !== undefined &&
      Number(fields.target_system) !== 0 &&
      Number(fields.target_system) !== Number(this._sourceIds.sysid)
    ) {
      return false;
    }
    if (
      ...
      Number(fields.target_component) !== Number(this._sourceIds.compid)
    ) {
      return false;
    }
    return true;
  ...
  _onAck(decoded) {
    ...
    if (!this._addressedToUs(fields)) return;
