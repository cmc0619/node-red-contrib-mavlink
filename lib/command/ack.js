 ...
 *   extension, command-specific — often the reason for a denial); null when
 *   the frame carries no such field, and on every settle without an ack
 */
...
class AckWaiter {
   ...
   * @param {(progress: number|null, resultParam2: number|null) => void}
   *   [opts.onInProgress]  called on each IN_PROGRESS ack, before the re-arm.
   *   `progress` is the 0–100 percentage, null when the frame omits the
   *   extension field or reports the 255 unknown sentinel
  ...
  _onAck(decoded) {
    ...
    if (result === MAV_RESULT.IN_PROGRESS) {
      ...
      if (this._onInProgress) {
        ...
        const progress = fields.progress === 255 ? null : orNull(fields.progress);
        this._onInProgress(progress, orNull(fields.result_param2));
      }
    ...
    if (TERMINAL_RESULTS.has(result)) {
      ...
      this._settle({
        ...
        retries: this._retries + this._resends,
        elapsed: this._now() - this._startMs,
        resultParam2: orNull(fields.result_param2),
...
/**
 * A COMMAND_ACK extension field decoded from a frame that does not carry it is
 * `undefined`; status records carry `null` for "absent".
 *
