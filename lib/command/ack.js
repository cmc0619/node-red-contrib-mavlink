 ...
 *     microservice's recovery for a command or ack lost on the wire; MAVSDK
 *     parity at 3), then settle 'timeout' so the caller runs the §9
 *     classification (peer-table check → unconfirmed) unchanged
 *
...
/** Default re-sends after an ack-timeout window closes (MAVSDK parity: 3). */
...
class AckWaiter {
   ...
   * @param {number}  [opts.maxResends]  re-sends after an ack-timeout window
   *   before settling 'timeout' (default {@link DEFAULT_MAX_RESENDS})
  ...
  constructor(opts) {
    ...
    this._maxResends = opts.maxResends !== undefined ? opts.maxResends : DEFAULT_MAX_RESENDS;
   ...
   * is the command-microservice recovery for both). Only after the final
   * window closes does it resolve with the timeout marker — the caller decides
   * what to do next (check peer table etc), exactly as before.
   */
  _onTimeout() {
    if (this._settled) return;
    if (!this._noAutoRetry && this._resends < this._maxResends) {
      this._resends += 1;
      this._confirmation += 1;
