 ...
 *     deadline forever
 *   - TEMPORARILY_REJECTED (1): back off and retry while the budget lasts
 *   - ACCEPTED (0): terminal success
 *   - everything else: terminal failure
 *   - silence: re-send while retries remain — the command protocol's
 *     confirmation transmissions, for a frame that dropped on the way out.
 *     Budget spent, settle 'timeout' so the caller runs the §9 classification
 *     (peer-table check → unconfirmed). An answer marks the transmission as
 *     arrived: after IN_PROGRESS the command is in work and is never re-sent
 *     into — silence after that is the vehicle's, not the link's.
 *     TEMPORARILY_REJECTED is the one answer that asks for another
 *     transmission, and that retry starts unheard: it can drop like the first.
 *
...
const { MAV_RESULT, RESULT_NAME } = require('./status-record');
 ...
 * @property {'ack'} [confirmedBy]  present only when a terminal ack answered
 * @property {number} [sysid]  the responder's system id; present with confirmedBy
 * @property {number} [compid]  the responder's component id; present with
 *   confirmedBy. A target addressed at component 0 is answered by one
 *   component, and completion reads that one's state (§14)
 * @property {number}  retries       - total re-transmissions beyond the first
 *   send (TEMPORARILY_REJECTED back-offs and silence re-sends alike); equals
 *   the final confirmation byte
 *   on the LONG carrier
...
class AckWaiter {
   ...
   * @param {number}  opts.timeoutMs  the ack window armed after every send
   * @param {number}  opts.maxRetries  re-sends beyond the first, one budget
   *   spent on TEMPORARILY_REJECTED back-offs and on silence alike
   ...
   *   identity; an ack explicitly addressed to a *different* GCS (MAVLink 2
   *   target_system/target_component extension fields) is ignored
  ...
  constructor(opts) {
    ...
    this._onInProgress = opts.onInProgress;
  ...
  start() {
    return new Promise((resolve) => {
      ...
      // admission — must never settle a transaction; keyless links carry no
      // mark and are unaffected (§7 trust ruling). One filter site covers every
      // AckWaiter caller.
      ...
      try {
        ...
        // finally has dropped its cancel handle by then.
        this._settled = true;
        this._cleanup();
        throw err;
      }
   ...
   * unless the command was addressed to all components (target compid 0).
   *
   * @param {object} decoded  decoded COMMAND_ACK message
   * @returns {boolean}
   */
  _matchesSource(decoded) {
    return (this._targetSystem === 0 || decoded.sysid === this._targetSystem)
      && (this._targetComponent === 0 || decoded.compid === this._targetComponent);
  }
  ...
  _onAck(decoded) {
    ...
    // An answer proves this transmission arrived: from here silence is the
    // vehicle going quiet, never a dropped frame, and is not re-sent into.
    // A TEMPORARILY_REJECTED retry is a new transmission and clears this.
    this._heard = true;
    ...
    if (result === MAV_RESULT.IN_PROGRESS) {
      ...
      // re-arms let periodic IN_PROGRESS extend the deadline forever.
      // At or past the ceiling the currently armed window is left to expire.
      const remaining =
        this._startMs + this._timeoutMs * IN_PROGRESS_CEILING - this._now();
    ...
    // Every result but IN_PROGRESS is terminal — including a MAV_RESULT this
    // table has no name for, which settles with `resultCode` alone.
    this._settle({
  ...
  /**
   * One re-transmission: bump the confirmation byte (0 = first transmission,
   * 1–255 = confirmation transmissions), re-arm the window, send. Shared by
   * the TEMPORARILY_REJECTED back-off and the silence path. The new frame
   * starts unheard — a rejection answered the previous one, and this one can
   * drop on the way out exactly as the first could.
   *
   * Timer context: nothing above this catches. sendFn reaches
   * Connection.send, which throws by design — QueueOverflowError at the
   * Control band cap, or identity resolution failing on a dead link — and a
   * throw escaping here is an uncaughtException, not a failed command. Settle
   * the transaction instead, on exactly the saturated or dropped link where
   * the retry is most likely to be the one that throws. The initial send is
   * inside the start() executor, so it rejects rather than escaping.
   */
  _resend() {
    this._retries += 1;
    ...
    try {
      ...
      this._settle({
        ...
        elapsed: this._now() - this._startMs,
        resultParam2: null,
        detail: `retry send failed: ${err.message}`,
      });
    }
  }

  /**
   * Called when the ack window closes. Unanswered with retries left, the
   * frame is taken as dropped and re-sent; otherwise resolves with the
   * timeout marker — the caller decides what to do next (check peer table
   * etc).
   */
  _onTimeout() {
    if (!this._heard && this._retries < this._maxRetries) {
      this._resend();
      return;
    }
    this._settle({
...
function cancelSlot() {
  return {
    ...
    /**
     * Run one waiter in the slot: cancel whatever is in flight, hold this one
     * while it runs, release it when it settles.
     *
     * @param {AckWaiter} waiter
     * @returns {Promise<AckResult>}
     */
    async run(waiter) {
      this.cancel();
...
/**
 * The AckWaiter for a pre-built carrier message on a Connection node. Owns
 * the keys every acked caller (Command, Move, Payload, Fan-out) needs: the
 * subscribe hook, the per-carrier sendFn, the (command,
 * target, source) match, and the send options.
 *
 * @param {{subscribe: Function, send: Function, resolveSourceIds: Function}} connection
 * @param {{name: string, fields: object}} message  built carrier message
 * @param {object} opts
 * @param {string} opts.band  Connection.send band
 * @param {{sysid: number, compid: number}} opts.target
 * @param {string} [opts.identityId]
 * @param {number} opts.timeoutMs
 * @param {number} [opts.maxRetries]
 * @param {Function} [opts.onInProgress]
 * @returns {AckWaiter}
 */
function ackWaiterFor(connection, message, opts) {
  const { band, target, identityId } = opts;
  return new AckWaiter({
    subscribe: (filter, handler) => connection.subscribe(filter, handler),
    sendFn: sendFnFor(connection, message, { band, target, identityId }),
    ...
    sourceIds: connection.resolveSourceIds(identityId),
    timeoutMs: opts.timeoutMs,
    maxRetries: opts.maxRetries,
    onInProgress: opts.onInProgress,
  });
}

/**
 * The status-record fields an {@link AckResult} carries, one spelling for
 * every node that reports one. `confirmedBy` rides through: 'ack' only when a
 * terminal ack settled the run, absent on timeout, cancel and a failed
 * re-send.
 *
 * @param {AckResult} outcome
 * @returns {object}
 */
function ackRecordFields(outcome) {
  return {
 ...
 * Only an ack *explicitly addressed elsewhere* is rejected: 0 means
 * unaddressed and passes — and a MAVLink 1 ack, or a MAVLink 2 ack with the
 * extension truncated away, decodes those fields as 0 (§14). One gate, shared
 * by AckWaiter and Fan-out's broadcast confirm (§9/§10).
 *
 * @param {object} fields  decoded COMMAND_ACK fields
 * @param {{sysid: number, compid: number}} sourceIds  our own source ids
 * @returns {boolean}
 */
function ackAddressedTo(fields, sourceIds) {
  return (fields.target_system === 0 || fields.target_system === sourceIds.sysid)
    && (fields.target_component === 0 || fields.target_component === sourceIds.compid);
}

/**
 * A node's ack wait, one spelling for Command, Move and Payload: badge while
 * sending and on every IN_PROGRESS (a takeoff or a long reposition answers it
 * for seconds, §9, and without this the operator watches an unchanging wait),
 * then settle to the AckWaiter outcome under the node's cancel slot.
 *
 * @param {object} node  Node-RED node instance
 * @param {{run: Function}} slot  the node's cancelSlot
 * @param {object} connection  Connection node
 * @param {{name: string, fields: object}} message
 * @param {string} label  badge word for the command
 * @param {{target: object, identityId: string, timeoutMs: number, maxRetries: number}} opts
 * @returns {Promise<AckResult>}
 */
function awaitAckWithBadge(node, slot, connection, message, label, opts) {
  applyActionStatus(node, 'sending', `${label}\u2026`);
  return slot.run(ackWaiterFor(connection, message, {
    ...opts,
    band: BAND.CONTROL,
    onInProgress: (progress) => applyActionStatus(
      node,
      'sending',
      progress === null ? `in progress ${label}\u2026` : `in progress ${progress}% ${label}\u2026`
    ),
  }));
}
