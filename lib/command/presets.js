...
function blankLocationRefusal(preset, userParams) {
  ...
  // them. §9 (#263) rules that where a location is *present* it must also be in
  // range, and takeoff/land expose param5/6 without a `requireLocation` key —
  // nesting the range test inside that gate let 200°/200° through on exactly
  // the two presets the presence rule deliberately exempts. Blank there still
  // means "here" (see requireLocationFor); 200 there is still garbage.
  const range = latLonRangeRefusal(preset, userParams);
...
/**
 * The refusal for a present-but-out-of-range coordinate, or null when both are
 * legal. Runs only after the presence checks pass: where a preset's location
 * is present it must also be in range — |lat| ≤ 90, |lon| ≤ 180 (§9, #263).
 ...
 * hazard class Move's requireGlobalPosition guard names, closed here on the
 * preset path.
 *
 * @param {object} preset
 * @param {Object<number, *>} userParams  operator input, keyed 1-7
 * @returns {string|null}
 */
function latLonRangeRefusal(preset, userParams) {
