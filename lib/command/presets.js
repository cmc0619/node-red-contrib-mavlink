...
const PRESETS = [
  ...
  },
  {
    ...
    // Same hazard the land preset pinned away: the editor renders no
  ...
  {
    id: 'run_prearm_checks',
    group: 'system',
    name: 'Run Prearm Checks',
...
function presetGroups() {
  ...
  return order.map((g) => ({ group: g, presets: byGroup.get(g) }));
}
...
function buildParamArray(preset, userParams) {
  ...
  // zero-fill there is a live command, not an absence (GCS parity:
  //  QGC/MAVSDK transmit exactly these for unspecified fields).
