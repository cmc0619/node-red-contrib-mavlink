...
/**
 * The gimbal-aim path selects one of three different wire messages, so it is a
 * routing token. This lookup stays tolerant — null for an unknown combination,
 * a `legacy` preview for a blank path — because the editor-metadata callers
 * (fieldMetaFromBundle, carrierMattersFor) render off it and must not throw.
 * The runtime crater for a blank/unknown path lives in buildPayloadMessage via
 * {@link requireGimbalPath}, before this is reached.
 *
 * @param {string} topic
 * @param {string} verb
 * @param {string} [path]
...
/** The three gimbal-aim paths, each a different wire message. */
const GIMBAL_AIM_PATHS = ['legacy', 'manager', 'manager-cmd'];

/**
 * Affirmative dispatch for the gimbal-aim path token at runtime: a blank or
 * unknown path craters naming the path vocabulary, rather than `|| 'legacy'`
 * silently picking DO_MOUNT_CONTROL or a composite-key miss mis-reporting
 * "unknown verb". The editor's select always saves a member (no §14 blank
 * carve-out like sendAs).
 *
 * @param {*} path
 */
function requireGimbalPath(path) {
  if (!GIMBAL_AIM_PATHS.includes(path)) {
    throw new Error(
      `unknown gimbal aim path ${JSON.stringify(path)} — expected one of ${GIMBAL_AIM_PATHS.join(', ')}`
    );
  }
}
...
function buildPayloadMessage(input) {
  ...
  // Affirmative dispatch: the gimbal-aim path routes three different wire
  // messages, so a blank/unknown path craters here (recipeFor itself stays
  // tolerant for the editor-metadata callers).
  if (topic === 'gimbal' && verb === 'aim') requireGimbalPath(input.path);
 ...
 * @param {object} values
 * @param {string} [carrier]  'int' builds COMMAND_INT; anything else COMMAND_LONG (§9)
...
function buildFromRecipe(recipe, target, values, carrier, frame) {
  ...
  // §9: 'int' builds COMMAND_INT; everything else the frameless COMMAND_LONG.
 ...
 * @param {number[]} [params]
 * @param {string} carrier  'int' builds COMMAND_INT; anything else COMMAND_LONG
 * @param {number} [frame]  MAV_FRAME for the INT carrier
...
function commandBacked(target, command, params = [], carrier, frame, coordKinds) {
  // Blank slots beyond the recipe's params pad to 0; NaN rides — a legal wire
  // float (manager-cmd rates, "axis not rate-controlled").
  const normalized = [0, 1, 2, 3, 4, 5, 6].map((i) => valueOr(params[i], 0));
  const message = carrier === CARRIER.INT
    ? buildCommandInt(command, target.sysid, target.compid, normalized, { frame, coordKinds })
    : buildCommandLong(command, target.sysid, target.compid, normalized, 0);
  return { confirmation: 'command_ack', message };
}

/**
 * Blank means fallback; everything else is Number()'d. NaN is a legal wire
 * float — "not used" per the dialect's invalid=NaN annotations — so it rides.
 *
 * @param {*} value
