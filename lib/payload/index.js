 ...
 *   check. A blank operator/msg value is NaN — incomplete info must not invent
 *   recipe defaults or 0 (§0). Pinned slots are recipe-owned constants.
 * @returns {number}
 */
...
function commandBacked(target, command, params, carrier, frame, coordKinds) {
  ...
  // slotValue is already NaN and must not be re-invented as 0 here.
  const normalized = [0, 1, 2, 3, 4, 5, 6].map((i) => (
    i < params.length ? Number(params[i]) : 0
  ));
  let message;
  switch (carrier) {
    case CARRIER.INT:
