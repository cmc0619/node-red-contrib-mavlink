...
const PAYLOAD_RECIPES = {
  'camera|photo|': {
    ...
    params: [
      ...
      { field: 'sequence', driver: captureSequence },
    ],
  },
  'camera|stop-photo|': {
    kind: 'command',
    ...
    // topics). Camera id only: the dialect marks params 2-7 reserved.
  ...
  'camera|start-stream|': {
    kind: 'command',
    command: MAV_CMD.VIDEO_START_STREAMING,
    params: [
      { field: 'streamId', default: 0 },
      { field: 'cameraId', default: 0 },
    ],
  ...
  'camera|stop-stream|': {
    kind: 'command',
    command: MAV_CMD.VIDEO_STOP_STREAMING,
    params: [
      { field: 'streamId', default: 0 },
      { field: 'cameraId', default: 0 },
    ],
  ...
  'camera|track-point|': {
    kind: 'command',
    command: MAV_CMD.CAMERA_TRACK_POINT,
    params: [
      { field: 'pointX', default: 0 },
      { field: 'pointY', default: 0 },
      { field: 'radius', default: 0 },
      { field: 'cameraId', default: 0 },
    ],
  },
  'camera|track-rectangle|': {
    kind: 'command',
    coordKinds: { 5: 'raw', 6: 'raw' },
    command: MAV_CMD.CAMERA_TRACK_RECTANGLE,
    params: [
      { field: 'topLeftX', default: 0 },
      { field: 'topLeftY', default: 0 },
      { field: 'bottomRightX', default: 1 },
      { field: 'bottomRightY', default: 1 },
      { field: 'cameraId', default: 0 },
    ],
  },
  'camera|stop-tracking|': {
    kind: 'command',
    command: MAV_CMD.CAMERA_STOP_TRACKING,
    params: [{ field: 'cameraId', default: 0 }],
  },
  'camera|storage-format|': {
    ...
    params: [
      { field: 'storageId', default: 1 },
      { field: 'format', default: 0 },
      { field: 'resetImageLog', default: 0 },
    ],
  },
  'camera|set-mode|': {
    kind: 'command',
  ...
  'gimbal|aim|manager': {
    ...
    fields: [
      ...
      // firmwares read as ambiguous and silently drop.
  ...
  'gimbal|aim|manager-cmd': {
    ...
    // the command form acks, the message form cannot). Rates default to NaN
    // for the same reason: an angle aim must not also
    // command a zero rate.
    params: [
      { field: 'pitch', default: 0 },
      { field: 'yaw', default: 0 },
  ...
  'gimbal|configure|': {
    kind: 'command',
    command: MAV_CMD.DO_GIMBAL_MANAGER_CONFIGURE,
    params: [
      { field: 'primarySysid', default: -1 },
      { field: 'primaryCompid', default: -1 },
      { field: 'secondarySysid', default: -1 },
      { field: 'secondaryCompid', default: -1 },
      null,
      null,
      { field: 'gimbalDeviceId', default: 0 },
    ],
  ...
  'gimbal|take|': {
    ...
    command: MAV_CMD.DO_GIMBAL_MANAGER_CONFIGURE,
    params: [
      // -2 is the protocol's "set itself" value; the MAVLink header carries
      // the sender identity, so this remains correct on every delivery tier.
      { field: 'primarySysid', default: -2, pinned: true },
      { field: 'primaryCompid', default: -2, pinned: true },
      { field: 'secondarySysid', default: -1, pinned: true },
      { field: 'secondaryCompid', default: -1, pinned: true },
      null,
      null,
      { field: 'gimbalDeviceId', default: 0 },
    ],
  ...
  'gimbal|release|': {
    ...
    command: MAV_CMD.DO_GIMBAL_MANAGER_CONFIGURE,
    params: [
      { field: 'primarySysid', default: -3, pinned: true },
      { field: 'primaryCompid', default: -3, pinned: true },
      { field: 'secondarySysid', default: -1, pinned: true },
      { field: 'secondaryCompid', default: -1, pinned: true },
      null,
      null,
  ...
  'gimbal|aim|attitude': {
    ...
    // reasoning the manager pitch/yaw paths follow for their rates).
    fields: [
      { field: 'flags', wire: 'flags', default: 0 },
      { field: 'gimbalDeviceId', wire: 'gimbal_device_id', default: 0 },
      {
 ...
 * returns undefined and buildPayloadMessage craters on the missing recipe the
 * same as any other unmapped topic/verb — a blank path is not a routing choice
 * we get to pick.
 *
 * @param {string} topic
 * @param {string} verb
...
function recipeFor(topic, verb, path) {
  ...
  return PAYLOAD_RECIPES[`${topic}|${verb}|${pathKey}`];
}
...
function buildPayloadMessage(input) {
  ...
  return buildFromRecipe(
    ...
    input.target, input.values, input.carrier, input.frame, input.bookkeeping
  );
}
 ...
 * @param {object} [bookkeeping]  driver-owned counters a `driver` slot draws on
 ...
 *   missing message
 */
function buildFromRecipe(recipe, target, values, carrier, frame, bookkeeping) {
  switch (recipe.kind) {
    case 'message': {
    ...
    case 'command': {
      const params = recipe.params.map((slot) => (
        slot && !slot.driver ? slotValue(values, slot) : 0
      ));
      // Driver-owned slots read the resolved operator slots, so they go last.
      recipe.params.forEach((slot, i) => {
        if (slot && slot.driver) params[i] = slot.driver(target, params, bookkeeping);
      });
...
/**
 * @param {object} values
 * @param {{field: string, valueKey?: string, default?: number, required?: boolean,
 *          pinned?: boolean, driver?: Function}} slot
 ...
 *   constants. Unset values are not invented here (§0).
...
function slotValue(values, slot) {
  ...
  if (Number.isNaN(slot.default)) return NaN;
  return undefined;
}
...
function metaFromRow(row, opts) {
  ...
  if (!row) return empty;
  if (!opts.allowHidden && isHiddenParam(row)) return empty;
  ...
  const enumTable = enumName && opts.enums ? opts.enums[enumName] : null;
  return {
    label: row.label,
    ...
    enum: enumName,
    bitmask: Boolean(row.bitmask) || row.display === 'bitmask' || (enumTable !== null && Boolean(enumTable.bitmask)),
  };
}
...
function fieldMetaFromBundle(bundle, topic, verb, path) {
  ...
  // The recipe's default, not the XML's: it is what a pinned slot encodes and
  // what the dialog shows as the placeholder; a blank exposed slot passes
  // through unset (§0). The gimbal
  // manager's NaN rates ("axis not rate controlled") reach the dialog as null
  // — JSON has no NaN — and render blank.
  ...
  switch (recipe.kind) {
    case 'command': {
      ...
      recipe.params.forEach((slot, i) => {
        if (!slot || slot.pinned || slot.driver) return;
        const param = cmd
    ...
    case 'message': {
      const message = bundle.messages[recipe.message];
      for (const slot of recipe.fields) {
...
function carrierMattersFor(bundle, topic, verb, path) {
  ...
  const cmd = commandByValue(bundle, recipe.command);
  return Boolean(cmd.hasLocation);
}
...
/**
 * The capture sequence for one IMAGE_START_CAPTURE, as the dialect defines
 * it: for a single capture (param3 == 1) a number starting from 1 that goes up
 * with every capture command, so a camera can tell a re-transmission from a
 * second photo; 0 for anything else. It is bookkeeping the driver owns, like
 * the header `seq` nobody types — so the slot renders no control and takes
 * nothing from the operator. The counter itself lives on the node, per camera
 * component, and comes in through `bookkeeping` (471#115; MAVSDK keeps the
 * same per-component counter and stamps 0 for component 0).
 *
 * A declaration, not an expression, so PAYLOAD_RECIPES above can name it.
 *
 * @param {{sysid:number,compid:number}} target
 * @param {number[]} params  the other slots, already resolved
 * @param {{captureSequence: (compid:number)=>number}} bookkeeping
 * @returns {number}
 */
function captureSequence(target, params, bookkeeping) {
  return params[2] === 1 ? bookkeeping.captureSequence(target.compid) : 0;
}
