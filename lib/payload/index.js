...
function commandBacked(target, command, params = [], carrier, frame, coordKinds) {
  // A slot value is already a number out of slotValue — including a recipe's
  // deliberate NaN sentinel (manager-cmd rates, "axis not rate-controlled"),
  // which valueOr would refuse as non-finite. valueOr stays for the padding
  // beyond the recipe's params, where undefined must become 0.
  const normalized = [0, 1, 2, 3, 4, 5, 6].map((i) => (
    typeof params[i] === 'number' ? params[i] : valueOr(params[i], 0)
  ));
