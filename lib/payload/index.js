  ...
  switch (recipe.kind) {
    ...
    case 'command': {
      ...
      recipe.params.forEach((slot, i) => {
        if (slot?.driver) params[i] = slot.driver(target, params);
      });
...
/**
 * IMAGE_START_CAPTURE sequence numbers, one counter per camera (sysid/compid),
 * process-wide like the FTP request sequence: two Payload nodes shooting one
 * camera share its count. The dialect starts a camera at 1, and a
 * re-transmission carries the number of the command it repeats, so a restart
 * that begins again at 1 is the same exposure MAVSDK accepts.
 *
 ...
 * nothing from the operator (471#115; MAVSDK keeps the same per-camera counter).
 *
 ...
 * @returns {number}
 */
function captureSequence(target, params) {
  if (params[2] !== 1) return 0;
  const key = `${target.sysid}/${target.compid}`;
  const next = captureSequences.has(key) ? captureSequences.get(key) + 1 : 1;
  captureSequences.set(key, next);
  return next;
}
