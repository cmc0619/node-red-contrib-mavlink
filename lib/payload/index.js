 ...
 *          increment?: *, enum?: *}|null|undefined} row
...
function metaFromRow(row, opts) {
  ...
  const num = (value) => (hasValue(value) && Number.isFinite(Number(value)) ? Number(value) : null);
  return {
...
function fieldMetaFromBundle(bundle, topic, verb, path) {
  ...
  if (recipe.kind === 'command') {
    ...
    (recipe.params || []).forEach((slot, i) => {
      ...
      const param = cmd
        ...
        : null;
      out[slot.valueKey || slot.field] = metaFromRow(param);
    });
    return out;
  }
  ...
  for (const slot of recipe.fields || []) {
    const f = message
      ...
      : null;
    out[slot.valueKey || slot.field] = metaFromRow(f, { allowHidden: true });
  }
  return out;
}

/**
 * The fields the dialog renders for a verb, straight from the recipe — no
 * dialect needed, so the editor can decide visibility without a round trip.
 * Pinned slots are excluded: the recipe fills those, the operator does not.
 *
 * @param {string} topic
 * @param {string} verb
 * @param {string} [path]
 * @returns {string[]} row keys, sorted
 */
function payloadFormFields(topic, verb, path) {
  const recipe = recipeFor(topic, verb, path);
  if (!recipe) return [];
  const slots = recipe.kind === 'command' ? recipe.params || [] : recipe.fields || [];
  return slots
    .filter((slot) => slot && !slot.pinned)
    .map((slot) => slot.valueKey || slot.field)
    .sort();
}
