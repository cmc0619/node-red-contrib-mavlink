...
function buildFromRecipe(recipe, target, values, carrier, frame) {
  ...
  // An unmatched kind selects no builder (§5); PAYLOAD_RECIPES is the
  // vocabulary, and the caller craters on the missing message.
  return undefined;
}
...
function fieldMetaFromBundle(bundle, topic, verb, path) {
  ...
  // An unmatched kind derives no fields (§5) — same empty answer as the
  // soft no-recipe path above.
  return out;
}
