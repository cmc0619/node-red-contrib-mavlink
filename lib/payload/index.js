...
/**
 * Dialect description for one MAV_CMD param index, or '' if missing/hidden.
 *
 * @param {object} bundle
...
function descriptionForCommandParam(bundle, commandId, paramIndex) {
  ...
  if (!param || isHiddenParam(param)) return '';
  const text = param.description != null ? String(param.description).trim() : '';
  return text;
}

/**
 * Dialect description for one message field, or ''.
 *
 * @param {object} bundle
...
/**
 * Join {@link PAYLOAD_RECIPES} to a dialect bundle so the editor can set
...
function fieldTipsFromBundle(bundle, topic, verb, path) {
  ...
  const tips = {};
  if (!recipe || !bundle) return tips;

  if (recipe.kind === 'command') {
    (recipe.params || []).forEach((slot, i) => {
      if (!slot) return;
      const text = descriptionForCommandParam(bundle, recipe.command, i + 1);
      if (text) tips[slot.field] = text;
    });
    return tips;
  }

  for (const slot of recipe.fields || []) {
    const text = descriptionForMessageField(bundle, recipe.message, slot.wire);
    if (text) tips[slot.field] = text;
  }
  return tips;
}
