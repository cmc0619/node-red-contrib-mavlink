...
/**
 * The gimbal-aim path (legacy/manager/manager-cmd) composes into the recipe
 * key. No `|| 'legacy'` default: a blank or unknown path misses, so recipeFor
 * returns null and buildPayloadMessage throws the same `unknown payload verb`
 * crater as any other unmapped topic/verb — a blank path is not a routing
 * choice we get to pick.
 *
 * @param {string} topic
 * @param {string} verb
 * @param {string} [path]
