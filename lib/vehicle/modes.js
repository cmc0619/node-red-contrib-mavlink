 ...
 *
 * @param {ModeContext} ctx
 * @returns {{byName: Object<string, number>,
 *   byValue: Object<string, string>, prefix: string}|null}
 */
function apModeTable(ctx) {
  ...
  return { byName: table.byName, byValue: table.byValue, prefix: `${enumName}_` };
}
...
function modeNumberFor(name, ctx) {
  ...
  const held = ctx.component?.modes;
  if (held) {
  ...
  switch (ctx.firmware) {
    case 'ardupilot': {
      ...
      const value = table ? table.byName[`${table.prefix}${key}`] : undefined;
