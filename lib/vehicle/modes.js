...
/** @param {*} name @returns {string} the case-folded match key */
 ...
 * @returns {string}
 */
function shortName(entry, prefix) {
...
function publishedName(entry, ctx) {
  ...
  if (entry.standardMode) {
    const table = ctx.bundle && ctx.bundle.enums
      ? ctx.bundle.enums[STANDARD_MODE_ENUM]
      : null;
    const std = table
      ? table.entries.find((e) => Number(e.value) === Number(entry.standardMode))
      : null;
...
function apModeTable(ctx) {
  ...
  const table = enumName && ctx.bundle && ctx.bundle.enums
    ? ctx.bundle.enums[enumName]
    : null;
  if (!table) return null;
  return { entries: table.entries, prefix: `${enumName}_` };
}
...
function modeNumberFor(name, ctx) {
  ...
  switch (ctx.firmware) {
    case 'ardupilot': {
      ...
      const entry = table
        ? table.entries.find((e) => nameKey(shortName(e, table.prefix)) === key)
        : null;
...
function modeNameFor(number, ctx) {
  ...
  switch (ctx.firmware) {
    case 'ardupilot': {
      const table = apModeTable(ctx);
      const entry = table
        ? table.entries.find((e) => Number(e.value) === Number(number))
        : null;
      if (entry) return shortName(entry, table.prefix);
      break;
    }
    case 'px4': {
