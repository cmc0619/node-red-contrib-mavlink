...
/**
 * Require a firmware to be one of the known values. Affirmative dispatch: the
 * former `includes(x) ? x : 'custom'` silently sank a blank or typo'd firmware
 * into `custom`, disabling firmware-specific behaviour the operator never asked
 * to disable. `custom` stays a real, selectable member — it is just no longer
 * the blank-catch-all. The Vehicle editor's select saves a member (§14).
 *
 * @param {string} firmware
 * @returns {string}
 */
function requireFirmware(firmware) {
  if (!FIRMWARE_TYPES.includes(firmware)) {
    throw new Error(
      `Vehicle Profile: unknown firmware ${JSON.stringify(firmware)} — expected one of ${FIRMWARE_TYPES.join(', ')}`
    );
  }
  return firmware;
}
...
/**
 * Coerce a vehicle family to one of the known values, blank/unknown → 'unknown'.
 * This stays a coercion — not a require — because {@link catalogLabel} in
 * lib/param/seed.js leans on it: an unnamed ArduPilot vehicle legitimately has
 * no family and reads as "unknown" (the union-of-names catalog). The Vehicle
 * *config* token is guarded separately by {@link requireFamily}.
 *
 * @param {string} family
 * @returns {string}
 */
...
/**
 * Require a vehicle family to be one of the known values. Affirmative dispatch
 * for the Vehicle config token: the former `includes(x) ? x : 'unknown'` sank a
 * blank/typo into `unknown`. `unknown` stays a real, selectable member (the
 * editor's first option) — just not the blank-catch-all. The editor saves a
 * member (§14).
 *
 * @param {string} family
 * @returns {string}
...
function requireFamily(family) {
  if (!VEHICLE_FAMILIES.includes(family)) {
    throw new Error(
      `Vehicle Profile: unknown vehicle family ${JSON.stringify(family)} — expected one of ${VEHICLE_FAMILIES.join(', ')}`
    );
  }
  return family;
}
...
/**
 * Require a non-blank dialect/revision token. Affirmative dispatch: the former
 * `config.dialect || 'ardupilotmega'` / `config.dialectRevision || 'seed'`
 * inherited from *code* on a blank — but "inherit" means from an upstream
 * config node, never a runtime literal, and the Vehicle Profile is itself the
 * source, so a blank is a broken profile that must crater (owner ruling). The
 * editor's pickers always save a member; a blank reaches here only via a
 * hand-edited flow. A bare guard is required because loadBundledSet drops a
 * blank name with `.filter(Boolean)` rather than throwing.
 *
 * @param {*} value
 * @param {string} label
 * @param {string} [name]  profile name for the message
 * @returns {string}
 */
function requireDialectToken(value, label, name) {
  if (typeof value !== 'string' || !value.trim()) {
    throw new Error(`Vehicle Profile '${name || 'unnamed'}': ${label} is required.`);
  }
  return value;
}
...
function dialectPicks(config) {
  const picks = [
    {
      dialect: requireDialectToken(config.dialect, 'dialect', config.name).toLowerCase(),
      revision: requireDialectToken(config.dialectRevision, 'dialect version', config.name),
    },
  ];
