...
function dialectPicks(config) {
  ...
  for (const token of String(config.additionalDialects).split(',')) {
    ...
    picks.push({
      dialect: token.slice(0, at).toLowerCase(),
      revision: token.slice(at + 1),
    });
  }
  return picks;
}
 ...
 *
 * @param {VehicleConfig} config
 * @returns {import('../metadata/compile').DialectBundle}
 */
function resolveDialect(config) {
  ...
  // Dated catalog snapshot. A file the manifest lists but the disk lacks
  // craters in compileXmlFromFile, naming it.
  const { catalog, entry } = catalogLookup(config, picks[0], config.name);
  return compileXmlFromFile(catalog.filePath(entry, revision));
}

/**
 * Resolve one dialect pick against the userDir XML catalog: find the pick's
 * snapshot manifest and locate the dialect's entry file inside it. The one
 * owner of the two refusals both snapshot paths (single-dialect and
 * multi-dialect profiles) share.
 *
 * @param {VehicleConfig} config
...
function catalogLookup(config, pick, label) {
  const catalog = new XmlCatalog({ baseDir: config.catalogBaseDir });
 ...
 * with any snapshot pick is assembled here: each pick contributes the files
 * its own include chain reaches, from its own source (seed or snapshot), in
 * selection order, then one synthetic entry includes every root. A snapshot
 * holds a whole definitions directory, so layering all of it replaced files
 * another pick had taken from the seed — `minimal@seed` + `icarous@<snap>`
 * compiled the snapshot's minimal.xml, though icarous includes nothing. A file
 * two picks' chains share is the later pick's, as a later root wins.
 *
 * @param {VehicleConfig} config
...
function resolveDialectSet(config, picks) {
  ...
  const seed = seedSources();
  const files = {};
  const entries = [];
  for (const pick of picks) {
    const source = pick.revision === 'seed'
      ? { entry: seedEntryFor(pick.dialect), files: seed }
      : snapshotFiles(config, pick, label);
    for (const name of resolveIncludeOrder(source.files, source.entry, new Map())) {
      files[name] = source.files[name];
    }
    entries.push(source.entry);
  }

  return compileProfile(files, entries, picks.map((p) => p.dialect).join('+'));
}
...
function snapshotFiles(config, pick, label) {
  ...
  for (const name of names) {
    files[name] = fs.readFileSync(catalog.filePath(name, pick.revision), 'utf8');
  }
  return { entry, files };
}
