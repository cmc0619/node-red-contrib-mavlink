...
const { compileXml } = require('../metadata/compile');
...
function resolveDialect(config) {
  ...
  if (!xmlPath) {
    ...
    throw new Error(
      `Vehicle Profile '${config.name}': missing XML '${entry}' in snapshot '${revision}'.`
    );
  }
...
function resolveDialectSet(config, picks) {
  ...
  return { ...compileXml(files, PROFILE_ENTRY), dialect: picks.map((p) => p.dialect).join('+'),};
}
