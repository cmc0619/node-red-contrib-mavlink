 ...
 *
 * Preferred library pick: `dialect` + `dialectRevision` (`seed` or snapshot id).
 * Custom path: `dialectSource === 'custom'` + `customDialectPath` compiles the
 * absolute XML — normal operating mode for path profiles.
 *
...
function resolveDialect(config) {
  ...
  if (revision === undefined || revision === null || revision === '') {
    // Path profiles omit revision → compile customDialectPath; else seed.
