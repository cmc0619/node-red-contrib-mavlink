 ...
 * decodes a number.
 *
 * INT64, UINT64 and REAL64 have no row: the 4-byte float slot cannot carry
 * them (they travel via PARAM_EXT), the editor never offers them, and the one
 * way one arrives is a vehicle echoing its own 64-bit parameter. The caller's
 * read of the missing row craters on its own — loud, not legible (§9).
 *
 * @param {number} paramType MAV_PARAM_TYPE value.
 * @returns {{name:string, kind:string, bytes:number}|undefined} Table entry.
 */
