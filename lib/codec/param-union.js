 ...
 */
function resolveParamType(paramType, _fieldName) {
  let key = paramType;
...
function paramValueToWire(value, paramType, fieldName = 'param_value') {
  ...
  writeInt(buf, n, info);
  // JS Number cannot carry every float32 bit pattern (signaling NaNs collapse
  // when later written with writeFloatLE). Fail loud rather than corrupt the
  // integer the vehicle will read back from the float slot.
  assertFloatBitsSurvive(buf, fieldName);
...
function paramValueFromWire(wireFloat, paramType, fieldName = 'param_value') {
  ...
  // Round-trip the float bytes through writeFloatLE again — if the pattern
  // cannot survive JS Number, the integer read would not match the vehicle.
  assertFloatBitsSurvive(buf, fieldName);
  return readInt(buf, info);
}

/**
 * Confirm a 4-byte float slot round-trips through `Number` without bit changes.
 * Signaling-NaN (and other non-canonical) patterns are rejected: they would
 * silently become a different integer after the next `writeFloatLE`.
 *
 * @param {Buffer} buf Four-byte buffer holding the candidate float bits.
 * @param {string} fieldName Field name for error messages.
 */
function assertFloatBitsSurvive(buf, fieldName) {
  const asFloat = buf.readFloatLE(0);
  const again = Buffer.alloc(4);
  again.writeFloatLE(asFloat, 0);
  if (!buf.equals(again)) {
    fail(
      fieldName,
      `integer bit pattern 0x${buf.toString('hex')} cannot survive JS float (would become 0x${again.toString('hex')})`
    );
  }
}
