 ...
 * decodes a number.
 *
 * A type with no entry is refused by name. The 4-byte float slot genuinely
 * cannot carry INT64, UINT64 or REAL64, so this is the format's own refusal
 * surfaced (§0 rule 1), not a guard: the editor never offers those types, and
 * the one way they arrive is a vehicle echoing its own 64-bit parameter.
 *
 * @param {number} paramType MAV_PARAM_TYPE value.
 * @returns {{name:string, kind:string, bytes:number}} Table entry.
 * @throws {Error} when the type does not fit the PARAM_VALUE slot.
 */
function resolveParamType(paramType) {
  const info = PARAM_TYPES[paramType];
  if (info === undefined) {
    throw new Error(`MAV_PARAM_TYPE ${paramType} does not fit the 4-byte PARAM_VALUE slot; 64-bit and real64 parameters travel via PARAM_EXT`);
  }
  return info;
}
