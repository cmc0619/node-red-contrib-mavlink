 ...
 */
function resolveToken(enumMeta, token, use64) {
  if (typeof token === 'bigint') {
    return use64 ? token : Number(token);
  }
  if (typeof token === 'number') {
    const value = BigInt(token);
    return use64 ? value : Number(value);
  }
  if (typeof token === 'string') {
    const s = token.trim();
    if (/^[+-]?\d+$/.test(s)) {
      const big = BigInt(s);
      return use64 ? big : Number(big);
    }
    const entry = enumMeta.entries.find((e) => e.name === s);
    if (use64) {
      return BigInt(entry.value);
    }
    return Number(entry.value);
  }
  return use64 ? BigInt(token) : Number(token);
}
 ...
 */
function assembleBitmask(enumMeta, tokens, use64 = false, _fieldName = 'bitmask') {
  let mask = use64 ? BigInt(0) : 0;
  for (const token of tokens) {
    const value = resolveToken(enumMeta, token, use64);
...
function normalizeMask(mask, use64) {
  return use64 ? BigInt(mask) : Number(mask);
}
