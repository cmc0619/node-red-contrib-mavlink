...
function resolveToken(enumMeta, token, fieldName, use64) {
  if (typeof token === 'bigint') return use64 ? token : Number(token);
  if (typeof token === 'number') return use64 ? BigInt(token) : token;
  if (typeof token === 'string') {
    const s = token.trim();
    if (/^[+-]?\d+$/.test(s)) return use64 ? BigInt(s) : Number(s);
    const entry = enumMeta && enumMeta.entries.find((e) => e.name === s);
    if (!entry) fail(fieldName, `unknown bitmask entry ${JSON.stringify(token)}`);
    return use64 ? BigInt(entry.value) : Number(entry.value);
  }
