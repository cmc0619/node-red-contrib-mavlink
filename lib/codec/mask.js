...
function bitsOf(value, use64) {
  ...
  if (use64) {
    ...
    while (remaining > BigInt(0)) {
      ...
      remaining = remaining / BigInt(TWO);
      bit += 1;
    }
    return bits;
  }
...
function resolveToken(enumMeta, token, fieldName, use64) {
  if (typeof token === 'bigint') {
    ...
    return use64 ? token : Number(token);
  }
  if (typeof token === 'number') {
    // Infinity would make bitsOf() loop forever (remaining never shrinks);
    // negatives/fractionals would silently collapse to the wrong mask.
    if (!Number.isFinite(token)) fail(fieldName, `bitmask token ${token} is not a finite number`);
    if (!Number.isInteger(token)) fail(fieldName, `bitmask token ${token} is not an integer`);
    if (token < 0) fail(fieldName, `bitmask token ${token} is negative`);
    return use64 ? BigInt(token) : token;
  }
  if (typeof token === 'string') {
    const s = token.trim();
    if (/^\d+$/.test(s)) {
      const n = use64 ? BigInt(s) : Number(s);
      return n;
    }
    ...
    if (use64 ? entryValue < 0n : entryValue < 0) {
      fail(fieldName, `bitmask entry ${entry.name} has a negative value`);
    }
    return entryValue;
  }
...
function normalizeMask(mask, use64) {
  if (use64) return typeof mask === 'bigint' ? mask : BigInt(mask);
  return typeof mask === 'bigint' ? Number(mask) : mask;
}

module.exports = { assembleBitmask, decodeBitmask, testBit, setBit, bitsOf };
