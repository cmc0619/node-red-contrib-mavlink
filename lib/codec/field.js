...
/**
 * Convert a JS degrees value to a degE7 integer: × 10⁷, rounded.
 *
 * A `Number` or `string` is always degrees. There is deliberately no
 * "looks already scaled" pass-through: whole-degree coordinates are ordinary
 * input (`lat: -35`), so treating an integer as a wire value silently commanded
 * −0.0000035° instead of −35°. A `bigint` is still taken as a wire value —
 * that is a property of the type (it cannot express 47.397742), not a guess
 * about the number.
 *
...
function degreesToDegE7(fieldName, value) {
  ...
  fail(fieldName, `expected degrees or degE7 integer, got ${typeof value}`);
}
