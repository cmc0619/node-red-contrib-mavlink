...
function degreesToDegE7(fieldName, value) {
  ...
  if (!Number.isFinite(n)) fail(fieldName, `degE7 value ${JSON.stringify(value)} is not a finite number of degrees`);
  if (Math.abs(n) > 180) {
    // The ±180° bound rejects, exactly as ArduPilot's check_latlng does on
    // decode — it never switches interpretation. The usual way to land here
    // is passing a raw degE7 wire integer as degrees; say so.
    fail(fieldName, `${n} is outside the ±180° coordinate range — a raw degE7 wire value must be divided by 1e7 first`);
  }
...
function degE7Sentinel(field) {
  return field && field.invalid === 'INT32_MAX' ? 2147483647 : null;
}
