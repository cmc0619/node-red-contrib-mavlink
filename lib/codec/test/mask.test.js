...
const { assembleBitmask, decodeBitmask, encodeField, decodeField, FieldCodecError } = require('..');
...
test('encodeField resolves a scalar enum entry name to its value', () => {
  ...
  // Non-bitmask enums must not accept an array of names (would silently OR).
  assert.throws(() => encodeField(f, ['MAV_TYPE_GENERIC', 'MAV_TYPE_QUADROTOR'], mavType), FieldCodecError);
});

test('bitmask tokens that are negative, fractional, or non-finite are rejected', () => {
  expectFieldError(() => assembleBitmask(modeFlags, [-1]), FieldCodecError, 'bitmask');
  expectFieldError(() => assembleBitmask(modeFlags, ['-1']), FieldCodecError, 'bitmask');
  expectFieldError(() => assembleBitmask(modeFlags, [1.5]), FieldCodecError, 'bitmask');
  expectFieldError(() => assembleBitmask(modeFlags, [Infinity]), FieldCodecError, 'bitmask');
  expectFieldError(() => assembleBitmask(modeFlags, [9007199254740993n]), FieldCodecError, 'bitmask');
  expectFieldError(() => assembleBitmask(modeFlags, ['9007199254740993']), FieldCodecError, 'bitmask');
  expectFieldError(() => decodeBitmask(modeFlags, Infinity), FieldCodecError, 'bitmask');
  expectFieldError(() => decodeBitmask(modeFlags, -1), FieldCodecError, 'bitmask');
  expectFieldError(() => decodeBitmask(modeFlags, 1.5), FieldCodecError, 'bitmask');
});
