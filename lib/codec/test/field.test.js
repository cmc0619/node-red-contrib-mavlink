...
const { field, expectFieldError, writePayload } = require('./helpers');
...
test('an oversized BigInt on a ≤32-bit field fails loud at the Buffer write (§14)', () => {
  const f = field({ name: 'custom_mode', type: 'uint32_t' });
  // A BigInt within range converts and encodes like any integer.
  assert.equal(encodeField(f, 42n), 42);
  // Type-width fit is Buffer's job: any BigInt beyond ±2^53 is far beyond
  // every ≤32-bit width, so the write rejects it — never a silent wrap.
  const wire = encodeField(f, 2n ** 64n);
  assert.throws(
    () => writePayload([f], { custom_mode: wire }),
    { name: 'RangeError', code: 'ERR_OUT_OF_RANGE', message: /It must be >= 0 and <= 4294967295/ }
  );
});
...
test('degE7 fields are raw int32 wire values — no unit conversion either direction (§9)', () => {
  ...
  // typed operator surfaces (command/fanout/payload/mission builders).
