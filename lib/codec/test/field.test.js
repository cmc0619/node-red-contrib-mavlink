...
test('degE7 encode is value-blind: whole-number degrees scale like any others', () => {
  ...
  assert.throws(() => encodeField(f, 473977420), /divided by 1e7/);
});

test('an out-of-range degE7 sentinel survives decode → re-encode', () => {
  // CAMERA_FOV_STATUS.lat_image carries INT32_MIN for "at infinity, not
  // intersecting the horizon" — a second sentinel the `invalid` attribute
  // (INT32_MAX) does not cover. Forwarding a decoded frame back through the
  // codec must reproduce it exactly. A semantic ±180° bound would reject the
  // −214.7483648° that INT32_MIN decodes to; the wire-width check does not.
  const f = field({ name: 'lat_image', type: 'int32_t', units: 'degE7', invalid: 'INT32_MAX' });
  const INT32_MIN = -2147483648;
  const decoded = decodeField(f, INT32_MIN);
  assert.equal(decoded, -214.7483648);
  assert.equal(encodeField(f, decoded), INT32_MIN);

  // A coordinate that is invalid but encodable still encodes — the vehicle
  // rejects it (ArduPilot's check_latlng), the codec does not second-guess it.
  assert.equal(encodeField(f, 200), 2000000000);
});

test('degE7 rejects non-finite input as a field error that names the value', () => {
  const f = field({ name: 'lat', type: 'int32_t', units: 'degE7' });
  for (const bad of [NaN, Infinity, -Infinity, 'NaN', 10n ** 40n]) {
    // The failure must be the codec's own error naming the field — an oversized
    // BigInt renders as Infinity, and building the message must not throw a
    // TypeError of its own on the way out.
    assert.throws(() => encodeField(f, bad), FieldCodecError);
  }
  assert.throws(() => encodeField(f, NaN), /degE7 value NaN/);
  assert.throws(() => encodeField(f, Infinity), /degE7 value Infinity/);
});
