 ...
 *
 * Accepts a finite integer `Number`, or a decimal integer `string`
 * (`/^[+-]?\d+$/`, whitespace trimmed). Rejects `undefined`, `null`, `''`,
 * whitespace-only and non-numeric strings, `NaN`/`Infinity`, non-integers, and
 * any non-number/non-string type. Type-width overflow is left to `Buffer`
 * (DESIGN.md §14); a declared `minValue`/`maxValue` narrower than the type is
 * enforced here — report, never wrap.
 *
...
function toInteger(fieldName, value, bounds) {
  ...
  if (typeof value === 'number') {
    ...
    if (value > BigInt(Number.MAX_SAFE_INTEGER) || value < BigInt(Number.MIN_SAFE_INTEGER)) {
      fail(fieldName, 'BigInt exceeds safe range for a ≤32-bit field');
    }
    n = Number(value);
  } else {
