 ...
 * each component gains a `modeName` beside the raw number — the ladder's
 * answer, undefined (serialising away) when it has none. The live
...
function snapshotPeers(peerTable, filter = {}, modes) {
  ...
  const sysid = filter.sysid === undefined ? null : Number(filter.sysid);
  const compid = filter.compid === undefined ? null : Number(filter.compid);
  ...
  for (const peer of peers) {
    for (const component of peer.components) {
      if (component.autopilotVersion) {
        const packed = component.autopilotVersion.flightSwVersion;
        component.autopilotVersion = {
          ...
          softwareVersion: {
            ...
            patch: (packed >>> 8) & 0xff,
            releaseType: packed & 0xff,
          },
          flightCustomVersion: Buffer.from(component.autopilotVersion.flightCustomVersion).toString('hex'),
        };
      }
      if (modes) {
        component.modeName = modeNameFor(component.flightMode, {
          ...modes,
 ...
 * @param {{events: string[], modes?: object}} options  `events` is the
 *   editor's saved pick, subscribed as given
...
function createStateFeed(peerTable, options, emit) {
  ...
  for (const event of events) {
    const listener = (payload) => emit(recordFor(peerTable, event, payload, names(event, payload)));
...
/**
 * @param {import('../connection/peer-table').PeerTable} peerTable  projects
 *   the record's component on its own clock
 * @param {string} event
 * @param {object} payload
 ...
 * @returns {object}
 */
function recordFor(peerTable, event, payload, names) {
  ...
  return {
    ...
    // The table's projection, never the live component: a later heartbeat
    // rewrites the live one in place, and its endpoints Map is the return
    // route (§14).
    component: payload.component ? peerTable.projectComponent(payload.component) : undefined,
    from: payload.from,
    to: payload.to,
