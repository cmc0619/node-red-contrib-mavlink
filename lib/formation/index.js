...
function slotOffsets(shape, count, spacing) {
  ...
  // Zero spacing stacks every follower on one point — a commanded collision,
  // not a formation. Negative would silently mirror the pattern.
...
function bodyToNed(offset, headingDeg, pitchDeg = 0) {
  const p = (finite(pitchDeg, 'pitch') * Math.PI) / 180;
