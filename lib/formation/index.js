...
function slotOffsets(shape, count, spacing) {
  ...
  // and every use below is arithmetic, which reads the saved value as is.
  const out = [];
  switch (shape) {
    case 'line':
      // Abreast, centered on the reference: 0, +1, -1, +2, -2, ... spacings
      for (let i = 0; i < count; i += 1) {
        const rank = Math.ceil(i / 2);
        const side = i % 2 === 1 ? 1 : -1;
        out.push({ forward: 0, right: i === 0 ? 0 : side * rank * spacing, down: 0 });
      }
      break;
    case 'column':
      // Single file trailing directly behind the reference.
      for (let i = 0; i < count; i += 1) {
        out.push({ forward: -i * spacing, right: 0, down: 0 });
      }
      break;
    case 'wedge':
      ...
      for (let i = 0; i < count; i += 1) {
        const rank = Math.ceil(i / 2);
        const side = i % 2 === 1 ? 1 : -1;
        out.push({ forward: i === 0 ? 0 : -rank * spacing, right: i === 0 ? 0 : side * rank * spacing, down: 0 });
      }
      break;
    case 'circle':
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        out.push({ forward: spacing * Math.cos(theta), right: spacing * Math.sin(theta), down: 0 });
      }
      break;
    case 'sphere': {
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        const idx = i - 1;
        // y from +1 (up) to −1 (down) across the follower set — exactly ±1
        // at the ends, so the radius there is sqrt(0).
        const y = ring === 1 ? 0 : 1 - (idx / (ring - 1)) * 2;
        const radiusAtY = Math.sqrt(1 - y * y);
        const theta = golden * idx;
        out.push({
          forward: spacing * Math.cos(theta) * radiusAtY,
          right: spacing * Math.sin(theta) * radiusAtY,
          down: -spacing * y,
        });
      }
      break;
    ...
    case 'grid': {
      ...
      // never shifted; `cols` is only read for followers, where count − 1 >= 1.
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        out.push({ forward: -row * spacing, right: (col - (cols - 1) / 2) * spacing, down: 0 });
      }
      break;
    }
 ...
 */
function bodyToNed(offset, headingDeg, pitchDeg) {
 ...
 * slot positions, and Fan-out's per-member metre offsets reuse it
 ...
 * @param {number} opts.pitchDeg      pitches the whole pattern (around body Y /
 *   right); the formation node resolves it from payload or config
