...
class LogTransfer {
  ...
  start() {
    return new Promise((resolve) => {
      ...
      // Source narrowing is the subscription filter's (sysid/compid below,
      // 0 = wildcard); the handler sees only frames that already match.
  ...
  _onTimeout() {
    ...
    if (this._stepRetries < this._maxRetries) {
      ...
      if (this._settled) return;
      // A retry with no next request to compute is the end of what this
      // machine can do. Returning here would leave the promise pending with
      // no timer armed and the caller's lock held, which is the silence §0
      // rule 3 forbids, so it settles as the stall it is.
      if (next) {
