...
class LogTransfer {
  ...
  _onTimeout() {
    ...
    if (this._stepRetries < this._maxRetries) {
      ...
      this._stepMessage = next.message;
      this._arm();
  ...
  /** @returns {{label:string,message:object}|null} */
  _retryStep() {
    return { label: this._stepLabel, message: this._stepMessage };
  }
