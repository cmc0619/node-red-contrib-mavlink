...
function matchesParamEcho(request, decoded) {
  ...
  // is nothing to compare: decline the match and let the confirm report an
  // honest echo timeout, rather than misread an integer echo as a guessed
  // REAL32. Reachable only when a malformed frame omits param_type on a
  // request that carries none — reads have no type, and sets fail loud
  // without one (#222). Kept because param_type arrives off an untrusted wire.
