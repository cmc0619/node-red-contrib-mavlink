...
const { PARAM_TYPES, paramValueFromWire, paramValueToWire } = require('../codec/param-union');
...
function buildParamMessage(input) {
  ...
  switch (input.action) {
    ...
    case 'read': {
      const paramIndex = Number(input.paramIndex);
      return {
    ...
    case 'set': {
      ...
      const encoding = resolveParamEncoding(input);
      return {
        name: 'PARAM_SET',
        fields: {
          target_system: target.sysid,
...
function matchesParamEcho(request, decoded) {
  ...
  // were encoded per *that* type — so that is the type the echo is decoded
  // with. The request's type governs how the outbound PARAM_SET is *encoded*;
  // the vehicle is the authority on its own parameter's type when reading the
  // reply: an int parameter set through a REAL32-typed request would decode
  // bytewise as a denormal (1 arrives as 1.4e-45) and never match, so a set
  // the vehicle actually applied would report "echo timeout".
  const type = resolveParamType(fields.param_type);
  const encoding = resolveParamEncoding(request);
...
function echoTargetMatches(target, decoded) {
  if (Number(decoded.sysid) !== Number(target.sysid)) return false;
  if (Number(target.compid) !== 0 && Number(decoded.compid) !== Number(target.compid)) return false;
  return true;
}
...
function matchesParamEchoWire(sent, target, decoded, encoding) {
  ...
  // A bytewise non-REAL32 slot carries an integer's raw bit pattern, not a
  // genuine float (mavlink-audit-20260905 #5). Comparing that pattern through
  // Object.is/Math.fround treats every one of the ~2^24 int32 patterns that
  // land in the float32 NaN band (exponent all-ones) as equal to every other
  // — Object.is defines all NaNs as the same value — so a vehicle that echoed
  // back a genuinely different integer in that band would still read as
  // "confirmed". matchesParamEcho above avoids exactly this by decoding
  // through paramValueFromWire before comparing; do the same here.
  //
  // Known residual, shared with matchesParamEcho: both sides arrive here as
  // JS Numbers, and float32→double conversion quiets a signalling NaN (sets
  // bit 22), so an int32 whose pattern is an sNaN and its qNaN twin
  // (0x7F800001 vs 0x7FC00001) decode to the same integer and still compare
  // equal. paramValueToWire makes the same conversion when it serialises such
  // a value, so the driver cannot put an sNaN-pattern integer on the wire
  // faithfully either. Closing it means carrying the raw 32-bit slot through
  // serialise/deserialise instead of a Number — a codec-contract change, not
  // a matcher fix (CodeRabbit on #459).
  if (
    encoding === PARAM_ENCODING.BYTEWISE &&
    Number(sentFields.param_type) !== PARAM_TYPE.MAV_PARAM_TYPE_REAL32
  ) {
    return (
      paramValueFromWire(Number(echo.param_value), sentFields.param_type)
      === paramValueFromWire(Number(sentFields.param_value), sentFields.param_type)
    );
  }
...
/**
 * Match a PARAM_VALUE reply to a confirm-tier PARAM_REQUEST_READ by whichever
 * field the request put on the wire — the index when one was sent (>= 0), the
 * name otherwise (buildParamMessage sends an empty param_id on an index read,
 * so the id cannot identify that reply). No value compare: the reply *is* the
 * value.
 *
 * @param {object} request
 * @param {object} decoded
...
function matchesParamReadReply(request, decoded) {
  const fields = decoded.fields;
  const index = Number(request.paramIndex);
 ...
 *
 * The component that answers first is pinned beside the count, and frames
 * from any other component are ignored: a list request to component 0 draws
 * one table per component, keyed by the same indices, and index 0 from the
 * autopilot plus index 1 from a camera is neither table. The collect is the
 * first responder's table, as a component-0 command completes on the
 * component that replied (§14.144).
 *
 * `accept` reports what it did with the frame, because a caller watching the
 * stream for life must tell a kept frame from an ignored one: the ordered
 ...
 *   deduped per index so a re-delivered frame warns once
 * @returns {{accept: (decoded: object) => (object[]|true|null), size: number}}
 */
function createParamListCollector(options = {}) {
  ...
  return {
    ...
    accept(decoded) {
      ...
      if (source === null) source = decoded.compid;
      else if (decoded.compid !== source) return null;
...
function resolveParamType(paramType) {
  if (/^\d+$/.test(paramType)) return Number(paramType);
  return PARAM_TYPE[paramType];
}

/**
 * Strip the wire's NUL padding: `param_id` is a 16-byte char array.
 *
 * @param {*} value
 * @returns {string}
 */
...
/**
 * Whether an echoed parameter value is the value that was sent, at the
 * precision the wire carried (§14.80). An exact wire (bytewise integer) keeps
 * the vehicle's bits, so any difference is a different stored value — past
 * 2^24 `Math.fround` would collide consecutive integers and confirm one.
 * Otherwise the value crossed as a float32, so the echo is the float32
 * quantization of what was sent (47.9 returns as 47.900001525878906) and the
 * two compare as float32. There is no absolute epsilon: two distinct float32
 * values (0 and 5e-7) are two stored values, not one.
 *
 * @param {number} a
 * @param {number} b
...
/**
 * Read the peer's parameter-encoding capabilities for an addressed component.
 *
