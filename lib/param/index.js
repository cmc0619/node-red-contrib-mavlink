...
function matchesParamEcho(request, decoded) {
  ...
  // reported honestly as an echo timeout. (A real vehicle always populates
  // param_type; this is the malformed-frame path.)
...
function numericEqual(a, b, options = {}) {
  ...
  // An exact wire (bytewise integer) preserves the vehicle's bits verbatim, so
  // there is no quantization to forgive: both sides are integers here — the
  // codec rejects a non-integer value for an integer type at encode time, so a
  // set that reached the wire had one — and any difference is a different
  // stored value. Granting tolerance would confirm a value the vehicle never
  // held, since past 2^24 consecutive integers collide under Math.fround.
