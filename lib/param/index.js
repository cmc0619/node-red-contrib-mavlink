...
function matchesParamEcho(request, decoded) {
  ...
  const type = resolveParamType(
    isKnownParamType(fields.param_type)
      ? fields.param_type
      : request.paramType || 'MAV_PARAM_TYPE_REAL32'
  );
  const encoding = resolveParamEncoding({
...
function numericEqual(a, b, options = {}) {
  ...
  // anything past the tolerance above is a genuinely different stored value.
  if (options.exactWire) return false;
