...
/**
 * Construct a parameter bundle machine. Backup and restore are separate
 * machines rather than one class reading an operation field, so nothing
 * downstream re-reads the verb to remember which exchange it is running.
 *
 * @param {string} operation
 * @param {object} opts
 * @returns {ParamBackup|ParamRestore|undefined}
 */
function createMachine(operation, opts) {
  switch (operation) {
    case OPERATION.BACKUP:
      return new ParamBackup(opts);
    case OPERATION.RESTORE:
      return new ParamRestore(opts);
...
module.exports = { createMachine, ParamBackup, ParamRestore, locks, OPERATION, jsonSafeValue };
