...
/**
 * Load and cache the seed blob. The seed ships in the package
 * (`package.json#files`), so a read that fails craters, naming the file.
 *
 * @returns {object}
 */
function load() {
  ...
  const blob = path.join(SEED_DIR, path.basename(active.file));
 ...
 * @returns {Map<string, object>} empty when the firmware is unknown — never
 *   a partial guess at a different firmware's parameters
 */
function defsFor(opts = {}) {
  const firmware = opts.firmware;
  const result = new Map();
  const entry = load().firmwares[firmware];
  const vehicles = entry?.vehicles;
  if (!vehicles) return result;

  const family = opts.vehicleFamily;
...
function catalogLabel(opts) {
  ...
  const parts = [FIRMWARE_LABEL[firmware]];
  // Name the ArduPilot vehicle only when defsFor serves that vehicle's own
  // document (the same ARDUPILOT_VEHICLE map defsFor decides named-vs-union
  // with), so the label never disagrees with what is served. A blank, unknown,
  // or unrecognised family gets the union catalog and goes unnamed here; a bad
  // config shows as the node's red ring, not as a label to narrate.
  if (firmware === 'ardupilot') {
    const family = opts.vehicleFamily;
    if (ARDUPILOT_VEHICLE[family]) parts.push(VEHICLE_FAMILY_LABEL[family]);
  }
  const origin = opts.source === 'profile'
    ? 'downloaded for this profile'
    : 'shipped seed';
  return `${parts.join(' ')} \u00b7 ${count} definition${count === 1 ? '' : 's'} (${origin})`;
}
