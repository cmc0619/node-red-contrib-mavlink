...
const {
  FIRMWARE_LABEL, VEHICLE_FAMILY_LABEL,
} = require('../vehicle');
 ...
 *
 * `bits` acts too: the editor renders switches from it and folds masks, so a
 * wrong document's bit labels set the wrong flags with the same confidence as
 * a wrong enumeration naming the wrong mode.
 *
...
function unionSafe(def) {
  const { min, max, increment, values, type, bits, ...rest } = def;
  return rest;
}
...
/**
 * Name the definition set that was actually served, for the operator who is
 * looking at a parameter and wondering why it knows nothing about it.
 *
 * The dialog cannot compose this itself and be trusted: it sends the firmware
 * it *thinks* applies, but the route falls back to the deployed Vehicle
 * Profile when the query omits one, so the two can disagree. The answer has to
 * come from whoever resolved it. A Connection named "PX4" bound to a profile
 * set to ArduPilot is exactly the case that costs an afternoon, and it looks
 * from the outside like the Type field refusing to narrow.
 *
 * The family is named only for ArduPilot, because only ArduPilot has a
 * document per vehicle -- {@link defsFor} ignores the family for everything
 * else, and printing it would imply a distinction that was never made.
 *
 * @param {object} opts
 * @param {string} opts.firmware
 * @param {string} [opts.vehicleFamily]
 * @param {number} opts.count
 * @param {'seed'|'profile'} [opts.source]
 * @returns {string}
 */
function catalogLabel(opts) {
  const firmware = String((opts && opts.firmware) || '');
  const count = Number((opts && opts.count) || 0);
  const parts = [FIRMWARE_LABEL[firmware] || firmware || 'Unknown firmware'];
  // An unnamed ArduPilot vehicle is served the union of every document's
  // names, so it is the one case where the *shape* of the answer differs and
  // saying so beats naming the family: it is why the Value field offers no
  // bounds and no choice list, which otherwise reads as a second fault.
  let unit = 'definition';
  if (firmware === 'ardupilot') {
    // Blank (the query omitted the family and no deployed profile filled it) and
    // an explicit 'unknown' pick both mean no ArduPilot vehicle document — the
    // union-of-names catalog. A present specific family is trusted and named
    // as-is; the editor's select is the protector, so a bad token is not ours to
    // coerce into 'unknown' (§5: no blank check that selects a default member).
    const family = String((opts && opts.vehicleFamily) || '').trim();
    if (!family || family === 'unknown') {
      parts.push('(no vehicle named)');
      unit = 'name';
    } else {
      parts.push(VEHICLE_FAMILY_LABEL[family] || family);
    }
  }
  const origin = opts && opts.source === 'profile'
    ? 'downloaded for this profile'
    : 'shipped seed';
  return `${parts.join(' ')} \u00b7 ${count} ${unit}${count === 1 ? '' : 's'} (${origin})`;
}
