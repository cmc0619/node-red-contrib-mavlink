 ...
 * 2026-08-05). ArduPilot states none — `apm.pdef.json` and `apm.pdef.xml` publish only Description,
...
function xmlParamEntry(node, groupName) {
  ...
  const rawValues = (node.values?.value) || [];
  ...
  const rawBits = (node.bitmask?.bit) || [];
 ...
 * Elements are snake_case where the JSON is camelCase, values
