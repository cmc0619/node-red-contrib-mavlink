...
/**
 * Host suffixes permitted for operator-supplied param-defs URLs. Keeps the
 * admin fetch on known public firmware/metadata hosts rather than arbitrary
 * HTTPS destinations (SSRF). Exact match or subdomain of each suffix.
 */
const ALLOWED_DEFS_HOST_SUFFIXES = [
...
function assertSafeDefsUrl(urlString) {
  ...
  // Hostnames must be on the firmware/metadata allowlist — IP literals never are.
  if (!isAllowedDefsHost(host)) {
    const err = new Error(
      `parameter defs URL host is not on the allowlist (${ALLOWED_DEFS_HOST_SUFFIXES.join(', ')})`
    );
    err.code = 'PARAM_DEFS_URL_FORBIDDEN';
    throw err;
  }
 ...
 * @returns {boolean}
 */
function isAllowedDefsHost(host) {
  if (!host || isBlockedIpLiteral(host)) return false;
  const h = host.toLowerCase();
  return ALLOWED_DEFS_HOST_SUFFIXES.some((suffix) => h === suffix || h.endsWith(`.${suffix}`));
}
...
function isBlockedIpLiteral(host) {
  ...
  // IPv4-mapped / IPv4-compatible IPv6 — classify the embedded v4.
  ...
  if (bare.includes(':')) {
    ...
    // IPv4-compatible compressed forms Node may normalize to (::7f00:1).
    const compat = ipv4CompatibleFromHextets(lower);
    if (compat) return isBlockedIpv4(compat);
  }
  return false;
}
...
/**
 * Deprecated IPv4-compatible IPv6 (::a.b.c.d / ::7f00:1).
 *
 * @param {string} lower
 * @returns {string|null}
 */
function ipv4CompatibleFromHextets(lower) {
  const dotted = lower.match(/^::(\d{1,3}(?:\.\d{1,3}){3})$/);
  if (dotted) return dotted[1];
  const hextets = lower.match(/^::([0-9a-f]{1,4}):([0-9a-f]{1,4})$/);
  if (!hextets) return null;
  const hi = parseInt(hextets[1], 16);
  const lo = parseInt(hextets[2], 16);
  if (!Number.isFinite(hi) || !Number.isFinite(lo)) return null;
  if (hi === 0 && lo === 1) return '127.0.0.1'; // ::1 already handled; keep consistent
  return `${(hi >> 8) & 255}.${hi & 255}.${(lo >> 8) & 255}.${lo & 255}`;
}
