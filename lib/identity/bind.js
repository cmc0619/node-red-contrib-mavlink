...
/**
 * Merge a vehicle sysid into a resolved identity value object.
 * A no-op for non-companion identities (derivesSysidFromVehicle false).
 *
 * @param {{sysid: number|null, compid: number, derivesSysidFromVehicle: boolean}} identity
 * @param {number} vehicleSysid
 * @returns {{sysid: number, compid: number}}
 */
function applyVehicleSysid(identity, vehicleSysid) {
  if (!identity.derivesSysidFromVehicle) return { sysid: identity.sysid, compid: identity.compid };
  return { sysid: vehicleSysid, compid: identity.compid };
}

module.exports = { bindVehicleSysid, releaseVehicleSysid, applyVehicleSysid };
