...
/**
 * The preset for a role. `role` is editor-guaranteed — a `required` select of
 * exactly the ROLE_PRESETS keys (mavlink-local-identity.html) — so this is a
 * bare lookup that trusts the saved member (§6). A hand-edited non-member
 * returns `undefined` and craters at the consumer that reads the preset, at
 * construction (this node has no inputs; deploy is its only door).
 *
...
module.exports = { ROLE_PRESETS, rolePreset };
