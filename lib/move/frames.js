 ...
 * time origin. Monotonic on purpose: `Date.now()` steps with NTP
 * adjustments, and a clock that can run backward breaks the jitter
 * estimation consumers hang on this field (Codacy, #292). A build with no
 * explicit `timeBootMs` stamps the clock instead of 0, and stream ticks and
 * the synthesized brake packet stamp fresh at send time: a stamp frozen at
 * build would claim every tick of a 4 Hz stream left at the same instant.
 * The `>>> 0` wrap after ~49.7 days is the field's own uint32 wrap.
 *
 * This is the one blank arm that is not a substitution: `time_boot_ms` is the
 * sender's own stamp, never operator intent, so there is no value the caller
 * failed to give us.
 */
const bootNow = () => Math.floor(performance.now()) >>> 0;

/**
 * The one number conversion Move owns, and it asks exactly one question: did
 * you give me anything? A blank is a value nobody supplied, and picking one
 * here is the silent substitution the no-defaults ruling exists to stop (owner,
 * 2026-08-14). Callers whose field has a *dialect-defined* no-change encoding —
 * DO_CHANGE_SPEED's −1, CONDITION_YAW's 0 — spell that sentinel at their own
 * call site, where it reads as the wire's word for "unset" and not as our guess.
 *
 * **It does not check the number is finite, deliberately** (owner ruling,
 * 2026-08-14). That check was inherited from `numberOr` and was a guardrail —
 * the thing §2 gives to the editor, not the driver. It was also either
 * forbidden or redundant, never useful:
 *
 * - On a **float** field NaN is legal MAVLink and the wire carries it (measured:
 *   `param1: NaN` serializes and decodes back NaN). Refusing it here refuses a
 *   value the vehicle would have received and judged for itself.
 * - On an **integer** field `wire.js` already refuses non-finite at the one
 *   choke point every outbound message crosses, and says more than this could:
 *   it names the field and the broadcast hazard behind it.
 *
 * So garbage rides out as NaN on floats. That is not an oversight — it is the
 * driver coercing and sending, with the dialog validating and the vehicle
 * judging, exactly as everywhere else.
 *
 * `label` names the field in the refusal, because one build calls this for
 * roll, pitch, yaw, rates, thrust, speed, throttle, heading and the coordinate
 * triplets — an unlabelled refusal leaves the operator hunting (Gitar, #303).
 *
 * @param {*} value
 * @param {string} label  the field name, as the operator's dialog spells it
 * @returns {number}
 */
function requireNumber(value, label) {
  if (isBlank(value)) {
    throw new Error(`Move needs ${label} — nothing supplied`);
  }
  return Number(value);
}
 ...
 * altitude sign flip: exactly once, at encode. NaN passes through, because NaN
 * floats are legal MAVLink.
 *
 * It used to answer 0 for `undefined`, for the mask-ignored fields whose bytes
 * still have to hold something. That arm is gone (owner ruling, 2026-08-14):
 * the 0 is legitimate *only* because a type_mask ignore bit says nobody reads
 * it, and this function cannot see that bit. Hidden here, the coupling was
 * convention — a new call site passing a blank without setting the bit would
 * have got a commanded yaw of 0, which is north, silently. The filler is spelled
 * at the call sites now, beside the bit that makes it legal.
 *
 * @param {*} value  degrees
