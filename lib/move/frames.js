...
/**
 * The one number conversion Move owns. It has no fallback arm: a blank is a
 * value nobody supplied, and picking one here is the silent substitution the
 * no-defaults ruling exists to stop (owner, 2026-08-14). Callers whose field
 * has a *dialect-defined* no-change encoding — DO_CHANGE_SPEED's −1,
 * CONDITION_YAW's 0 — spell that sentinel at their own call site, where it
 * reads as the wire's word for "unset" and not as our guess.
 *
 ...
 * triplets — an unlabelled "expected a finite number" leaves the operator
 * hunting (Gitar, #303).
 *
 * @param {*} value
...
function requireNumber(value, label) {
  ...
  if (!Number.isFinite(n)) {
    throw new Error(`Move ${label} expected a finite number, got ${JSON.stringify(value)}`);
  }
  return n;
}
