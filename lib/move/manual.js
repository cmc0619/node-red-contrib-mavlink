 ...
 * ArduSub is flown, and the only motion primitive Blimp has (§9 roster). Three
 * things make it unlike every other Move action:
 *
...
function buildManualMessage(input) {
  return {
    ...
    fields: {
      ...
      buttons: isBlank(input.buttons) ? 0 : numberOrThrow(input.buttons, 'buttons'),
    },
  };
}
...
function axis(value) {
  ...
  return Math.round(numberOrThrow(value, 'axis') * AXIS_SCALE);
}
...
function numberOrThrow(value, name) {
  ...
  if (!Number.isFinite(n)) {
    throw new Error(`Move manual ${name} expected a finite number, got ${JSON.stringify(value)}`);
  }
  return n;
}
