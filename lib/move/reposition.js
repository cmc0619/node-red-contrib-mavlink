...
function buildRepositionMessage(input) {
  ...
  return buildCommandInt(
    ...
    target.sysid,
    target.compid,
    // The only coordinate handling left, and it is not a guard: a blank
    // encodes 0 — null island, accepted with the global guard's removal
    // (§14) — while an explicit NaN throws here rather than surfacing as the
