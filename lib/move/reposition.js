...
/** Carrier names Move config and `msg.payload.carrier` speak. */
const MOVE_CARRIERS = ['setpoint', 'reposition'];
...
/**
 * Resolve the Move carrier name. Blank inherits the setpoint default — the
 * safe direction for a flow saved before the field existed (#239); an unknown
 * carrier is runtime-boundary data and throws naming the valid set.
 *
 * @param {*} value
 * @returns {string}
 */
function resolveMoveCarrier(value) {
  if (isBlank(value)) return 'setpoint';
  if (!MOVE_CARRIERS.includes(value)) {
    throw new Error(
      `unknown Move carrier ${JSON.stringify(value)} — expected one of ${MOVE_CARRIERS.join(', ')}`
    );
  }
  return value;
}
