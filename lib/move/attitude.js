 ...
 * three things — no value, an ignore bit set, and 0 written into bytes that are
 * sent whether or not anyone reads them — and those used to be stated in three
 * separate places per field, four fields over. Filler with no bit is a
 * commanded value nobody typed, which is the silent-wrong-value shape the
 * no-defaults ruling exists to prevent; deriving both here makes that
 * unrepresentable rather than merely visible.
 *
 * The 0 is not a substitution: it is legal *because* the bit is set, and the
 * bit is set right here.
 *
 * @param {*} value
 * @param {string} label      names the field if it is present but unusable
 * @param {number} ignoreBit  this field's *_IGNORE bit
 * @param {function(number): number} [convert]  wire conversion, where there is one
 * @returns {{value: number, mask: number}}
 */
function optional(value, label, ignoreBit, convert) {
  if (isBlank(value)) return { value: 0, mask: ignoreBit };
  const n = requireNumber(value, label);
  return { value: convert ? convert(n) : n, mask: 0 };
}
...
function buildAttitudeMessage(input) {
  ...
  const rollRate = optional(input.rollRate, 'roll rate', MASK.BODY_ROLL_RATE_IGNORE, degreesToRadians);
  const pitchRate = optional(input.pitchRate, 'pitch rate', MASK.BODY_PITCH_RATE_IGNORE, degreesToRadians);
  const yawRate = optional(input.yawRate, 'yaw rate', MASK.BODY_YAW_RATE_IGNORE, degreesToRadians);
  // Thrust is already normalised 0..1 — no wire conversion, so no converter.
  const thrust = optional(input.thrust, 'thrust', MASK.THROTTLE_IGNORE);
