...
function createMoveStream(options) {
  ...
  const stop = () => {
    ...
    // stop() runs from teardown paths with no catch above it — the node's
    // close handler and the TTL timer tick. A synchronous send throw there
    // (disabled connection, queue overflow on a dead link) must not hang the
    // redeploy or crash the process; a link that can't carry the zero-velocity
    // stop setpoint has nothing to stop anyway.
    try {
      options.connection.send(buildStopMessage(options.message), sendOptions(options));
    } catch {
      /* link unusable — the stop setpoint has nowhere to go */
    }
  };
