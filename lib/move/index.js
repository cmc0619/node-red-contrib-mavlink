 ...
 * yaw-only). Frame still resolves through resolveModeAndFrame, which is also
...
function advisoryFor(input) {
  ...
  if (input.firmware === 'ardupilot' && mode === 'yaw-only') {
    // Measured 2026-08-08 (#179): Copter-4.7.0 GUIDED held yaw under a yaw-only
    // stream (type_mask 2559). Source: all-ignore pos/vel/accel → hold_position()
    // never reads yaw. Prefer velocity (or position) + yaw.
 ...
 * per second until `ttlMs` elapses, then send the zero-velocity stop packet.
