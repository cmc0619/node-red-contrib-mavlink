...
function resolveModeAndFrame(input) {
  ...
  // The removed spelling is refused, not ignored. Dropping it silently is
  // "permissive fallback that hides an incorrect property name" (AGENTS.md
  // "Failure behavior"), and the hidden result is geographic: a caller who
  // passes coordinateFrame: 'BODY_OFFSET_NED' with north/east/up would fall to
  // the LOCAL_NED default and fly an *absolute* move to the local origin
  // instead of an offset from where the vehicle is. Own-property check for the
  // same reason the mode guard uses one — `{}.coordinateFrame` is undefined,
  // but an inherited key must not arm this either.
  if (Object.prototype.hasOwnProperty.call(input, 'coordinateFrame')) {
    throw new Error('Move `coordinateFrame` was removed — use `frame`');
  }

  // Deliberately not `isBlank`: that treats whitespace as blank, which here
  // would mean a frame of ' ' silently defaults to LOCAL_NED. The node resolves
  // `payload.frame` against the configured frame *before* this point, and that
  // resolution does not trim (#174) — so a whitespace frame reaching here has
  // already skipped the configured value, and defaulting would discard it
  // exactly the way the blank-frame bug did. Throwing below is the loud option.
