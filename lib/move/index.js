...
function buildMoveMessage(input) {
  ...
  // Present values go through numberOr like every other numeric field below:
  // the mask — not NaN — is a setpoint's "don't care" channel, so junk that
  // coerces to NaN (`'abc'`, and an explicit NaN) would ride the wire with the
  // ignore bit *clear*, commanding a heading the vehicle cannot read. The
  // command path spells the same refusal (reposition.js); the fallback here is
  // unreachable because blank already returned undefined.
  const yaw = isBlank(input.yaw) ? undefined : numberOr(input.yaw, 0);
