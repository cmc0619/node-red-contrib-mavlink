...
function buildMoveMessage(input) {
  ...
  if (!global && uses.position && (isBlank(p.north) || isBlank(p.east) || isBlank(p.up))) {
    // Same §10 rule on the local plane: a blank north/east/up zero-filled into
    // a position setpoint commands the world origin (LOCAL_NED) or a hold at
    // the current spot (offset/body frames) the operator never asked for. An
    // explicit 0 is a value; blank refuses. Velocity and acceleration blanks
    // stay 0 — a zero rate is inert, not a place.
