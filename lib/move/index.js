...
// Degrees → wire degE7 scaling has one owner (§9 "Coordinate frames"):
// operator inputs are always decimal degrees — whole numbers included — and
// lib/command/carrier.js holds the rationale.
const { scaleLatLon } = require('../command/carrier');
...
const { MAV_FRAME, isGlobalFrame } = require('../command/carrier');
const {
  MODES,
  bootNow,
  degreesToRadians,
 ...
 * they are the byte-identical wire form.
 */
const PX4_COMPAT_FRAME = {
  [MAV_FRAME.GLOBAL]: MAV_FRAME.GLOBAL_INT,
  [MAV_FRAME.GLOBAL_RELATIVE_ALT]: MAV_FRAME.GLOBAL_RELATIVE_ALT_INT,
  [MAV_FRAME.GLOBAL_TERRAIN_ALT]: MAV_FRAME.GLOBAL_TERRAIN_ALT_INT,
};
  ...
  if (global) {
    return {
      ...
      fields: {
        ...
        // the spec-current number.
        coordinate_frame: PX4_COMPAT_FRAME[frame],
        lat_int: uses.position ? scaleLatLon(p.lat) : 0,
        lon_int: uses.position ? scaleLatLon(p.lon) : 0,
...
function createMoveStream(options) {
  ...
  // wins), which is correct for setpoints and invisible here.
  ...
  const sendSetpoint = () => {
    ...
    // On a message without the field (MANUAL_CONTROL) the stamp is inert: the
    // serializer copies only the names the message declares.
    const message = options.message;
    const fields = { ...message.fields, time_boot_ms: bootNow() };
  ...
  // setpoints do — the measured zero-velocity brake (§14). Attitude and
 ...
 * `SET_POSITION_TARGET_LOCAL_NED invalid`; we do not send that (§14).
 *
 * @param {object} message
 * @returns {{name:string, fields: object}}
 */
function buildStopMessage(message) {
  // Target ids are the streamed setpoint's own (DESIGN.md §14: unresolved
  // target beats wrong airframe).
...
const { positionFrom, velocityFrom, accelFrom } = require('./from-config');
...
const { buildAttitudeMessage } = require('./attitude');
