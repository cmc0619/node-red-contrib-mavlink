 ...
 * mask-ignored). The legacy mode names are aliases so saved flows and Fan-out
 * actions keep building unchanged; their frame default rides along.
 */
const MODES = {
...
const LEGACY_MODES = {
  'local-position': { mode: 'position', frame: MAV_FRAME.LOCAL_NED },
  'local-velocity': { mode: 'velocity', frame: MAV_FRAME.LOCAL_NED },
  'global-position': { mode: 'position', frame: MAV_FRAME.GLOBAL_RELATIVE_ALT_INT },
};
...
/**
 * Resolve the mode name and numeric frame, honouring the legacy mode aliases
 * (which carry their historical frame default) and both frame spellings the
 * API accepts: a MAV_FRAME member name in `input.frame`, or a raw number in
...
function resolveModeAndFrame(input) {
  const raw = input.mode || 'position';
  const legacy = LEGACY_MODES[raw];
  const mode = legacy ? legacy.mode : raw;
  if (!MODES[mode]) {
    throw new Error(
      `unknown Move mode ${JSON.stringify(raw)} — expected one of ${Object.keys(MODES).join(', ')}`
    );
  }
  ...
  if (frame === undefined || frame === null || frame === '') {
    frame = legacy ? legacy.frame : MAV_FRAME.LOCAL_NED;
