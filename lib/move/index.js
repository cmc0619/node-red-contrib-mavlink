 ...
 * "boot" for this sender is module load. A build with no explicit
 * `timeBootMs` stamps the clock instead of 0, and stream ticks and the
 * synthesized brake packet stamp fresh at send time: a stamp frozen at build
 * would claim every tick of a 4 Hz stream left at the same instant. The
 * `>>> 0` wrap after ~49.7 days is the field's own uint32 wrap.
 */
const BOOT_MS = Date.now();
const bootNow = () => (Date.now() - BOOT_MS) >>> 0;
