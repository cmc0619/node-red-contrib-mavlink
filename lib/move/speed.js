...
function buildSpeedMessage(input) {
  ...
  const speed = isBlank(input.speed) ? -1 : numberOrThrow(input.speed, 'speed');
  const throttle = isBlank(input.throttle) ? -1 : numberOrThrow(input.throttle, 'throttle');
...
function numberOrThrow(value, name) {
  ...
  if (!Number.isFinite(n)) {
    throw new Error(`Move speed ${name} expected a finite number, got ${JSON.stringify(value)}`);
  }
  return n;
}
