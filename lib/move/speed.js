 ...
 * *to* — which is the refusal arm of the coerce-vs-refuse test, the same
 * answer an unknown Move action gets when it matches no dispatch case.
 *
 * @param {object} input
...
function buildSpeedMessage(input) {
  ...
  // No blank default. speedType is a routing token, and DO_CHANGE_SPEED param1
  // has no dialect "unchanged" encoding the way param2 (speed) has −1 — so a
  // blank is operator intent we do not hold, not a sentinel we may spend. It
  // throws (enumValue craters on blank), the peer of frameForAltRef and
  // frameForReference, which the same 2026-08-14 ruling made throw (§14).
  const type = enumValue(SPEED_TYPE, input.speedType, 'speed type');
