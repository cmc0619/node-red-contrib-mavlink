 ...
 *
 * `speedType` reads straight off the SPEED_TYPE table. DO_CHANGE_SPEED param1
 * has no dialect "unchanged" encoding the way param2 (speed) has −1, so there
 * is no sentinel to spend on a token that is not a member — the editor's
 * select is what keeps one from being saved.
 *
 * @param {object} input
...
function buildSpeedMessage(input) {
  ...
  return buildCommandLong(
    ...
    target.sysid,
    target.compid,
    [SPEED_TYPE[input.speedType], speed, throttle, 0, 0, 0, 0],
    0
  );
}
