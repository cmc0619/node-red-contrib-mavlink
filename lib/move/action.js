...
function frameForAltRef(value) {
  // Blank is the GCS default, above home. An unrecognised token throws (owner
  // ruling, 2026-08-14: *"if a flow sends nonsense then it deserves to fail"*),
  // for the same reason the speed type does — and this one matters more. The
  // old coercion sent anything that was not `msl` to `home`, so a payload
  // override of `'MSL'` or `'amsl'` flew the whole goto at the wrong altitude
  // datum, silently, and home-vs-MSL is hundreds of metres apart.
  //
  // The comment this replaces defended coercion by arguing that returning
  // `undefined` would leave resolveModeAndFrame reading a blank frame and
  // defaulting to LOCAL_NED — a goto quietly becoming a local setpoint with
  // its lat/lon read as metres. That hazard is real and this still avoids it:
  // throwing is not returning undefined.
  if (isBlank(value)) return ALT_REF_FRAME.home;
  if (Object.prototype.hasOwnProperty.call(ALT_REF_FRAME, value)) return ALT_REF_FRAME[value];
  throw new Error(
    `unknown Move altitude reference ${JSON.stringify(value)} — expected one of `
    + `${Object.keys(ALT_REF_FRAME).join(', ')}`
  );
}
...
function frameForReference(value, firmware) {
  // Blank is world/LOCAL_NED, the frame that works on every stack. An
  // unrecognised token throws, like the altitude reference and the speed type
  // (owner ruling, 2026-08-14) — this used to coerce anything that was not
  // `body` or `offset` to world, so a payload override of `'BODY'` silently
  // flew a world-frame setpoint instead of the body-frame one it asked for.
  //
  // The firmware refusal below is a different animal and predates the ruling:
  // it is not about the operator's input at all — we asked the vehicle what it
  // is and got no answer, so there is no frame number to pick. Guessing wrong
  // is silently dropped by the vehicle, the failure mode with no symptom.
  if (isBlank(value) || value === 'world') return MAV_FRAME.LOCAL_NED;
  if (value === 'offset') return MAV_FRAME.LOCAL_OFFSET_NED;
  if (value !== 'body') {
    throw new Error(
      `unknown Move reference ${JSON.stringify(value)} — expected one of world, body, offset`
    );
  }
