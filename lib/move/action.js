...
function deriveSteerMode(groups) {
  ...
  // Acceleration wins a mix. MODES has no accel+position or accel+velocity
  // entry, so the other group is dropped — which used to refuse here. The
  // editor catches it now (mavlink-move.html `action`), where all three groups
  // are on screen together; a msg that fills both is trusted and takes the
  // drop, the flow author's bug to find.
