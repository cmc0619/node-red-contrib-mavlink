 ...
 *   IS the mode.
 *
 * A saved config without `action` throws (owner ruling, 2026-08-14, the
 * no-defaults ruling extended to selection): the editor always saves an
 * explicit action, so a blank one is a hand-edited or half-built config, not
 * a choice — see `resolveMoveAction` below.
 */
...
function resolveMoveAction(value) {
  // No blank default. The editor always saves an explicit action, so a blank
  // one is a hand-edited or half-built config, not a choice — and silently
  // reading it as steer picks a wire message nobody asked for (AGENTS.md).
  if (isBlank(value)) {
    throw new Error(
      `Move needs an action — expected one of ${[...MOVE_ACTIONS].join(', ')}`
    );
  }
...
function frameForAltRef(value) {
  ...
  if (isBlank(value)) {
    throw new Error('Move needs an altitude reference — expected home or msl');
  }
...
function frameForReference(value, firmware) {
  ...
  if (isBlank(value)) {
    throw new Error('Move needs a reference — expected world, body or offset');
  }
  if (value === 'body') {
