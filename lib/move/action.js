 ...
 *   (above home, MSL, or above terrain) is the only frame choice the operator
 *   ever sees.
...
/**
 * Altitude reference → global MAV_FRAME. Terrain packs the frame and nothing
 * else (pymavlink posture, owner ruling 2026-08-23): the vehicle resolves
 * height-above-terrain from its own terrain data, and one without any refuses
 * the command loudly — the driver serves no tiles and checks nothing.
 *
 * @param {*} value  'home' | 'msl' | 'terrain'
 ...
 * @param {{firmware: string}|null} profile  the Vehicle Profile rung; only
 *   body reads it, and the editor lets body through only with a profile
 * @returns {number|undefined}
 */
function frameForReference(value, profile) {
  switch (value) {
...
function deriveSteerMode(groups) {
  ...
  const has = (obj) => ['north', 'east', 'up'].some((k) => !isBlank(obj[k]));
  const name = [
