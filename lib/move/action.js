 ...
 * - `turn` — CONDITION_YAW. ArduPilot's *only* working yaw (§14): both of the
 *   yaw fields the position carriers already had are measured inert on Copter.
 * - `speed` — DO_CHANGE_SPEED, QGC's guided speed change.
 * - `attitude` — SET_ATTITUDE_TARGET, MAVSDK's Offboard attitude modes.
 * - `manual` — MANUAL_CONTROL: QGC's joystick, the way ArduSub is flown, and
 *   the only motion primitive Blimp has.
 *
...
function frameForReference(value, firmware) {
  ...
  // or `offset` resolves to world/LOCAL_NED, which is the coercion a blank
  // already got and the frame that works on every stack. Nothing is checked
  // here.
  //
