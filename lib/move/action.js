...
function deriveSteerMode(groups) {
  ...
  // Every combination is a mode: the name is just the filled groups joined,
  // and MODES carries all eight. Acceleration used to refuse alongside
  // position or velocity, which read a gap in that table as a rule about the
  // wire — the wire has an independent ignore bit per group, and ArduPilot
  // implements the combinations as named guided submodes (§14).
