 ...
 *   marked untrusted (unsigned-allowlisted, invalid-but-admitted) — advisory
 *   only; undefined = unsigned on a link that does not require signing — no
 *   signing regime is in play, so trusted-only gates must not read it as a mark
...
class SigningState {
  ...
  constructor(options = {}) {
    ...
    this._allowlist = new Set(DEFAULT_UNSIGNED_ALLOWLIST);
   ...
   * only the allowlist survives — and the allowlist is always untrusted (§7).
   * Without require-signed there is no signing regime to judge against, so the
   * verdict carries no trust mark at all — `false` is reserved for the
   * explicit untrusted mark that bars a frame from settling transactions, and
   * stamping it on every plain unsigned link would bar the normal case
   * (§7 trust ruling).
   *
   * @param {InboundFrame} frame
   * @returns {InboundVerdict}
  ...
  _acceptUnsigned(frame) {
    if (!this.requireSigned) return verdict(true, undefined, 'unsigned-accepted');
