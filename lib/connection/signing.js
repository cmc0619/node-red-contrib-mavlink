 ...
 * matching the HMAC); accept/reject **policy** and the timestamp store are ours.
 * This module owns only the policy and the store — it computes no HMAC.
 *
 ...
 * @property {number|null} linkId  link id from the packet's signature block
 *   (null when unsigned — the unsigned path never reads it)
 * @property {number|null} timestamp  48-bit signing timestamp, 10 µs units
 *   (null when unsigned, likewise unread)
 ...
 * @property {boolean|undefined} trusted  may this frame settle a transaction
 *   (§7 trust ruling): true = signed and verified; false = *explicitly*
 *   marked untrusted (invalid-but-admitted) — advisory only; undefined = traffic
 *   on a link without a verification key — no signing regime is in play, so
 *   trusted-only gates must not read it as a mark
...
class SigningState {
   ...
   * @param {boolean} [options.hasKey]  whether a verification key is configured
  ...
  constructor(options = {}) {
    ...
    /**
     * Highest inbound timestamp accepted as valid on ANY stream of this link —
     * a companion computer with no RTC/NTP has no other way to learn its clock
     * is behind real time. A cryptographically verified peer is the one
     * external time reference this module has; without folding it in, our own
     * outbound clock stays derived from local wall time alone, and a new peer
     * whose clock IS accurate can reject our very first packet to it at its
     * one-minute bootstrap floor (mavlink-audit-20260905 #11). One value for
     * the whole link, not per stream: it corrects local wall-clock error, a
     * property of this process, not of any single peer.
     * @type {number|undefined}
     */
    this._highestAcceptedInbound = undefined;
    /**
     ...
     * sharing one counter would show phantom gaps in both streams.
   ...
   * restarts without persistence (§7 "Outbound"). Also floored at the highest
   * timestamp accepted from any verified peer on this link — the one external
   * time reference available to a host with no RTC/NTP of its own, so a local
   * clock running behind real time does not fail bootstrap with the next new
   * peer it talks to (mavlink-audit-20260905 #11).
   *
   * @param {number} sysid
   * @param {number} compid
   * @param {number} [now]
  ...
  nextOutboundTimestamp(sysid, compid, now = this._now()) {
    ...
    const clock = this._highestAcceptedInbound === undefined
      ? timestampFromMs(now)
      : Math.max(timestampFromMs(now), this._highestAcceptedInbound);
  ...
  acceptInbound(frame, now = this._now()) {
    if (!this.hasKey) {
      return frame.signaturePresent
        ? verdict(true, undefined, 'signed-accepted-without-key')
        : verdict(true, undefined, 'unsigned-accepted');
    }
    if (!frame.signaturePresent) return this._rejectUnsigned();
    if (!frame.signatureValid) return this._acceptInvalid();
    return this._acceptValid(frame, now);
  }

  /**
   * Unsigned / v1 frame on a keyed link. A configured key puts inbound traffic
   * in the verification regime, so an absent signature is refused regardless
   * of whether this Connection signs outbound traffic.
   *
   * @returns {InboundVerdict}
   */
  _rejectUnsigned() {
    return verdict(false, false, 'unsigned-rejected-key-configured');
  }
  ...
  /**
   * Valid signature: the one-minute floor bootstraps first contact; after
   * that, monotonicity alone governs the established stream. A captured
   * packet can never exceed the last accepted timestamp, so re-checking the
   * floor there guarded nothing and locked out a peer whose clock lags local
   * time — no GPS lock, clock reset — even while its stream stayed strictly
   * monotonic (§7 signing). Advances the store only when the timestamp
   * is greater than the last accepted one for the stream.
   *
   * @param {InboundFrame} frame
   * @param {number} now
   * @returns {InboundVerdict}
  ...
  _acceptValid(frame, now) {
    ...
    if (last === undefined) {
      // Bootstrap only (§7 "First contact"): with no prior record the floor
      // is the sole replay defence, and without it no new peer is accepted.
      ...
      this._noteAccepted(frame.timestamp);
  ...
  /**
   * Raise the link-wide outbound clock floor to a freshly verified inbound
   * timestamp, if it is higher than anything accepted before. Called only
   * from the two `_acceptValid` branches that just wrote `_inbound` — never
   * from an unsigned or invalid-signature accept, matching this module's own
   * rule that only a genuinely verified packet may advance any store (§7 "On
   * accept").
   *
   * @param {number} timestamp
   */
  _noteAccepted(timestamp) {
    if (this._highestAcceptedInbound === undefined || timestamp > this._highestAcceptedInbound) {
      this._highestAcceptedInbound = timestamp;
    }
  }
}
...
function timestampFromMs(ms) {
  return Math.max(0, Math.floor((ms - MAVLINK_EPOCH_UNIX_S * 1000) * UNITS_PER_MS));
}

/**
 * @param {boolean} accept
 * @param {boolean|undefined} trusted
 * @param {string} reason
 * @returns {InboundVerdict}
 */
