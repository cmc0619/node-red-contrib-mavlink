 ...
 *   (§7 trust ruling #264): true = signed and verified; false = *explicitly*
...
class SigningState {
   ...
   * monotonic (§7 signing, #264). Advances the store only when the timestamp
  ...
  /**
   * Forget inbound replay memory (reconnect). The high-water marks described
   * the dead link's streams; keeping them would lock out a vehicle whose
   * clock reset while the link was down, because its post-reset timestamps
   * would read as replays. The next accepted frame per stream re-bootstraps
   * through the one-minute first-contact floor ({@link SigningState#_acceptValid}),
   * which is the replay defence while no record exists. Outbound state
   * (`_seq`, `_outbound`) is deliberately untouched: our own stream must stay
   * monotonic across the redial or the peer's replay defence starts dropping
   * our frames.
   */
  resetInbound() {
    this._inbound.clear();
