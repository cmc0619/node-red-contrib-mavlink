...
function tosFromDscp(dscp) {
  if (!Number.isInteger(dscp) || dscp < 0 || dscp > 63) {
    throw new RangeError('DSCP must be an integer between 0 and 63');
  }
  return dscp * 4;
}
...
function markWithSockopt(socket, tos, options) {
  ...
  // `sockopt` supports Node 18+, but obtains the fd through Node's deprecated
  // socket handle. Suppress that one synchronous implementation warning so a
  // successful optional optimisation does not spam the Node-RED log.
  withNoDeprecation(() => {
...
module.exports = { markSocket, tosFromDscp };
