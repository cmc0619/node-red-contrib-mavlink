...
// The admin endpoint's resolved `serialport` module — undefined until the
// first list, then the module, or null recording its absence. Lazy like
// loadSerialPort (UDP/TCP installs never touch the native dependency) and
// cached so the endpoint does not re-require per request.
let listedSerialPort;

/**
 * Enumerate host serial ports for the Connection editor's serial-path
 * dropdown (`GET /mavlink/serial-ports`). An absent optional `serialport`
 * dependency is an empty list, not an error — there is simply nothing to
 * enumerate, and the dropdown still takes free text.
 *
 * @returns {Promise<object[]>} serialport PortInfo records, [] when absent
 */
function listSerialPorts() {
  if (listedSerialPort === null) return Promise.resolve([]);
  if (listedSerialPort === undefined) {
    try {
      listedSerialPort = loadSerialPort();
    } catch (err) {
      if (err && err.code === 'SERIALPORT_MISSING') {
        listedSerialPort = null;
        return Promise.resolve([]);
      }
      throw err;
    }
  }
  return Promise.resolve(listedSerialPort.list());
}
