...
class SerialTransport extends EventEmitter {
  ...
  send(buffer, endpoint, callback) {
    ...
    const finish = (err) => {
      ...
      port.removeListener('error', onFail);
      port.removeListener('close', onFailClose);
      callback(err);
    };
    const onDrain = () => finish();
    const onFail = (err) => finish(err || new Error('Serial write aborted'));
    const onFailClose = () => {
      const err = new Error('Serial write aborted (port closed)');
      err.code = SERIAL_NO_DESTINATION;
      finish(err);
    };
    ...
    port.once('error', onFail);
    port.once('close', onFailClose);
  }
