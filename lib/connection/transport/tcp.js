 ...
 *
 * Events: `message` ({ buffer, address, port }), `endpoint-gone`
 * ({ address, port }) when a peer socket closes — server-mode accepted client
 * *or* client-mode remote disconnect — so the Connection can drop that stream
 * decoder, and `error`.
 */
...
const { writeDrained } = require('./drain');
...
/**
 * Bound on how long close() waits for a live socket's `end()`/`close` before
 * forcing it. A peer that never acks the FIN (paused, stalled, or gone
 * without a reset) would otherwise leave `end()`'s callback pending
 * indefinitely — the write-watchdog that would normally fault a stalled link
 * has already been disarmed by the time Connection close() gets here, so
 * nothing else bounds this wait (mavlink-audit-20260905 #10). Same value as
 * runtime.js's WRITE_TIMEOUT_MS: both are "how long we wait on one socket
...
class TcpTransport extends EventEmitter {
  ...
  /**
   * Open the client connection or server listener; resolves once ready.
   *
  ...
  close(callback) {
    ...
    if (this._socket) {
      ...
      const finish = () => {
        ...
        this._clearTimeout(closeTimer);
        socket.destroy();
        callback();
      };
      // A stalled peer that never acks the FIN would otherwise leave end()'s
      // callback (and 'close') pending forever; this is the one remaining
      // bound once Connection's own write watchdog has already been disarmed.
      // Every path into finish is registered below this line, so the timer
      // always exists by the time finish clears it.
      const closeTimer = this._setTimeout(finish, CLOSE_TIMEOUT_MS);
      closeTimer.unref();
  ...
  _write(socket, buffer, callback) {
    ...
    writeDrained(socket, buffer, set, (err) => {
