 ...
 * @property {string} [bindAddress]  listen address in server mode (the editor
 *   owns the 0.0.0.0 default); optional client-mode localAddress
...
class TcpTransport extends EventEmitter {
  ...
  _attachServerSocket(socket) {
    ...
    // the delayed close cannot wipe this socket's pipeline (Greptile #35).
    ...
    socket.on('data', (buffer) => {
      ...
      // own a fresh decoder after reconnect (Greptile #35).
  ...
  _resolveSocket(endpoint) {
    if (this._server && endpoint) {
      return this._clients.get(endpointKey(endpoint)) || null;
    }
    return this._selectSocket(endpoint);
  }
...
module.exports = { TcpTransport, TCP_NO_DESTINATION, TCP_PEER_GONE, TCP_CLOSED_DURING_OPEN };
