 ...
 *
 * Events: `message` ({ buffer, address, port }), `error`.
 */
 ...
 * @property {number} [broadcastPort]  port for that address; the editor
 *   requires it alongside the address (for multicast it is the bind port —
 *   every member joins the same group *and* port)
 */
...
class UdpTransport extends EventEmitter {
  ...
  open() {
    return new Promise((resolve, reject) => {
      // dgram picks the family from the socket type; a bind address with a
      // colon in it is IPv6.
      const type = this._config.bindAddress.includes(':') ? 'udp6' : 'udp4';
   ...
   * not a thing. Loopback stays at the OS default (on): ArduPilot's `mcast:`
   * exists so several tools on one host can share a link, and subnet broadcast
   * has no equivalent knob anyway — a bound socket receives those regardless.
   * The runtime drops our own echoed frames by bound identity instead.
   *
  ...
  broadcastDestination() {
    ...
    return { address, port: this._config.broadcastPort };
  }
   ...
   *   null selects the configured remote
   * @param {(err?: Error) => void} callback
   */
  send(buffer, endpoint, callback) {
    const { address, port } = endpoint === null
      ? { address: this._config.remoteAddress, port: this._config.remotePort }
      : endpoint;
    if (!address || !port) {
  ...
  setDscp(dscp) {
    return this._markDscp(this._socket, dscp, {
      family: this._socket.type === 'udp6' ? 'ipv6' : 'ipv4',
      logger: this._logger,
    });
  }
