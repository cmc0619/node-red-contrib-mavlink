...
/** Misconfig: only one of address/port is present — still worth a warn. */
const UDP_INCOMPLETE_DESTINATION = 'UDP_INCOMPLETE_DESTINATION';
...
class UdpTransport extends EventEmitter {
  ...
  send(buffer, endpoint, callback) {
    ...
    if (!address || !port) {
      // Half-configured remote is a real misconfig — do not silence it.
      const err = new Error('UDP send has incomplete destination (need both address and port)');
      err.code = UDP_INCOMPLETE_DESTINATION;
      callback(err);
      return;
    }
...
module.exports = { UdpTransport, UDP_NO_DESTINATION, UDP_INCOMPLETE_DESTINATION };
