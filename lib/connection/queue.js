...
class OutboundQueue {
  ...
  constructor(options = {}) {
     ...
     * only ever loses a mid-queue item to replacement, which re-enqueues at
     * the tail with a fresh seq, so the Map's insertion order (delete + set)
     * IS the band order and its first entry is the band's oldest.
     *
  ...
  _enqueueStreaming(item, explicitKey) {
    ...
    if (this._coalesce.delete(key)) {
      this._counts[BAND.STREAMING] -= 1;
    } else if (this._counts[BAND.STREAMING] >= this._capacity[BAND.STREAMING]) {
      // The Map's first entry is the band's oldest (insertion order = seq
      // order — coalescing re-inserts at the tail).
  ...
  /**
   * @param {number} band
   * @returns {QueueItem|null} the oldest item currently on `band`
   */
  _bandHead(band) {
    if (band === BAND.LIVENESS) return this._liveness.values().next().value || null;
    if (band === BAND.STREAMING) return this._coalesce.values().next().value || null;
  ...
  size() {
    return this._counts[0] + this._counts[1] + this._counts[2] + this._counts[3] + this._counts[4];
  }
