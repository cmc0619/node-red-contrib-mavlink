...
class OutboundQueue {
  ...
  enqueue(request) {
    ...
    switch (band) {
      ...
      case BAND.STREAMING: return this._enqueueStreaming(item);
      ...
      case BAND.BULK: return this._enqueueBounded(item, 'bulk transfer rejected (queue full)');
   ...
   * @returns {QueueItem}
   */
  _enqueueStreaming(item) {
    const key = streamingKey(item);
  ...
  /**
   * @param {number} band
   * @returns {QueueItem|undefined} the oldest item currently on `band`
   */
  _bandHead(band) {
    switch (band) {
      case BAND.LIVENESS: return this._liveness.values().next().value;
      case BAND.STREAMING: return this._coalesce.values().next().value;
      case BAND.EMERGENCY:
      case BAND.CONTROL:
      case BAND.BULK: {
        const fifo = this._fifos[band];
        return fifo.head < fifo.items.length ? fifo.items[fifo.head] : undefined;
      }
      default: break; // This space intentionally left blank (§5)
    }
    return undefined; // nothing matched: no behavior selected (§5)
  ...
  _removeHead(item) {
    switch (item.band) {
      case BAND.LIVENESS: this._liveness.delete(item.identityId); break;
      case BAND.STREAMING: this._coalesce.delete(item.coalesceKey); break;
      case BAND.EMERGENCY:
      case BAND.CONTROL:
      case BAND.BULK: {
