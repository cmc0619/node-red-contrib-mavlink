...
function matches(filter, decoded) {
  ...
  // with no target_system at all is addressed to no one and does not
  // (undefined never equals an id). A message that names a system but has
  // no target_component field — six in the bundled dialect: SET_MODE,
  // SET_HOME_POSITION, SET_GPS_GLOBAL_ORIGIN, CHANGE_OPERATOR_CONTROL,
  // CAMERA_STATUS, CAMERA_FEEDBACK — is the dialect's way of addressing
  // every component of that system, so the component rule reads only a
  // field that is there.
