 ...
 * @property {number} [toSysid]  match the message's own `target_system`
 *   field — 0 (broadcast) passes; a message with no such field does not
 * @property {number} [toCompid]  match the message's own `target_component`
 *   field — 0 (MAV_COMP_ID_ALL) passes, and so does a message that names a
 *   system but carries no component field at all (SET_MODE,
 *   SET_HOME_POSITION, …): the dialect addresses those to every component
...
class SubscriptionRegistry {
   ...
   *
   * @param {SubscriptionFilter} filter  {} matches everything
  ...
  subscribe(filter, handler) {
    ...
    this._nextId += 1;
    this._subs.set(id, { filter, handler });
    return () => {
...
function matches(filter, decoded) {
  ...
  // Addressed-to: the message's own target fields name a recipient — the
  // companion role's inbox. 0 is the broadcast value and passes. A message
  // with no target_system at all (HEARTBEAT, telemetry) is addressed to no
  // one and stays out whichever of the two filters is set. A message that
  // names a system but has no target_component field — six in the bundled
  // dialect: SET_MODE, SET_HOME_POSITION, SET_GPS_GLOBAL_ORIGIN,
  // CHANGE_OPERATOR_CONTROL, CAMERA_STATUS, CAMERA_FEEDBACK — is the
  // dialect's way of addressing every component of that system, so the
  // component rule reads only a field that is there.
