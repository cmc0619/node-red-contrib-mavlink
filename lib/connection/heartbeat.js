...
class HeartbeatScheduler {
  ...
  _baseIntervalMs() {
    // Fallback 1000 only when nothing is bound — never clamp slow identities
    // down to 1 Hz just because the constructor default is 1000.
    let min = null;
    for (const identity of this._identities.values()) {
      const n = this._identityIntervalMs(identity);
      min = min === null ? n : Math.min(min, n);
    }
    return min === null ? (this._intervalMs || 1000) : min;
  }
