...
/** Tick base when no identity is bound; every identity carries its own rate. */
...
class HeartbeatScheduler {
   ...
   *   identity per tick; a falsey result suppresses emission. Read as a plain
   *   boolean — an object is truthy however its members read, so a provider
   *   returning `{healthy: false}` would beat rather than fall silent
  ...
  tick() {
    ...
    for (const identity of this._identities.values()) {
      ...
      const healthy = Boolean(this._health(identity.id));
  ...
  /** @returns {number} smallest bound identity heartbeat interval */
  _baseIntervalMs() {
    // 1 Hz only when nothing is bound — never clamp slow identities down to
    // 1 Hz just because that is the tick base with an empty set.
    let min = null;
    for (const identity of this._identities.values()) {
      const n = identity.heartbeatIntervalMs;
      min = min === null ? n : Math.min(min, n);
    }
    return min === null ? DEFAULT_INTERVAL_MS : min;
  }
