...
const { endpointKey, DEFAULT_ENDPOINT_KEY } = require('./endpoint-key');
...
function buildWire(mav, bundle, signingKey, maxDecoders) {
  ...
  /**
   * node-mavlink's stock splitter skips any msgid absent from `magicNumbers`:
   * an unknown id's CRC seed lives in the message definition, so the frame is
   * CRC-unverifiable by construction, and the library's answer is silence.
   * Silence hides the one clue that diagnoses a dialect mismatch — the id
   * itself — so unknown ids pass validation here and surface as UNKNOWN_<id>
   * frames in decode().
   *
   * Declared once per wire rather than per decoder: `getDecoder` runs for
   * every new endpoint (up to `maxDecoders`, and again on every UDP source
   * churn), and re-evaluating a class body that never varies within a wire is
   * work for nothing.
   *
   * The return value 0 is node-mavlink's `PacketValidationResult.VALID` — the
   * enum is genuinely not on the package's runtime export surface, so there is
   * nothing to import and nothing to translate an `UNKNOWN` result into
   * either. The wire-decoders UNKNOWN test pins the pass-through, so an
   * upstream renumber or rename fails loud.
   *
   * `this.magicNumbers`, not the closure, on purpose: it is the exact
   * expression the base class consults, so "is this id known?" is one question
   * asked once rather than two lookups that merely happen to agree. It is also
   * what lets this class sit out here at all — it closes over nothing.
   */
  class DialectSplitter extends mav.MavLinkPacketSplitter {
    validatePacket(buffer, Protocol) {
      const header = new Protocol().header(buffer);
      const magic = this.magicNumbers[header.msgid];
      if (magic === null || magic === undefined) return 0;
      return super.validatePacket(buffer, Protocol);
    }
  }
  ...
  return {
     ...
     * @returns {number}
     */
    evictIdleDecoders(now = Date.now(), idleMs = 15_000) {
      let removed = 0;
      for (const [key, entry] of decoders) {
...
function assignFields(instance, fields, messageMeta) {
  ...
  for (const [i, snake] of names.entries()) {
    if (!Object.prototype.hasOwnProperty.call(fields, snake)) continue;
    const value = fields[snake];
    // node-mavlink writes integer fields with Buffer's write*Int* methods,
    // whose range check lets NaN and ±Infinity through as 0 (measured, §14) —
    // so an unresolved NaN target_system silently serializes as 0, the
    // broadcast address every vehicle on the link acts on. Refuse non-finite
    // integers here, the one choke point every outbound message crosses.
    // Checked via Number(value), not typeof: a string ('abc') hits the same
    // Buffer coercion as a bare NaN and must be caught too — and so do a
    // present-but-blank '' and null, which Number() turns into a finite 0 the
    // same silent way (Codacy/Sourcery, #305). Absent (undefined) stays
    // legal: a fixed-layout message fills unset fields with 0 by design, and
    // partial field objects (the brake packet, decoded round-trips) rely on
    // it — absent is the builder not speaking, blank is speaking emptily.
    // Floats/doubles stay untouched (a NaN float is legal MAVLink, "field not
    // used"); char is a string by contract; ARRAY fields are skipped outright
    // — Number([1,2]) is NaN by coercion accident, not a bad element, and the
    // guard's subject is the single-value target-id class (probed on #305:
    // the unskipped check refused GPS_STATUS's legitimate uint8[20]s).
    if (metaFields && typeof value !== 'undefined' && !metaFields[i].arrayLength) {
      const info = typeInfo(metaFields[i].type);
      if (info && info.kind !== 'float' && info.kind !== 'double' && info.kind !== 'char') {
        // isBlank, not an inline re-spelling: the enclosing if already
        // excludes undefined, which is the one member of isBlank's set that
        // must stay legal here.
        const blank = isBlank(value);
        if (blank || !Number.isFinite(Number(value))) {
          throw new Error(
            `field '${snake}' is ${blank ? 'blank' : value}; an integer field must be finite — ` +
            'node-mavlink would silently serialize it as 0 (broadcast, for a target id)'
          );
        }
      }
    }
    instance[snakeToCamel(snake)] = value;
  }
}
