...
function buildWire(mav, bundle, signingKey, maxDecoders) {
   ...
   *
   * 2 is UNKNOWN and 0 is VALID. `PacketValidationResult` is not on the
   * package's runtime export surface, so the numbers cannot be imported; the
   * wire-decoders UNKNOWN test pins the behaviour end to end, so an upstream
   * renumber fails loud. A CRC failure on a *known* id still returns INVALID
   * and is still dropped — untouched.
   *
