 ...
 * @returns {Wire}
 */
function buildWire(mav, bundle, signingKey) {
  ...
  /** @type {Map<string, {splitter: object, parser: object, lastUsed: number}>} */
  ...
  function getDecoder(epKey, now) {
    ...
    if (!entry) {
      ...
      entry = { splitter, parser, lastUsed: now };
  ...
  return {
     ...
     */
    decode(buffer, endpoint) {
      const epKey = endpointKey(endpoint);
      const { splitter, parser } = getDecoder(epKey, Date.now());
...
module.exports = { createWire, snakeToCamel, endpointKey, DEFAULT_ENDPOINT_KEY };
