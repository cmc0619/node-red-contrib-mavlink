 ...
 * imported (Gitar #344). The wire-decoders UNKNOWN test pins the behaviour end
 * to end, so an upstream renumber fails loud rather than silently reclassifying
 * frames.
 */
...
function buildWire(mav, bundle, signingKey, maxDecoders) {
  ...
  return {
    serialize(message, ctx) {
      ...
      // node-mavlink / Buffer write undefined|NaN integers as 0 (silent). Paint
      // core integers poison so an omitted field cannot ride the class default
      // 0 — closest we get to pymavlink's struct.pack refusing None (§14.55/56).
      // Extension ints keep class 0 (MAVLink 2 trailing truncation, §14.65).
      poisonIntegerFields(instance, messageMeta);
      assignFields(instance, message.fields, messageMeta);
      assertIntegerFieldsFinite(instance, messageMeta);
...
/**
 * True for a single scalar integer wire field (not float/char/array).
 * @param {{type: string, arrayLength?: number, extension?: boolean}|null|undefined} field
 * @returns {boolean}
 */
function isIntegerWireField(field) {
  if (!field || field.arrayLength) return false;
  const info = typeInfo(field.type);
  return !!(info && info.kind !== 'float' && info.kind !== 'double' && info.kind !== 'char');
}

/**
 * Core (non-extension) scalar integers — the fields poison-init covers.
 * MAVLink 2 extensions omit as trailing zeros by design (§14.65); class
 * default 0 matches that wire meaning, so they are not poisoned.
 *
 * @param {{type: string, arrayLength?: number, extension?: boolean}|null|undefined} field
 * @returns {boolean}
 */
function isCoreIntegerWireField(field) {
  return isIntegerWireField(field) && !field.extension;
}

/**
 * Replace class-default core-integer 0s with NaN so an omitted field cannot
 * reach Buffer as a silent zero. Extensions, floats, chars, and arrays are
 * left alone.
 *
 * @param {object} instance
 * @param {object} [messageMeta]
 */
function poisonIntegerFields(instance, messageMeta) {
  if (!messageMeta) return;
  for (const field of messageMeta.fields) {
    if (!isCoreIntegerWireField(field)) continue;
    instance[snakeToCamel(field.name)] = NaN;
  }
}

/**
 * Refuse any core integer field that is still blank or non-finite after
 * assign — leftover poison, explicit NaN, or Buffer-silent blanks.
 * Extensions stay at class 0 when omitted (§14.65).
 *
 * @param {object} instance
 * @param {object} [messageMeta]
 */
function assertIntegerFieldsFinite(instance, messageMeta) {
  if (!messageMeta) return;
  for (const field of messageMeta.fields) {
    if (!isCoreIntegerWireField(field)) continue;
    const snake = field.name;
    const value = instance[snakeToCamel(snake)];
...
function assignFields(instance, fields, messageMeta) {
  ...
  for (const [i, snake] of names.entries()) {
    ...
    // Integer undefined: do not overwrite poison — Buffer would write 0.
    // Blank/non-finite when speaking: refuse here (same choke as the post-assert).
    // Floats/doubles stay untouched (NaN float is legal MAVLink). Arrays skipped
    // — Number([1,2]) is NaN by coercion accident (#305).
    if (metaFields && isIntegerWireField(metaFields[i])) {
