...
function buildWire(mav, bundle, key) {
  const registry = buildRegistry(mav);
  ...
  const splitter = new mav.MavLinkPacketSplitter();
...
/**
 * Assemble a msgid→class registry from every bundled `node-mavlink` dialect
 * module that ships a REGISTRY.
 *
 * @param {object} mav
 * @returns {Object<number, Function>}
 */
function buildRegistry(mav) {
  const modules = ['minimal', 'standard', 'common', 'ardupilotmega'];
  const registry = {};
