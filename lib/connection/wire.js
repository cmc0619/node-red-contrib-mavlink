...
function buildWire(mav, bundle, signingKey, maxDecoders) {
  ...
  function getDecoder(epKey, now) {
    ...
    if (!entry) {
      ...
      //
      // node-mavlink's stock splitter skips any msgid absent from
      // `magicNumbers`: an unknown id's CRC seed lives in the message
      // definition, so the frame is CRC-unverifiable by construction, and the
      // library's answer is silence. Silence hides the one clue that
      // diagnoses a dialect mismatch — the id itself — so unknown ids pass
      // validation here and surface as UNKNOWN_<id> frames in decode().
      // The return value 0 is node-mavlink's PacketValidationResult.VALID
      // (the enum is genuinely not on the package's runtime export surface,
      // so there is nothing to import and nothing to translate an UNKNOWN
      // result into either); the wire-decoders UNKNOWN test pins the
      // pass-through, so an upstream renumber or rename fails loud.
      // `this.magicNumbers` — not the closure — on purpose: it is the exact
      // expression the base class consults, so "is this id known?" is one
      // question asked once, not two lookups that merely happen to agree.
      const splitter = new (class extends mav.MavLinkPacketSplitter {
