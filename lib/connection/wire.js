 ...
 * @property {number} [timestamp]  48-bit signing timestamp, 10 µs units since
 *   2015-01-01 (from SigningState); required when `sign` is set
 *
...
function buildWire(mav, bundle, signingKey, maxDecoders) {
  ...
  return {
    serialize(message, ctx) {
      ...
      const sign = !!(ctx.sign && ctx.key);
      // IFLAG_SIGNED must be set before serialize: it lives in the CRC'd header
      // and is what tells a receiver's splitter to expect the 13-byte signature.
      const protocol = new mav.MavLinkProtocolV2(
        ctx.sysid,
        ctx.compid,
        sign ? mav.MavLinkProtocolV2.IFLAG_SIGNED : 0,
      );
      const frame = protocol.serialize(instance, ctx.seq);
      if (!sign) return frame;
      // node-mavlink's sign() stamps a Date.now()-derived timestamp; the spec's
      // strictly-increasing per-stream value is SigningState's (§7 "Outbound"), so
      // overwrite it and recompute the HMAC, which covers the timestamp bytes.
      const signed = protocol.sign(frame, ctx.linkId || 0, ctx.key);
      const signature = new mav.MavLinkPacketSignature(signed);
      signature.timestamp = ctx.timestamp;
      signature.signature = signature.calculate(ctx.key);
      return signed;
    },
