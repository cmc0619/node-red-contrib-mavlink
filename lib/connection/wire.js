 ...
 * @property {() => number} [invalidPacketCount]
 */
...
function buildWire(mav, bundle, signingKey, maxDecoders) {
   ...
   *
   * The tally counts CRC *validation failures*, not distinct corrupt frames:
   * on INVALID the base splitter drops one byte and rescans from the next, so
   * a stray START byte inside a long corrupt payload can raise the verdict
   * again for the same physical frame (Gitar, #390). That suits the one job
   * this number has — a rising delta means the link is degrading — and it is
   * why nothing here reports it as a frame count.
   *
  ...
  class DialectSplitter extends mav.MavLinkPacketSplitter {
    validatePacket(buffer, Protocol) {
      ...
      if (result === PACKET_INVALID) invalidPackets += 1;
