...
function assignFields(instance, fields, messageMeta) {
  ...
  for (const [i, snake] of names.entries()) {
    ...
    // Floats stay untouched: a NaN float is legal MAVLink ("field not used").
    if (metaFields && typeof value === 'number' && !Number.isFinite(value)) {
      const info = typeInfo(metaFields[i].type);
      if (info && info.kind !== 'float' && info.kind !== 'double') {
        throw new Error(
