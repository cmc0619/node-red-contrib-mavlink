 ...
 * names keyed by the bundle) and `node-mavlink`'s message classes.
 *
 * `node-mavlink` is required the first time a wire is built, not at module
 * load, so the rest of `lib/connection` (queue, peer table, subscriptions,
 * signing policy) loads and tests without it.
 *
 ...
 * @property {(endpoint: {address: string, port: number}|null) => void} [releaseDecoder]
 ...
 * imported. The wire-decoders UNKNOWN and CRC-corruption tests
...
function createWire(options) {
  ...
  const maxDecoders = options.maxDecoders === undefined ? DEFAULT_MAX_DECODERS : options.maxDecoders;
  return buildWire(mav, options.bundle, options.key, maxDecoders);
}

/**
 * @param {object} mav  the loaded `node-mavlink` module
 * @param {object} bundle  dialect bundle
 * @param {Buffer|undefined} signingKey  signing key for inbound verification
...
function buildWire(mav, bundle, signingKey, maxDecoders) {
   ...
   * frame. A rising delta is the one job this number has, and
  ...
  class DialectSplitter extends mav.MavLinkPacketSplitter {
    validatePacket(buffer, Protocol) {
      // The v2 wire spec requires silently discarding a packet whose
      // incompatibility flags carry a bit this implementation does not
      // understand — an unrecognized bit means the frame's wire format may
      // differ in a way CRC alone cannot catch. IFLAG_SIGNED is the only bit
      // this driver implements. Not a corruption verdict (a dialect mismatch
      // isn't tallied as one either): the frame is simply not ours to read.
      if (
        Protocol === mav.MavLinkProtocolV2 &&
        (buffer[2] & ~mav.MavLinkProtocolV2.IFLAG_SIGNED) !== 0
      ) {
  ...
  /**
   * Longest a splitter's retained tail can legitimately be: the widest v2
   * header, the largest payload a one-byte LEN field can declare (255), the
   * CRC, and a signature block. node-mavlink's own splitter retains its
   * *entire* buffer, unbounded, whenever no start marker (0xFE/0xFD) appears
   * anywhere in it — a sender that never emits either byte (adversarial, or
   * just a misconfigured device sharing the port) grows that buffer forever;
   * the decoder-count cap above bounds how many endpoints exist, not how much
   * one of them can make its own splitter hold (mavlink-audit-20260905 #1).
   * Past this many retained bytes with nothing decoded out of them, the tail
   * is proven incapable of starting a legal frame — it is garbage, not a
   * still-arriving one.
   */
  const MAX_PENDING_BYTES =
    mav.MavLinkProtocolV2.PAYLOAD_OFFSET +
    255 +
    mav.MavLinkProtocol.CHECKSUM_LENGTH +
    mav.MavLinkPacketSignature.SIGNATURE_LENGTH;

  /** True when the splitter still holds an incomplete MAVLink frame. */
  function hasPendingPartial(entry) {
    const buf = entry.splitter.buffer;
  ...
  function getDecoder(epKey, now) {
    ...
    if (!entry) {
      ...
      // Two paused-mode streams, drained by hand in decode() so each frame is
      // returned with the datagram's endpoint still in scope. Not piped: a
      // pipe stalls at the parser's 16-object high-water mark and resumes on
      // a later tick, past the datagram (measured, §14).
      const splitter = new DialectSplitter({}, { magicNumbers });
      const parser = new mav.MavLinkPacketParser();
  ...
  return {
    serialize(message, ctx) {
      ...
      assertArraysFit(message.fields, messageMeta);
      const instance = new Clazz();
      assignFields(instance, message.fields, messageMeta);
      const willSign = ctx.sign;
    ...
    decode(buffer, endpoint, now = Date.now()) {
      ...
      // Discard proven garbage: past MAX_PENDING_BYTES with no frame decoded
      // out of it, nothing legal is still arriving in there.
      if (splitter.buffer.length > MAX_PENDING_BYTES) splitter.buffer = Buffer.alloc(0);
      for (let chunk = splitter.read(); chunk !== null; chunk = splitter.read()) parser.write(chunk);
      const frames = [];
      ...
      // proof that keeps a real peer out of the first eviction tier. An
      // UNKNOWN_<id> frame is CRC-unverifiable by construction, so a
      ...
      while ((packet = parser.read()) !== null) {
        ...
        const signaturePresent = Boolean(packet.signature);
        const signature = {
          signaturePresent,
          signatureValid:
    ...
    releaseDecoder(endpoint) {
      decoders.delete(endpointKey(endpoint));
    },
...
function coreIntegerFieldNames(messageMeta) {
  ...
  for (const field of messageMeta.fields) {
    if (field.constValue !== undefined || field.extension || field.arrayLength) continue;
    const info = typeInfo(field.type);
    if (info.kind === 'float' || info.kind === 'double' || info.kind === 'char') continue;
...
const arraySpecsByMeta = new WeakMap();

/** @param {object} messageMeta @returns {{name: string, arrayLength: number}[]} */
function arrayFieldSpecs(messageMeta) {
  let specs = arraySpecsByMeta.get(messageMeta);
  if (specs) return specs;
  specs = [];
  for (const field of messageMeta.fields) {
    if (field.constValue !== undefined || !field.arrayLength) continue;
    specs.push({ name: field.name, arrayLength: field.arrayLength });
  }
  arraySpecsByMeta.set(messageMeta, specs);
  return specs;
}

/**
 * An array field longer than its declared length must not pack. The
 * serializer loops `i < value.length && i < maxLen`, so the tail past
 * `maxLen` is dropped and the frame reports sent — the same silent false
 * success as an unspoken core int, one line up (measured 470#9: a float[8]
 * handed eleven values put eight on the wire). A *shorter* array is the
 * protocol's own shape and pads: every `data[]` with a companion `len` relies
 * on it, so length is only ever checked upward.
 *
 * @param {Object<string, *>} fields
 * @param {object} messageMeta
 */
function assertArraysFit(fields, messageMeta) {
  for (const { name, arrayLength } of arrayFieldSpecs(messageMeta)) {
    const value = fields[name];
    if (isBlank(value)) continue;
    if (value.length > arrayLength) {
      throw new Error(`invalid packet: ${name} carries ${value.length} entries, ${arrayLength} fit`);
    }
  }
}

/**
 * Copy wire values (snake_case, keyed by the bundle's field names) onto a
...
function assignFields(instance, fields, messageMeta) {
  for (const { name: snake, constValue } of messageMeta.fields) {
