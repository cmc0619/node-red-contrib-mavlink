...
function assignFields(instance, fields, messageMeta) {
  ...
  for (const [i, snake] of names.entries()) {
    ...
    // Buffer coercion as a bare NaN and must be caught too. Floats/doubles
    // stay untouched (a NaN float is legal MAVLink, "field not used"), as
    // does char (its wire-ready value is a string by contract, not a number).
    if (metaFields && typeof value !== 'undefined') {
      const info = typeInfo(metaFields[i].type);
      if (info && info.kind !== 'float' && info.kind !== 'double' && info.kind !== 'char'
          && !Number.isFinite(Number(value))) {
        throw new Error(
          `field '${snake}' is ${value}; an integer field must be finite — ` +
