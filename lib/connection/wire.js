...
function buildWire(mav, bundle, signingKey, maxDecoders) {
   ...
   * (1) never-validated (junk/spoof), (2) validated with empty splitter buffer,
   * (3) Map-LRU — so a live peer mid-frame is not the first casualty of source
   * churn (Greptile #33).
   *
  ...
  /**
   * Evict one pipeline under the cap: never-validated, then idle-complete
   * validated, then Map-LRU among remaining mid-frame entries.
   */
  function evictOne() {
