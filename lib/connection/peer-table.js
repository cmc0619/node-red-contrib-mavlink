...
class PeerTable extends EventEmitter {
  ...
  markPrimaryFailed(sysid, compid) {
    ...
    if (component.endpoints.size <= 1) {
      // Sole endpoint failed (e.g. TCP peer gone). Drop it so endpointFor
      // returns null — keep targeting a dead peer would spam loud send fails.
...
/**
 * @param {Endpoint} endpoint
 * @returns {string} `address:port` key — never address alone (§8)
 */
