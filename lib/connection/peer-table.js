...
class PeerTable extends EventEmitter {
  ...
  /**
   * Unique primary endpoints for a `target_system = 0` write.
   *
   * On a shared bus one datagram reaches everyone; on udpclient SITL each
   * vehicle has its own return path, so the same frame must be written to
   * every learned peer endpoint (compid-pinned, typically autopilot 1).
   * Expired components are skipped; stale ones still get the write.
   *
   * @param {number} compid
  ...
  endpointsForBroadcast(compid) {
    ...
    for (const peer of this._peers.values()) {
      ...
      if (!component || component.state === 'expired' || !component.primaryEndpoint) continue;
      const ep = component.endpoints.get(component.primaryEndpoint);
      if (!ep) continue;
