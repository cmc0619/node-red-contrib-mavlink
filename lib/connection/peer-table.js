...
class PeerTable extends EventEmitter {
   ...
   * period — one owner of the value, so the timer and the table cannot
   * disagree.
   *
   * @returns {number}
   */
  ...
  update(decoded, endpoint, now = this._now()) {
    ...
    if (!component.sections.heartbeat && component.state === 'stale') component.state = 'active';
  }
   ...
   * the *absence* of messages. It only ever demotes: a component returns to
   * active when a frame arrives — its HEARTBEAT, or any frame for a component
   * that sends none (see `update`) — never on a sweep. After
   * {@link PeerTable#demoteEndpoints} a still-fresh timestamp is not a
   * relearned route, and reviving on it put a member with no endpoint back in
   ...
   * {@link PeerTable#markPrimaryFailed}.
   *
   * @param {number} sysid
   * @param {number} compid
   ...
   * @param {number} [sysid]  scope to one already-known system: a directed
   *   send naming a real sysid and component 0 means "every component of
   *   *this* system" (§8 MAV_COMP_ID_ALL), a narrower question than every
   *   peer — an ordinary directed lookup would look for a component literally
   *   registered as compid 0, which no real peer ever advertises, and reach
   *   nobody (mavlink-audit-20260905 #6). Omitted: every system.
   ...
   */
  endpointsForBroadcast(compid, sysid) {
    const out = [];
    const seen = new Set();
    const peers = sysid === undefined ? this._peers.values() : [this._peers.get(sysid)];
  ...
  /**
   * A plain, JSON-serializable view of one live component on this table's
   * clock: what `snapshot()` lists and what the State feed emits. The live
   * component owns routing state (the endpoints Map, the primary key) and is
   * rewritten by every heartbeat, so it never leaves the table.
   *
  ...
  projectComponent(component) {
    return {
      ...
      missionCurrent: projectMissionCurrent(component.missionCurrent),
      gimbalManagers: [...component.gimbalManagers.values()].map((manager) => ({
        gimbalDeviceId: manager.gimbalDeviceId,
        information: manager.information ? { ...manager.information } : null,
        status: manager.status ? { ...manager.status } : null,
      })),
      autopilotVersion: component.autopilotVersion
        ? {
            ...component.autopilotVersion,
            capabilities: String(component.autopilotVersion.capabilities),
            flightCustomVersion: [...component.autopilotVersion.flightCustomVersion],
          }
        : null,
      ...
      statustext: component.statustext.map((row) => ({ ...row })),
    };
  }
  ...
  snapshot() {
    ...
    for (const peer of this._peers.values()) {
      ...
      for (const component of peer.components.values()) {
        components.push(this.projectComponent(component));
      }
   ...
   * reports 0 matched instead of a phantom success (§2). Only a frame
   * arriving on the new link clears the mark (see `sweep`), and it relearns
   * the primary endpoint in the same `update`. The cost is that a
   * still-heartbeating peer reads stale until its next heartbeat, and that
   * this transition carries no `stale` event: both are the price of not
   * lying about a dispatch.
   */
  demoteEndpoints() {
    for (const peer of this._peers.values()) {
      for (const component of peer.components.values()) {
...
const SOURCES = {
  HEARTBEAT(table, component, fields, now) {
    ...
    component.autopilot = fields.autopilot;
    component.systemStatus = fields.system_status;
    component.baseMode = fields.base_mode;
    component.flightMode = fields.custom_mode;
    component.armed = Math.floor(component.baseMode / SAFETY_ARMED_BIT) % 2 === 1;
    ...
    if (prevArmed !== null && component.armed !== prevArmed) {
      emitChanged(table, 'armed-changed', component, prevArmed, component.armed);
    }
    if (prevMode !== null && component.flightMode !== prevMode) {
      emitChanged(table, 'mode-changed', component, prevMode, component.flightMode);
    }
  },

  AUTOPILOT_VERSION(_table, component, fields) {
    component.autopilotVersion = {
      flightSwVersion: fields.flight_sw_version,
      boardVersion: fields.board_version,
      capabilities: fields.capabilities,
      flightCustomVersion: fields.flight_custom_version,
      vendorId: fields.vendor_id,
      productId: fields.product_id,
    };
    component.capabilities = fields.capabilities;
  },

  MISSION_CURRENT(table, component, fields, now) {
    const names = {
      seq: 'seq',
      total: 'total',
    ...
    const current = {};
    for (const [wire, projected] of Object.entries(names)) {
      if (Object.prototype.hasOwnProperty.call(fields, wire)) current[projected] = fields[wire];
    }
    component.missionCurrent = current;
    table._touchSection(component, 'missionCurrent', now, current);
  },

  GIMBAL_MANAGER_INFORMATION(table, component, fields, now) {
    let manager = component.gimbalManagers.get(fields.gimbal_device_id);
    if (!manager) {
      manager = { gimbalDeviceId: fields.gimbal_device_id, information: null, status: null };
      component.gimbalManagers.set(fields.gimbal_device_id, manager);
    }
    manager.information = {
      timeBootMs: fields.time_boot_ms,
      capFlags: fields.cap_flags,
      gimbalDeviceId: fields.gimbal_device_id,
      rollMin: fields.roll_min,
      rollMax: fields.roll_max,
      pitchMin: fields.pitch_min,
      pitchMax: fields.pitch_max,
      yawMin: fields.yaw_min,
      yawMax: fields.yaw_max,
    };
    table._touchSection(component, 'gimbalManagers', now);
  },

  GIMBAL_MANAGER_STATUS(table, component, fields, now) {
    let manager = component.gimbalManagers.get(fields.gimbal_device_id);
    if (!manager) {
      manager = { gimbalDeviceId: fields.gimbal_device_id, information: null, status: null };
      component.gimbalManagers.set(fields.gimbal_device_id, manager);
    ...
    manager.status = {
      ...
      flags: fields.flags,
      gimbalDeviceId: fields.gimbal_device_id,
      primaryControlSysid: fields.primary_control_sysid,
      primaryControlCompid: fields.primary_control_compid,
      secondaryControlSysid: fields.secondary_control_sysid,
      secondaryControlCompid: fields.secondary_control_compid,
    };
    table._touchSection(component, 'gimbalManagers', now);
  },
  ...
  SYS_STATUS(table, component, fields, now) {
    ...
    component.sysStatus = {
      sensorsPresent: fields.onboard_control_sensors_present,
      sensorsHealth: fields.onboard_control_sensors_health,
      batteryVoltage: fields.voltage_battery,
      batteryRemaining: fields.battery_remaining,
    };
    ...
    if (prevHealth !== null && health !== prevHealth) {
      // `changed`: the flipped MAV_SYS_STATUS_SENSOR bits, as one unsigned word.
      emitChanged(table, 'sensor-health-changed', component, prevHealth, health, {
  ...
  GPS_RAW_INT(table, component, fields, now) {
    ...
    component.gps = {
      fixType: fields.fix_type,
      satellites: fields.satellites_visible,
    };
    table._touchSection(component, 'gps', now, component.gps);
    if (prevFix !== null && component.gps.fixType !== prevFix) {
      emitChanged(table, 'gps-fix-changed', component, prevFix, component.gps.fixType);
    }
  },
  ...
  GLOBAL_POSITION_INT(table, component, fields, now) {
    component.position = {
      ...
      lon: fields.lon,
      alt: fields.alt,
      relativeAlt: fields.relative_alt,
      heading: fields.hdg,
    };
  ...
  BATTERY_STATUS(table, component, fields, now) {
    component.battery = {
      id: fields.id,
      remaining: fields.battery_remaining,
  ...
  HOME_POSITION(table, component, fields, now) {
    ...
    if (
      ...
      emitChanged(table, 'home-changed', component, projectHome(prevHome), projectHome(component.home));
    }
  },
  ...
  AVAILABLE_MODES(_table, component, fields) {
    ...
    // nothing gates on it.
    component.modes.set(fields.mode_index, {
      modeIndex: fields.mode_index,
      numberModes: fields.number_modes,
      standardMode: fields.standard_mode,
      customMode: fields.custom_mode,
      properties: fields.properties,
      name: fields.mode_name.replace(/\u0000+$/, ''),
    });
  },

  STATUSTEXT(table, component, fields) {
    const text = fields.text.replace(/\u0000+$/, '');
    component.statustext.push({ severity: fields.severity, text });
    if (component.statustext.length > table._opts.statustextHistory) component.statustext.shift();
    table.emit('statustext', { sysid: component.sysid, compid: component.compid, severity: fields.severity, text });
  },
};
...
/**
 * @param {object|null} current  raw MISSION_CURRENT fields in camelCase
 * @returns {object|null}
 */
function projectMissionCurrent(current) {
...
/**
 * Emit a `<field>-changed` event for a component whose tracked value moved.
 *
 * @param {PeerTable} table
 * @param {string} event
 * @param {object} component
 * @param {*} from
 * @param {*} to
 * @param {object} [extra]  event-specific fields alongside from/to
 */
function emitChanged(table, event, component, from, to, extra) {
  table.emit(event, { sysid: component.sysid, compid: component.compid, from, to, ...extra });
}
