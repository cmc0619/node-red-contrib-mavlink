...
class PeerTable extends EventEmitter {
  ...
  /**
   * Every distinct primary endpoint a `target_system = sysid, target_component
   * = 0` frame has to reach — MAV_COMP_ID_ALL (§8), scoped to one already-known
   * system instead of every peer. {@link PeerTable#endpointsForBroadcast}'s
   * `compid === 0` case answers "every component of every system"; a directed
   * send naming one real sysid and component 0 means "every component of
   * *this* system," which is a different, narrower question — treating it as
   * an ordinary directed lookup instead looked for a component literally
   * registered as compid 0, which no real peer ever advertises, so the send
   * silently reached nobody (mavlink-audit-20260905 #6).
   *
   * @param {number} sysid
   * @returns {{address: string, port: number}[]} possibly empty — the caller
   *   reads that as "fall back to the configured remote", same as a directed
   *   send to an unlearned endpoint.
   */
  endpointsForSystemBroadcast(sysid) {
    const peer = this._peers.get(sysid);
    if (!peer) return [];
    const out = [];
    const seen = new Set();
    for (const component of peer.components.values()) {
      if (!component.primaryEndpoint) continue;
      const ep = component.endpoints.get(component.primaryEndpoint);
      if (!ep) continue;
      const key = `${ep.address}:${ep.port}`;
      if (seen.has(key)) continue;
