...
class PeerTable extends EventEmitter {
   ...
   *
   * `state` is deliberately untouched. Liveness is sweep()'s to decide from
   * `lastSeen`, which a redial does not change: a peer still heartbeating is
   * still active, and one that is gone goes stale on sweep's own schedule
   * with the `stale` event that every other transition carries. Marking it
   * here would announce nothing and would hide the whole swarm from
   * `selectFanoutMembers`, which reads that field, until the next sweep.
   */
  demoteEndpoints() {
    for (const peer of this._peers.values()) {
      for (const component of peer.components.values()) {
