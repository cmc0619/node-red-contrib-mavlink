...
class PeerTable extends EventEmitter {
  ...
  update(decoded, endpoint, now = this._now()) {
    // No vehicle sources the broadcast id: a frame claiming sysid 0 would mint
    // a phantom peer 0 that walks into fan-out's `all` selection as a
    // broadcast-shaped member (§7 ruling #264).
    if (decoded.sysid === 0) return;
  ...
  /**
   * Demote every component's routing state without dropping its history
   * (reconnect): the learned endpoints described the dead link, so primacy is
   * forgotten and each record is marked stale; the first frame on the fresh
   * transport re-establishes primacy from live evidence (§8). Modes, section
   * data and statustext stay — mavlink-state keeps reading them while the
   * link is down. The stale mark is a signal, not a latch: sweep() owns
   * state transitions and returns a still-fresh record to active on its own
   * schedule.
   */
  demoteEndpoints() {
    for (const peer of this._peers.values()) {
