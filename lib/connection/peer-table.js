...
class PeerTable extends EventEmitter {
   ...
   */
  /**
   * The projection of one live component on this table's clock, for the
   * State feed: the same view `snapshot()` lists, with section ages that
   * agree with it under an injected clock.
   *
   * @param {object} component
   * @returns {object}
   */
  projectComponent(component) {
    return projectComponent(component, this._now());
  }
  ...
  snapshot() {
    ...
    for (const peer of this._peers.values()) {
      const components = [];
      for (const component of peer.components.values()) {
        components.push(projectComponent(component, this._now()));
      }
 ...
 */
/**
 * A plain, JSON-serializable view of one component: what `snapshot()` lists
 * and what the State feed emits. The live component owns routing state (the
 * endpoints Map, the primary key) and is rewritten by every heartbeat, so it
 ...
 * @returns {object}
 */
function projectComponent(component, now) {
  return {
    compid: component.compid,
    type: component.type,
    ...
    sections: sectionAges(component, now),
...
module.exports = { PeerTable, projectComponent };
