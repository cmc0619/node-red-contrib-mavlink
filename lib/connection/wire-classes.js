...
function buildMessageClass(mav, message) {
  ...
  const inits = ordered.map((f) => {
    ...
    if (f.constValue !== undefined) return { name, kind: 'constant', value: f.constValue };
  ...
  const cls = class extends mav.MavLinkData {
    constructor() {
      super();
      for (const { name, kind, value } of inits) {
        switch (kind) {
          case 'string': this[name] = ''; break;
          case 'array': this[name] = []; break;
          case 'bigint': this[name] = BigInt(0); break;
          case 'constant': this[name] = value; break;
          case 'zero': this[name] = 0; break;
...
/**
 * Synthesize a wire class for every message in a compiled dialect bundle. The
 * registry is built from the bundle alone, and the metadata compiler already
 * refuses name/id collisions inside a bundle, so every message maps to a class.
 *
 * @param {object} mav  loaded `node-mavlink` module
 * @param {object} bundle  compiled dialect bundle
 * @returns {Function[]}
 */
function synthesizeWireClasses(mav, bundle) {
  return Object.values(bundle.messages).map((message) => buildMessageClass(mav, message));
}

module.exports = { synthesizeWireClasses, snakeToCamel };
