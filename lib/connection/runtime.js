...
class Connection extends EventEmitter {
  ...
  _enqueueOutbound(message, band, identity, target) {
    ...
    const envelope = { name: message.name, message: deepCopy(message), identity, target };
  ...
  _onFrame(frame, endpoint, now) {
    ...
    // unsigned-allowlisted or invalid-accepted frame is admitted but
    // untrusted — either could be forged to redirect directed sends and
    // broadcast fan-out at a spoofed address. Both still dispatch; they just
    // teach the table nothing.
