 ...
 * (§7). The node does not construct a Connection when disabled.
 */
...
/**
 * Upper bound on one transport write's completion callback. All
 ...
 * always wins because close() cancels the pending attempt. A TCP client also
 * gets three initial retries; other initial transport failures stay terminal.
 */
const RECONNECT_BASE_MS = 1000;
const RECONNECT_CAP_MS = 30_000;
const INITIAL_TCP_RETRIES = 3;
...
class Connection extends EventEmitter {
   ...
   * @param {object} config.signing  { linkId, signOutbound, acceptInvalid,
   *   hasKey, key }
   * @param {object} config.heartbeat  { staleMs, expireMs }
   * @param {object} deps  injectable collaborators (now, timers, logger,
   *   transportFactory, createWire, wire, dgram). `deps.resolveIdentity` is
   *   required: the Connection node injects it, and every SITL script and
   *   test does the same. No default and no guard (§0).
   */
  ...
  /**
   * Frames this Connection received whose CRC failed on a msgid the bound
   * dialect carries — link corruption, monotonic since deploy. Reads 0 before
   * `start()` builds the wire, and for the injected wires the runtime's own
   * tests run against.
   *
   * @returns {number}
   */
  crcFailureCount() {
    if (!this._wire || typeof this._wire.crcFailureCount !== 'function') return 0;
    return this._wire.crcFailureCount();
  }

  /**
   * Build the runtime and open the transport.
   *
   * @returns {Promise<void>} resolves once listening or reconnecting; rejects
   *   on a terminal startup failure
   */
  async start() {
    ...
    this.queue = new OutboundQueue({ now: this._now });
    this.subscriptions = new SubscriptionRegistry({ logger: this._logger });
    this.peerTable = new PeerTable({
      now: this._now,
      heartbeatStaleMs: this._config.heartbeat.staleMs,
      heartbeatExpireMs: this._config.heartbeat.expireMs,
    });
  ...
  /** @returns {boolean} whether the bound transport is a TCP client */
  _isInitialTcpClient() {
    switch (this._config.transport.mode) {
      case 'tcp':
        return this._config.transport.remoteAddress && this._config.transport.remotePort;
  ...
  _resolveOutboundIdentity(identityId) {
    const resolve = this._deps.resolveIdentity;
   ...
   * Resolves exactly as send() does, and throws exactly where it would.
   *
  ...
  resolveSourceIds(identityId) {
    ...
    return { sysid: identity.sysid, compid: identity.compid };
  }

  /**
   * Enqueue an outbound message. Resolves the identity — an override rides as
   * given, never replaced by the default (§13), and an id this connection does
   * not carry throws here — serialize-validates, and hands the envelope to the
   * band queue; the driver sends it.
   *
   * @param {object} message  decoded-shape message { name, fields }
   * @param {object} [options]
   * @param {number} options.band
  ...
  send(message, options = {}) {
    ...
    // backstop that keeps _draining from wedging). A dry run: seq 0 and
    // timestamp 0, so no signing counter or sequence number is consumed — but
    // signed when the link signs, so a missing key is the signer's own
    // refusal here, not an envelope dropped in _pump after the node reported
    // `sent`.
    this._wire.serialize(message, {
      sysid: identity.sysid,
      compid: identity.compid,
      seq: 0,
      sign: this.signing.signOutbound,
      linkId: this.signing.linkId,
      key: this._config.signing.key,
      timestamp: 0,
    });
    const item = this._enqueueOutbound(message, options.band, identity, options.target);
    this._pump();
    return item;
  }
  ...
  close(done) {
    ...
    // CLOSED into ERROR from a link that was deliberately taken down.
   ...
   * @param {{sysid: number, compid: number}|undefined} target
   * @returns {object}
   */
  _enqueueOutbound(message, band, identity, target) {
    // The queue owns its copy. Out hands the same object downstream before
    // the pump serializes it, and the inbound path already copies per
    // subscriber (clone.js): a Function node editing the payload under
    // backpressure must not change what leaves the wire.
    const envelope = { name: message.name, message: deepCopy(message), identity, target: deepCopy(target) };
  ...
  _pump() {
    ...
    const isBroadcast = Boolean(target) && target.sysid === 0;
    // Component 0 is MAV_COMP_ID_ALL: "every component of the named system,"
    // not a literal component id — no real peer ever registers as compid 0,
    // so treating this as an ordinary directed lookup reaches nobody
    // (mavlink-audit-20260905 #6). Only meaningful for a real, nonzero sysid;
    // `isBroadcast` above already owns sysid 0.
    const isSystemBroadcast = Boolean(target) && target.sysid !== 0 && target.compid === 0;
    const isFanOut = isBroadcast || isSystemBroadcast;
    const endpoint = target && !isFanOut
    ...
    // target until an endpoint appears, then re-arm. The re-arm
    ...
    // while it is known. A fan-out does not warn: it has destinations of its
    // own (below), and before any peer is heard the configured remote is the
    // correct and only path.
    if (target && !isFanOut && !endpoint && !this._fallbackWarned.has(`${target.sysid}/${target.compid}`)) {
    ...
    // fallback. A fan-out (full broadcast, or one system's compid=0) resolves
    // in preference order:
    //
    //   1. A configured broadcast address (multicast group or subnet
    //      broadcast). The medium does the fan-out, so one write is both
    //      correct and the whole point of configuring it — the frame still
    //      names its real target, so a system-scoped send reaches only that
    //      system even though every peer is on the wire.
    //   2. Every learned matching endpoint (every peer for a full broadcast,
    //      one system's own components for a system-scoped one). Neither
    //      reaches everyone by itself on a star, where each vehicle has its
    //      own return path — "everyone [that matches]" is N writes.
    ...
    if (isBroadcast) {
      const configured = this._broadcastDestination();
      const learned = configured ? [] : this.peerTable.endpointsForBroadcast(target.compid);
      destinations = configured ? [configured] : (learned.length ? learned : [endpoint]);
    } else if (isSystemBroadcast) {
      const configured = this._broadcastDestination();
      const learned = configured ? [] : this.peerTable.endpointsForSystemBroadcast(target.sysid);
      destinations = configured ? [configured] : (learned.length ? learned : [endpoint]);
    }
    ...
    for (const destination of destinations) {
      ...
      this._transport.send(buffer, destination, (err) => {
        ...
        if (failure && !TRANSPORT_QUIET_SEND_CODES.has(failure.code)) {
          ...
          // into a permanently forgotten address. Truly dead
          // peers age out via the expiry sweep instead. A fan-out has no
          // single primary to demote, and one unreachable component must not
          // cost the others their addresses.
          if (target && !isFanOut && !TRANSPORT_TRANSIENT_SEND_CODES.has(failure.code)) {
    ...
    // Bound the completion callback: a transport that accepts the
   ...
   * untrusted frame (invalid-accepted) is delivered flagged so nothing
   * downstream treats it as authenticated (§7).
   *
  ...
  _onFrame(frame, endpoint, now) {
    // System 0 is the broadcast destination, never a source identity. Admit
    // only addressable nodes before peer learning or subscription dispatch.
    if (frame.sysid === 0) return;
    ...
    // Peer learning is gated on evidence (§8): only a CRC-verified frame that
    // signing did not explicitly mark untrusted may record where its sender
    // lives. An UNKNOWN_<id> frame is unverifiable by construction, and an
    // invalid-accepted frame is admitted but untrusted — either could be
    // forged to redirect directed sends and broadcast fan-out at a spoofed
    // address. Both still dispatch; they just teach the table nothing.
    const learnsEndpoint = frame.crcVerified && verdict.trusted !== false;
    this.peerTable.update(decoded, learnsEndpoint ? endpoint : null, now);
  ...
  _startSweep() {
    // The thresholds are read back from the peer table — the sweep runs on
    // its stale period and the UDP idle-decoder TTL is its expire period, so
    // the timer and the table can never disagree.
    ...
    this._sweepTimer = this._setInterval(() => {
      ...
      const mode = this._config.transport.mode;
      if (
        mode === 'udp' &&
        this._wire &&
  ...
  /** Disarm the in-flight write's completion timer. */
  _disarmWriteTimer() {
  ...
  _onTransportError(err) {
    ...
    // timer must not fire a second, spurious transport-error later.
    this._disarmWriteTimer();
    // Stop the heartbeat scheduler: it would only pile frames into a pump that
    // does not drain; reconnect restarts it. The sweep stays
    ...
    if (this._closed) return;
    if (this._everConnected || this._isInitialTcpClient()) {
      this._scheduleReconnect();
      return;
    }
  ...
  _scheduleReconnect() {
    ...
    const exhausted = !this._everConnected
      && this._initialReconnects === INITIAL_TCP_RETRIES;
    ...
    if (dead) {
      ...
      // no listener throws: the §2 process-kill. So the corpse
    ...
    if (!this._everConnected) this._initialReconnects += 1;
    this._logger.warn(
    ...
    this._reconnectTimer = this._setTimeout(() => {
      ...
      // loud, locatable, and not pretending to recover.
  ...
  async _attemptReconnect() {
    ...
    if (this._transport !== transport) {
      // An error event landed while open() was settling:
    ...
    const recovering = this._everConnected;
    this._reconnectAttempts = 0;
    // A link that just came up resumes live traffic, never stale intent (owner
    // ruling, 2026-08-18): a frame queued against a dead link describes an
    // aircraft that has moved on by the time the link returns, so it does not
    // ride. Acked frames stay loud: their waiters arm at send() and never see
    // an ack for a frame that never left, so each will time out
    // (the sender's ack timeout, 10 s by editor default) — reconnect usually beats that
    // clock, so most are still pending here — whereas transmitting one late
    // could land a command the flow has already given up on: the phantom
    // action §2 forbids. Send-tier frames have no
    // waiter: the flow saw `sent` when they were queued and hears nothing
    // here. That is the phantom `sent` of §14.131, accepted by the ruling in
    // this one place because an unacked MAVLink frame carries no delivery
    // report on any link — only the acked tiers ever could. Cleared at the
    // last moment before the pump resumes, so the outage has no tail; the
    // initial TCP retry path lands here and reads the same.
    this.queue.clear();
    if (recovering) {
      // Demote, do not clear, the peers: their endpoints described the dead
      // link, so primacy is forgotten and the first frame on the fresh
      // transport re-establishes it from live evidence (§8). History, modes
      // and section data stay — mavlink-state keeps reading them while the
      // link is down (the sweep kept running for exactly that reason).
      this.peerTable.demoteEndpoints();
    } else {
      this._startSweep();
    }
    this._ready = true;
