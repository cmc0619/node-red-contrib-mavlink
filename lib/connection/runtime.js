...
class Connection extends EventEmitter {
  ...
  async _attemptReconnect() {
    ...
    // ride. Acked frames stay loud either way: their waiters arm at send() and
    // never see an ack for a frame that never left, so each has timed out
    // already or will (DEFAULT_TIMEOUT_MS, lib/command/ack.js), whereas
    // transmitting one late could land a command the flow has already been
    // told failed — the phantom action §2 forbids. Send-tier frames have no
