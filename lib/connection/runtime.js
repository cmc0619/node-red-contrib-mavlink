...
class Connection extends EventEmitter {
   ...
   * assertion landing after close() is dropped: a lease on a closed link is
   * meaningless, and a timer armed here after close() cleared the lease map
   * would fire into torn-down state (§7 teardown race — the same class the
   * write and reconnect timers guard).
   *
   * @param {string} identityId
