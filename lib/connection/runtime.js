...
class Connection extends EventEmitter {
  ...
  async _attemptReconnect() {
    ...
    // transport re-establishes it from live evidence (§8). History, modes and
    // section data stay — mavlink-state keeps reading them while the link is
    // down (the sweep kept running for exactly that reason).
    this.peerTable.demoteEndpoints();
    // Inbound replay memory likewise described the dead link's streams. The
    // next signed frame per stream re-bootstraps through the first-contact
    // floor, so a vehicle whose clock reset during the outage is not locked
    // out as a "replay". Outbound sequence and timestamps stay monotonic —
    // resetInbound() touches only the inbound store.
    this.signing.resetInbound();
    this._ready = true;
    this._setState(STATE.CONNECTED);
