...
class Connection extends EventEmitter {
  ...
  _pump() {
    ...
    const endpoint = target && !isBroadcast
      ...
      : null;
    // Broadcast: one serialized frame, written to every learned peer endpoint
    // (udpclient star). Pre-peer (empty table) still uses the configured remote.
    const broadcastEndpoints = isBroadcast
    ...
    // while it is known. Broadcast (sysid 0) does not warn — either the peer
    // table supplies destinations or the configured remote is the pre-peer path.
    ...
    const destinations =
      isBroadcast && broadcastEndpoints && broadcastEndpoints.length > 0
        ? broadcastEndpoints
        : [endpoint];
    ...
    for (const dest of destinations) {
      if (typeof this._transport.setDscp === 'function') {
        this._transport.setDscp(item.dscp, dest);
      }
      this._transport.send(buffer, dest, (err) => {
        ...
        if (firstErr) {
          ...
          if (!TRANSPORT_QUIET_SEND_CODES.has(firstErr.code)) {
            this._logger.warn(`outbound send failed: ${firstErr.message}`);
            ...
            // peers age out via the expiry sweep instead. Broadcast has no
            // single primary to demote.
            if (
              target &&
              !isBroadcast &&
              !TRANSPORT_TRANSIENT_SEND_CODES.has(firstErr.code)
            ) {
