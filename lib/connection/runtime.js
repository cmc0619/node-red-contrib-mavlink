...
class Connection extends EventEmitter {
  ...
  _onDatagram(datagram) {
    ...
    const frames = this._wire.decode(datagram.buffer, endpoint);
    const now = this._now();
  ...
  _startSweep() {
    ...
    const idleMs =
      (this._config.heartbeat && this._config.heartbeat.expireMs) || 30_000;
    this._sweepTimer = this._setInterval(() => {
      this.peerTable.sweep();
      // UDP peers have no socket close — drop idle stream decoders so a dead
      // rinfo cannot keep a poisoned partial-frame buffer forever.
      if (this._wire && typeof this._wire.evictIdleDecoders === 'function') {
