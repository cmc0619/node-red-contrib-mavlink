...
class Connection extends EventEmitter {
  ...
  send(message, options = {}) {
    ...
    // enqueue()'s switch answers a band it does not recognise by selecting
    // nothing and returning undefined (§5). Swallowing that here would let the
    // sending node report a phantom green `sent` for a frame that never
    // entered the queue — the same §2 "no phantom success" the dry-run
    // serialize above enforces, so it throws on the same synchronous path.
    if (!item) {
      throw new Error(`no queue band selected for band ${band} — nothing enqueued`);
    }
    this._pump();
    return item;
  }
