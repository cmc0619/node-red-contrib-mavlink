  ...
  async start() {
    ...
    this.heartbeats = new HeartbeatScheduler({
      ...
      emit: ({ identity, message }) => {
        ...
        // the configured remote, starving every other star peer. Component 1
        // because `endpointsForBroadcast` takes its compid literally and the
        // autopilot is each vehicle's return path — the same broadcast target
        // fan-out uses.
        this._enqueueOutbound(message, BAND.LIVENESS, identity, { sysid: 0, compid: 1 });
        this._pump();
      },
    });
