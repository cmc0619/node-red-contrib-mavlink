...
class Connection extends EventEmitter {
  ...
  _pump() {
    ...
    // fallback. A broadcast has one per learned peer, because `target_system
    // = 0` only reaches everyone by itself on a shared medium; on a star each
    // vehicle has its own return path. An empty peer table falls back to the
    // same `null` as a directed send, which is the pre-peer path.
    //
    // The frame is serialized once either way: N datagrams carrying one
    // sequence number, not N frames burning N.
    const learned = isBroadcast ? this.peerTable.endpointsForBroadcast(target.compid) : null;
    const destinations = learned && learned.length ? learned : [endpoint];
