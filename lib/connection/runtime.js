...
class Connection extends EventEmitter {
  ...
  async _attemptReconnect() {
    ...
    // ruling, 2026-08-18). Everything queued against the down link already
    ...
    // success §2 forbids. Cleared here, at the last moment before the pump
    // resumes, so the outage has no tail. The initial TCP retry path lands here
    // too and the ruling reads the same: frames queued while dialing died with
    // their waiters.
    this.queue.clear();
    if (recovering) {
