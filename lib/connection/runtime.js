...
class Connection extends EventEmitter {
  ...
  close(done) {
    ...
    // _wire is null until start() builds it, so close-before-start skips this.
    if (this._wire) this._wire.clearDecoders();
    this._ready = false;
  ...
  _startSweep() {
    ...
    this._sweepTimer = this._setInterval(() => {
      ...
      if (mode === 'udp') this._wire.evictIdleDecoders(this._now(), idleMs);
    }, period);
