...
class Connection extends EventEmitter {
  ...
  async _attemptReconnect() {
    ...
    // ruling, 2026-08-18). Acked traffic queued against the down link already
    // reported its loud failure — ack waiters time out in seconds — so
    // transmitting those frames now would have the vehicle act on commands the
    // flows were told failed: a phantom action, the mirror of the phantom
    // success §2 forbids. Send-tier frames carry no delivery promise on a
    // lossy wire: dropped here they are indistinguishable from frames
    // transmitted the instant before the link died, so they clear with the
    // rest rather than arriving seconds stale. Cleared at the last moment
    // before the pump resumes, so the outage has no tail; the initial TCP
    // retry path lands here and reads the same.
    this.queue.clear();
    if (recovering) {
