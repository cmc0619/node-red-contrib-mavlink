...
class Connection extends EventEmitter {
  ...
  _pump() {
    ...
    // target until an endpoint appears, then re-arm (issue #91).
    if (target) {
      const targetKey = `${target.sysid}/${target.compid}`;
      if (!endpoint) {
        if (!this._fallbackWarned.has(targetKey)) {
          this._fallbackWarned.add(targetKey);
          this._logger.warn(
            `no known endpoint for target ${targetKey} — sending via the configured remote (if any)`
          );
        }
      } else {
        this._fallbackWarned.delete(targetKey);
      }
    }
    ...
    this._transport.send(buffer, endpoint, (err) => {
      ...
      if (err) {
        ...
        if (!TRANSPORT_QUIET_SEND_CODES.has(err.code)) {
          ...
          // peers age out via the expiry sweep instead.
          if (target && !TRANSPORT_TRANSIENT_SEND_CODES.has(err.code)) {
  ...
  _onTransportError(err) {
    ...
    // so stop the periodic work too: heartbeats would only pile into a pump
    // that never drains, and the sweep would tend a peer table nothing reads
    // (issue #93). close() does the same teardown.
    if (this.heartbeats) this.heartbeats.stop();
    this._stopSweep();
