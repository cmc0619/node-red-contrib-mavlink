 ...
 * registry, one signing channel state, and one heartbeat scheduler per bound
 * identity. It decodes everything arriving against its single bound Vehicle
 * Profile — one dialect, one firmware, no per-packet lookup (§7).
 *
...
const { createTransport, UDP_NO_DESTINATION } = require('./transport');
...
class Connection extends EventEmitter {
   ...
   * @param {object} [config.heartbeat]  { intervalMs, staleMs, expireMs }
  ...
  async start() {
    ...
    this.heartbeats = new HeartbeatScheduler({
      intervalMs: this._config.heartbeat && this._config.heartbeat.intervalMs,
      logger: this._logger,
    ...
    this._transport = this._transportFactory(this._config.transport, { dgram: this._deps.dgram });
  ...
  _pump() {
    ...
    const buffer = this._wire.serialize(envelope.message, {
    ...
    this._transport.send(buffer, endpoint, (err) => {
      this._draining = false;
      if (err) {
        // Listen-only UDP / no peer learned yet: drop quietly. Heartbeats and
        // speculative sends are normal before a remote exists; warn spam is not.
        if (err.code !== UDP_NO_DESTINATION) {
  ...
  _onDatagram(datagram) {
    ...
    const frames = this._wire.decode(datagram.buffer);
    const now = this._now();
  ...
  _startSweep() {
    ...
    this._sweepTimer = this._setInterval(() => this.peerTable.sweep(), period);
