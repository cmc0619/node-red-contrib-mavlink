...
class Connection extends EventEmitter {
  ...
  _pump() {
    ...
    // while it is known. Broadcast (sysid 0) is exempt — no endpoint is the
    // definition of correct there.
    if (target && target.sysid !== 0 && !endpoint && !this._fallbackWarned.has(`${target.sysid}/${target.compid}`)) {
